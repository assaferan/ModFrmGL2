...
intrinsic Norm(L::Lat) -> RngElt
{The positive generator of the ideal generated by (x,x) for x in L}

  require IsExact(L) : "Argument 1 must be an exact lattice";
  G, d:= IntegralMatrix( GramMatrix(L) );
  norm:= GCD(Set(Eltseq(2*G)) join Set(Diagonal(G)));
  if d ne 1 then norm := norm / d; end if;
  return norm;
end intrinsic;
