...
/*                                                          */
/* Revision version and last date: v1.1   2015/09/28        */
/*                                 v1.2   2015/10/05        */
/*                                 v1.3   2015/12/18        */
/*                                 v1.4   2015/12/30        */
/*                                 v1.5   2016/02/06        */
/*                                 v1.6   2016/05/11        */
/*  (func. Z4StandardForm renamed  v1.7   2016/06/30        */
/*   to StandardFormInfoInverted                            */ 
/*   and moved to CodesOverZ4.m)                            */
/*                                 v1.8   2017/11/02        */  
/*                                                          */
/************************************************************/
...
import "CodesOverZ4_uab.m": DoublePerm;
...
set of gamma+2*delta coordinate positions such that |C^J_bin|=2^(gamma+2*delta).
   ...
   _, perm := StandardFormInfoInverted(C);
   permInv := perm^(-1);
...
set of gamma+2*delta coordinate positions such that |C^J_bin|=2^(gamma+2*delta).         
    ...
    if (maxI le n) and ((gamma + delta) eq k) then
        checkSet := {1..n} diff I;
        deltaCp, gammaCp := Z4Type(PunctureCode(C, checkSet));
...
InformationSpace(C) applied to the dual code of C, given by function DualZ4(C).
}  
    require (BaseRing(C) eq Z4): "Code must be over Z4";
    require (#C ne 4^Length(C)): "Code cannot be the universe code";

    InfRSpace, InfVSpace := InformationSpace(DualZ4(C));
...
so the parameter AlgMethod is not relevant.
    ...
    Gs, perm := StandardFormInfoInverted(C);
    Cs := LinearCode(Gs);
...
PermutationDecodeInitialization := function(C, I, S, n, k, numSymG)
    ...
    GStandard :=  StandardFormInfoInverted(LinearCode(Gs));
