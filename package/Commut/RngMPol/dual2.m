...
function get_star_star_map(M,dual,dm,dd,dm2,can_mod)
// compute the M -> M** map
...
//print "Computing solutions...";
	...
	[wtc-wtd : wtc in can_mod`wts, wtd in dual`wts]);
      return rec<GradModMap |
...
{ Computes the graded double dual M** = Hom(Hom(M,can_R/I), can_R/I)
  where I is the annihilator of equidimensional module M over
  polynomial ring R. If retMap is true then the natural map M->M**
  is also computed and returned.}
    ...
    if retMap then
      mp :=  get_star_star_map(M,dual,dm,dd,dm2,
	((I`quotient_module)`M)`canonical_module);
      return dd,mp;
    else
      return dd;
    end if;
...
function mod_saturation_f(B,f)
// B is a basis for a submodule of P^n and f a non-zero
// element of P=k[x1,..xr] (usually with grevlex ordering)
// computes a basis for (<B>:f^infty) in P^n

    if #B eq 0 then return B; end if;
    P := Generic(Parent((Eltseq(B[1]))[1]));
    r := Rank(P); n := #(Eltseq(B[1]));
    P1 := PolynomialRing(BaseRing(P),r+1,"elim",1);
    hm := hom<P->P1|[P1.i : i in [2..r+1]]>;
    hm1 := hom<P1->P|[0] cat [P.i : i in [1..r]]>;

    F := EModule(P1,n,"top");
    S := sub<F|[F![hm(e) : e in Eltseq(b)] : b in B] cat
	[((P1.1)*f1-1)*F.i : i in [1..n]]> where f1 is hm(f);
    Groebner(S);
    B1 := Basis(S);
    B1 := [b : b in B1 | LeadingTerm(b) lt (P1.1)*F.1];
    return [[hm1(e) : e in Eltseq(b)] : b in B1];

end function;

function mod_saturation(M)
// M is a submodule of P^n, P := k[x1,..,xr]. Returns
// an M-complemetary set of vectors in P^n, B (possibly empty),
// s.t. M1 := <M,B> = &meet[(M:xi^infty) : 1 <= i <= r]

    F := Generic(M);
    B := [F!b : b in Basis(M)];
    P := BaseRing(F);
    M1 := sub<F|[F!b : b in mod_saturation_f(B,P.1)]>;
    for i in [2..Rank(P)] do
      M1 := M1 meet sub<F|[F!b : b in mod_saturation_f(B,P.i)]>;
    end for;
    return [b : b in Basis(M1) | b notin M];

end function;


function SaturateSubmodule(M,B,mps)
// M is a (saturated) graded module and B is the basis for a submodule.
// mps (poss. empty) is a sequence of homs from M to itself
// that preserves <B> <= M.
// Computes graded module N which is the saturation of <B> in M
// w.r.t. the maximal homogeneous ideal of the base polynomial
// ring of M. Also returns the restriction of the homs in mps
// to N

    G := Generic(M`M);
    G1 := (M`type le 1) select EModule(BaseRing(G),Rank(G)) else G;
    if M`type eq 1 then
      RM := RelationModule(M`M);
      M1 := sub<G1|[G1!b : b in B] cat [G1!b : b in Basis(RM)]>;
      bRel := true;
    else
      M1 := sub<G1|[G1!b : b in B]>;
      bRel := false;
    end if;
    B1 := mod_saturation(M1);
    B1 := [(M`M)!b : b in B1];
    if bRel then B1 := [b : b in B1 | b notin RM]; end if;
    N := sub<M`M|B cat B1>;
    B1 := Basis(N);
    mps1 := [Homomorphism(N,N,Matrix([Eltseq(N!(hm((M`M)!b))) : b in B1]) :
		Check := false) where hm is m`map : m in mps];
    return N,mps1;
...
// ASSUMING M/M[max^infty] IS EQUIDIMENSIONAL HERE!
    ...
    hpol := HilbertPolynomial(M);
    d := Degree(hpol)+1; // dim(M)
    R := BaseRing(M);
    n := Rank(R);
    ...
    //  R module at the end.
    try
      DM,dmp,hmsd := GradedDualWithHoms(GMprj, ideal<Ry|>, hms);
      DDM,ddmp,hmsdd := GradedDualWithHoms(DM, ideal<Ry|>, hmsd);
    catch e
    ...
    if hpol ne HilbertPolynomial(DDM`M) then
    // DDM is bigger than Mmax. Mmax is the Ry variable saturation of
    // M in DDM. Compute this now. Must first get the M -> M** map.
      ssmp := get_star_star_map(GMprj,DM,dmp,DDM,ddmp,
	GradedFreeModule(Ry,[Rank(Ry)]));
      N,xmps := SaturateSubmodule(DDM,[(ssmp`map)(b) : b in Basis(Mprj)],
		hmsdd);
    else
      N := DDM`M; xmps := [hm`map : hm in hmsdd];
    end if;

    // recover M1 as an R module from (Ry module) DDM
    M1 := StoRModule(N,R,[Matrix(hm) : hm in xmps],hinv);
