...
// finds a basis for degree n part of I/J, I >= J homog. ideals
    ...
    P := Generic(I);
    Mons_n := Setseq(MonomialsOfDegree(P,n)); // This order matters!
    Mons_n_iset := {@ m: m in Mons_n @};
    V := KSpace(BaseRing(P),#Mons_n);

    function make_vec(e)
	v := V!0;
	C, M := CoefficientsAndMonomials(e);
	for i := 1 to #M do
	    v[Index(Mons_n_iset, M[i])] := C[i];
	end for;
	//assert v eq V![MonomialCoefficient(e,m):m in Mons_n];
	return v;
    end function;
...
//"b:", b; "d:", d; "Monoms:", Monoms;

        //Ivecs cat:= [V![MonomialCoefficient(e,m):m in Mons_n] where
        Ivecs cat:= [make_vec(e) where e is b*m :m in Monoms];
   end for;
   //WI := sub<V|Ivecs>;
   WI := Rowspace(Matrix(Ivecs));
   delete Ivecs;
	...
	*/
        Jvecs cat:= [make_vec(e) where e is b*m :m in Monoms];
   end for;
   //WJ := sub<V|Jvecs>;
   WJ := Rowspace(Matrix(Jvecs));
   delete Jvecs;
