...
function PrintField(F)
  if AbsoluteDegree(F) eq 1 then return "Q"; 
  elif Type(F) eq FldQuad then  return "Q(sqrt("*Sprint(F.1^2)*"))";
  elif Type(F) eq FldCyc then return "Q(zeta_"*Sprint(CyclotomicOrder(F))*"))";
  end if;
  B:=BaseField(F); R<x>:=PR(B); defpol:=DefiningPolynomial(F);
  if Type(defpol) eq SeqEnum then defpol:=[R|x: x in defpol];
  else defpol:=R!defpol; end if;
...
{Print an Artin representation}
  if level eq "Minimal" then printf "Artin representation A "; return; end if;
  if level eq "Magma" then
    printf "%o !! %o",Sprint(A`K,"Magma"), DelCRs(Eltseq(Character(A)));
  else
    if not IsCharacter(Character(A)) or
       (not assigned A`K`artinrepdata`ramified)
      then condstr:="";
...
function IdentifyConjugacyClassQp(data,F,r)
  ...
  if v-vnext lt prec or vnext gt prec*2 then return 0; end if;
  // MW: Dec 2017, check vnext versus Universe prec also! 
...
function LocalData(K,p: prec:=20)
  ...
  try // prec:=prec+Valuation(A`disc,p); // MW, Nov 2017
    vprint ArtRep,1: Sprintf("Local data at p=%o; vdisc=%o; prec=%o",
                              p,Valuation(A`disc,p),prec);
    R,Zp,rts,D,I,ramgps,frob,act:=
     ComputeSplittingField(f,p: prec:=prec, Gal:=true);
    n:=#rts;
    ...
    class_map_prec:=1;                           // for the class map
    repeat  
      classmap:=[IdentifyConjugacyClassQp(A,c[3],rtsclassmap): c in CCD];
      if (0 notin classmap) and
          forall{rho: rho in A`C | IsCharacter
				   (Universe(CD)! [rho(CC[classmap[i]]) :
							i in [1..#CCD]])}
         then break; end if;
      class_map_prec*:=3;
      vprint ArtRep,2: "Class map: prec -> *"*Sprint(class_map_prec);
      error if class_map_prec gt 81,
      "Not enough precision to construct the class map";
      R2,m:=pAdicChangePrecision(R,class_map_prec*Precision(R));
...
procedure ComputeArtinRepresentations(~K:
  ...
  mbnd:=func<ev|n*Max([Abs(x): x in Eltseq(ev)])*rabs[1]^Degree(ev)*rabs[2]>;
  ...
  for j:=1 to #cycs do
    ...
    pow:=Sort(SetToSequence(&meet{Set(ConjugatePowers(G,cc[c])) :
				  c in classes}));
...
If not possible, return 0,false}
  ...
  repeat
    p:=NextPrime(p);
    if (Kdata`disc mod p eq 0) or (Fdata`disc mod p eq 0)
...
{A restricted to the smallest number field K such that A factors through the
  ...
  if not ok then
    if not Optimize then return A; end if; // MW Jul 2018, Tim returned false?!
    ...
    AMin:=K!!A;
    return AMin; // MW Jul 2018, Tim had A here??!
  end if;
...
function GenArtinCompose(A1,opfun,A2: charsonly:=false)
  ...
  /* //! Minimize? A1:=Minimize(A1); A2:=Minimize(A2); */
  ...
  // F:=OptimizedRepresentation(F); // MW, Jul 2018, make nicer reps?
  // seems much slower though
...
{Artin representations that factor through Q, i.e. just the principal
 character of Gal(Qbar/Q)}
  return ArtinRepresentations(RationalsAsNumberField():
			      Ramification:=Ramification,
      FactorDiscriminant:=FactorDiscriminant, p0:=p0, f:=f); end intrinsic;

intrinsic ArtinRepresentations(K::FldNum: Ramification:=false,
  FactorDiscriminant:=false, p0:="default", f:="default") -> SeqEnum
{All irreducible Artin representations
 that factor through the normal closure of K.}
...
{L-series of an Artin representation}
  ...
  return LSeries(1,gamma,Conductor(A),loc:
		 Parent:=A, Precision:=Precision, Sign:=sign);

end intrinsic;

intrinsic
 HodgeStructure(A::ArtRep : PrintHodgeVector:=false,PHV:=false) -> HodgeStruc
{The Hodge structure of an Artin representation}
  ...
  return HodgeStructure(ARR : PrintHodgeVector:=PrintHodgeVector, PHV:=PHV); 
 end intrinsic;
...
{The principal character of Gal(Qbar/Q) as an Artin representation}
  return ArtinRepresentations(Q)[1]; end intrinsic;
...
// should go elsewhere
intrinsic
EulerFactor(chi::GrpDrchElt,p::RngIntElt :
	    R:=Order(chi) le 2 select Rationals() else ComplexField())
-> RngUPolElt
{The Euler factor of a Dirichlet character at a prime}
  require IsPrime(p): "p must be prime"; 
  P:=PolynomialRing(R); return P!Polynomial([1,-chi(p)]); end intrinsic;
...
{Convert a Dirichlet character chi to an Artin representation A.
 ...
 specified by the field parameter.}
  ...
  if field cmpeq "undefined" then
    K:=OptimisedRepresentation
	     (AbsoluteField(NumberField(AbelianExtension(chi))));
  else
    K:=field;
  end if;
...
intrinsic OptimizedRepresentation(A::ArtRep) -> ArtRep
{Optimize the underlying field of an Artin representation}
 return OptimizedRepresentation(Field(A))!!A; end intrinsic;
...
intrinsic RationalDecomposition(A::ArtRep) -> SeqEnum
{Given an Artin representation, decompose it into constituents
 with rational-valued characters}
 ch:=Character(A); K:=Field(A); CT:=K`artinrepdata`C;
 CFS:=ClassFunctionSpace(CT); RC:=RationalCharacterTable(GaloisGroup(K));
 RCD:=RationalCharacterDecomposition(ch); A:=[];
 for i in [1..#RC] do if RCD[i] eq 0 then continue; end if;
//  s:=&+[CT[j]*D[j] : j in [1..#D] | D[j] ne 0]
//         where D:=Decomposition(CT,CFS!Eltseq(RC[i]));
  A cat:=[<K!!(CFS!Eltseq(RC[i])),RCD[i]>]; end for; return A; end intrinsic;
