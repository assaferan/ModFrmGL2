...
function is_coercible_seq_enum(R,E)
  if &and[IsCoercible(R`BaseRing, E[i]) : i in [1..#E]] then
    x :=New(SkwPolRngElt); x`Parent:=R; xC := E;
    while xC ne [] and xC[#xC] eq 0 do Prune(~xC); end while;
    x`Coefficients:= [(R`BaseRing)!xC[i] : i in [1..#xC]]; return true, x;
  else return false, "Illegal coercion"; end if; end function;

function is_coercible_rngupolelt(R,C,e) Cf:=[];
  E:=Eltseq(e); k:=R`BaseRing; d:=Degree(k) div Degree(CoefficientRing(C));
  if E eq [] then return true,R![0]; end if;
  for i in [1..#E - 1] do Append(~Cf,E[i]);
   for j in [1..d-1] do Append(~Cf,0); end for; end for;
  Append(~Cf,E[#E]); Rf:=R!Cf; return true, Rf; end function;

intrinsic IsCoercible(R::SkwPolRng,x::Any) -> BoolElt, .
{Whether or not x coerces into a skew polynomial ring}
 t:=Type(x); if ISA(t,SeqEnum) then return is_coercible_seq_enum(R,x); end if;
 if ISA(t,RngUPolElt) then
  C:=Centre(R); b,e:=IsCoercible(C,x);
  require b: "Polynomial must coerce into the center of skew polynomial ring";
  return is_coercible_rngupolelt(R,C,e); end if;
 if ISA(t,RngElt) then return IsCoercible(R,[x]); end if;
 if ISA(t,SkwPolRngElt) and x`Parent eq R then return true,x; end if;
