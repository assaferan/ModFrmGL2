...
//h;
  ...
  // Makes a huge diff in time; AHom not needed (AKS, Oct 2017):
  A := Hom(pc, M);
  return pc, A!ims;
end function;

InjHull := function(M)
//return injective hull P of M and map phi:M->P
  local G, K, J, entry, RQ, rho, IS, I, sno, pi, homs, pc, proj, B, N, h, JN,
        CISi, MQ, ISiQ, PSi, SP, rh;
  vprint KGModule, 1:
   "Computing injective hull of module of dimension", Dimension(M);
  G := Group(M);
  K := BaseRing(M);
  ComputeInfo(G,K);
  entry := [ t : t in G`modrepinfo | assigned t`field and t`field cmpeq K ][1];
  ...
  homs := [**];
  vprint KGModule,2: "getting injective homomorphisms";
  B := Basis(M);
  for i in [1..#IS] do
    CISi := sub< M | [IS[j] : j in [1..#IS] | j ne i] >;
    MQ, rho := quo< M | CISi >;
    ISiQ := rho(IS[i]);
    PSi :=  pi[ sno[i] ];
    SP := Socle(PSi);
    ...
    repeat
      rh := Random(h);
    until not ISiQ subset Kernel(rh);
    homs[i] := AHom(M, PSi)![b @ rho @ rh : b in B ];
  end for;

  pc, inj := DirectSum( [ pi[sno[i]] : i in [1..#IS] ]);
  vprint KGModule,1: "projective cover has dimension",Dimension(pc);
  //work out images of basis elements under projection
  ims := [];
  for b in B do
    Append(~ims, &+[ b @ homs[i] @ inj[i] : i in [1..#IS]] );
  end for;
  return pc, AHom(M, pc)!ims;
end function;
