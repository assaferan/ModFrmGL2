...
intrinsic PermutationModule(G::GrpPerm, R::Rng: SetCharacter := 0) -> ModGrp
{The natural permutation G-module of G over the ring R}

    M := InternalPermutationModule(G, R);

    if Characteristic(R) eq 0 and
	(SetCharacter cmpeq true or SetCharacter cmpeq 0 and assigned G`Classes)
    then
...
intrinsic GModule(G::GrpPerm, R::Rng: SetCharacter := 0) -> ModGrp
{The natural permutation G-module of G over the ring R}
