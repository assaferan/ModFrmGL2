...
declare attributes ModRng: tcond_info, modular_morphism,
    modular_orig_M, modular_orig_BB, orig_TM, uc_TM;
...
// printf "td: %o, diffs: %o, %o\n", td, td_diff, best_diff;
	...
	SPLIT_NZ := false;
	if SPLIT_NZ then // hack to force all components mapping to non-zero
	    "Orig C:", C;
	    CC := [t[1]: t in SplitChars];
	    "New CC:", CC;
	end if;

	if icond or tcond then
	    D := [Z | InnerProduct(Restriction(c, K), K1c): c in CC];
	elif tcond then
	    D := [SD[i, 1]: i in WantIndex];
	else
	    D := cond_traces(K, G!1, CC, f, cf);
	end if;
...
G := PSL(2, 251);
M := IrreducibleModules(G, RationalField() : MaxDegree := 400);
*/

if best_K cmpeq 0 then
...
*/
	...
	*/
    ...
    procedure setup_icond_action(icond_info, ~icond_MQ, ~icond_IQ)
	if icond and icond_MQ cmpeq 0 then
    ...
    // dim: Cd*oschur
    function get_BB(s, mor, dim, icond_MQ, icond_IQ)
	if tcond then
	    /*
	    function uc(v)
		F := BaseRing(v);
	    ...
	    */
	    ...
	    >;
	    */
	    uc_TM := func<B | puncond_basis_mat(B, G, K)>;
	    spin := G;
	end if;

	function uc(v)
	    F := BaseRing(v);
	    //"par v:", Parent(v); "par mor:", Parent(mor);
	    v := v*Matrix(F, mor);
	    return uc_TM(v);
	end function;
    ...
    if Type(BaseRing(S[1])) eq FldFin then
	vprint ZReps: "MOD CASE; return without splitting";
	assert #C eq 1 and #CD eq 1;
	i := 1;
	oschur := WantSchurIndices[i];
	Cd := Z!Degree(C[i]);
	setup_icond_action(icond_info, ~icond_MQ, ~icond_IQ);
	s := S[1];
	RQ := get_BB(s, s`modular_morphism, Cd*oschur, icond_MQ, icond_IQ);
	Mi := s`modular_orig_M;
	mor := Morphism(Mi, TM);
	//"HERE Cd*oschur:", Cd*oschur; "HERE Mi:", Mi; "HERE CD:", CD;
	srank := Cd*oschur * ExactQuotient(Dimension(Mi), Z!CD[1]);
	s`modular_orig_BB := get_BB(
	    Mi, mor, srank, icond_MQ, icond_IQ
	);
	BQ := 0;
	return;
    end if;
...
//"Got morphism:", mor; Parent(mor);
		    ...
		    if Blackbox then
			RQ := get_BB(s, mor, Cd*oschur, icond_MQ, icond_IQ);
			BQ := 0;
			return;
		    end if;
...
//"Do uc new v:", v;
			...
			// Feb 17: use Character now (Char sometimes slow)
