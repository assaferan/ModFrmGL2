...
MinPoly(X) = x^2 - x}
    ...
    vprint ZMeataxe: "Split via Conic:", E;
	...
	if FieldGen cmpne 0 then
	    ...
	    if Opt then
		K<w> := NumberField(f);
		vprint ZMeataxe: "First K:", K;
		vprint ZMeataxe: "Get OptimizedRepresentation";
		vtime ZMeataxe: OK := OptimizedRepresentation(K);
		vprint ZMeataxe, 2: OK;
		word := Polynomial(Eltseq(K!OK.1));
		vprint ZMeataxe: "Move to OK";
		vtime ZMeataxe: W := Evaluate(word, W);
		delete word;
    ...
    vprint ZMeataxe: "Field:", K;

    w_is_square, sqrt_w := IsSquare(w);
    vprint ZMeataxe: "w_is_square:", w_is_square;
    if w_is_square then
	vprint ZMeataxe: "sqrt_w:", sqrt_w;
    else
	sqrt_w := 0;
    end if;
...
//"HACK", 41; for i1 in [41..#MO], i2 in [1..i1-1] do
	...
	vprintf ZMeataxe: "Try generators %o, %o\n", i1, i2;

	A := sub<MatrixRing(GFp, nc) | matsp>;
	dim := Dimension(A);
        if d ne dim then
	    vprint ZMeataxe: "Do not generate";
	    continue;
	end if;
        count +:= 1;

	vprint ZMeataxe: "Dim:", dim;
	...
	for i := 1 to Nrows(N) do
	    ...
	    */
	    ...
	    vprint Meataxe, 2: "Rel:", i, r;
	    Append(~rels, r);
	end for;

	rels := Reduce(rels);

	vprint ZMeataxe: "rels:", rels;
	...
	procedure try_monomials(a, b, ~pt)
	    ...
	    vprint ZMeataxe: "Monomial case:", ac, ae, bc, be;

	    if not (w_is_square or ae mod 2 eq 0 and be mod 2 eq 0) then
		vprint ZMeataxe: "Not square";
		return;
	    end if;
		...
		"Try subfield", SK;
		C := Conic([SK | 1, ac, bc]);
		vprintf ZMeataxe: "IsLocallySolvable... ";
		vtime ZMeataxe: solvable := IsLocallySolvable(C);
		vprint ZMeataxe: solvable;
		if solvable then
		    break;
		end if;
	    end for;
	    ...
	    vprint ZMeataxe: "Get Rational Point";
	    vtime ZMeataxe: l, pt := HasRationalPoint(C);
	    assert l;
	    vprint ZMeataxe: "Point over subfield:", pt;
	...
	IndentPush();
	vprint ZMeataxe, 2: "a:", a;
	vprint ZMeataxe, 2: "Norm a:", na;
	vprint ZMeataxe, 2: "Min poly degree a:", Degree(MinimalPolynomial(a));
	vprint ZMeataxe, 2: "b:", b;
	vprint ZMeataxe, 2: "Norm b:", nb;
	vprint ZMeataxe, 2: "Min poly degree b:", Degree(MinimalPolynomial(b));

	SK := sub<K | a, b>;
	vprint ZMeataxe, 2: "Subfield for a & b:", SK;
	vprintf ZMeataxe, 2: "Subfield degree: %o (out of %o)\n",
	Degree(SK), Degree(K);
...
//NicerQuaternionAlgebra := 1;
    ...
    if 1 eq 1 then
	try
	    vprintf ZMeataxe: "NicerQuaternionAlgebra ... ";
	    vtime ZMeataxe:
	    ...
	    vprintf ZMeataxe: "Skip since failed (%o)\n", e;
	end try;
    else
	//"SKIP NICE";
    end if;
	...
	vprint Meataxe, 2: "a:", a;
	vprint Meataxe, 2: "Norm a:", na;
	vprint Meataxe, 2: "Min poly degree a:", Degree(MinimalPolynomial(a));
	vprint Meataxe, 2: "b:", b;
	vprint Meataxe, 2: "Norm b:", nb;
	vprint Meataxe, 2: "Min poly degree b:", Degree(MinimalPolynomial(b));
	...
	if pt cmpne 0 then
        ...
        prod := na*nb;
	vprint ZMeataxe: "prod:", prod;
IndentPop();

	if prod lt best_prod then
	    vprint ZMeataxe: "NEW BEST!";
	    best_prod := prod;
...
//"BREAK OUT"; break;
	...
	vtime ZMeataxe: bool, pt := HasRationalPoint(conic);
	SetVerbose("Conic", v);
	assert bool;
    end if;

    vprint ZMeataxe: "Point:", pt;
