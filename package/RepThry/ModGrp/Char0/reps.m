...
/*
Computing irreducible rational modules (written over Z) of finite groups.
...
function char_table_dp(G, D, P, Pf, Pfi, Gf)
    /*
    Return CharacterTable(G), where G is known to be isomorphic
    to P = DirectProduct(D) (with maps Pf, Pfi), with iso Gf: G -> P.
    */
	...
	Xi := 0;
	for j := 1 to i - 1 do
	    l, Hf := IsIsomorphic(D[j], H);
	    if l then
//printf "%o: reuse CT %o\n", i, j;
		Xj := CT[j];
		Xi := [CharacterOfImage(c, Hf, H): c in Xj];
		break;
	    end if;
	end for;
	if Xi cmpeq 0 then
    ...
    CR := CharacterRing(G);
    if 1 eq 1 then
	CP := [Y: Y in CartesianProduct(CT)];
	Y := [[*0: j in [1 .. #CP]*]: i in [1 .. #CP]];
//time
	for j := 1 to #C do
	    x := Gf(C[j, 3]);
	    xq := [Pfi[i](x): i in [1 .. d]];
	    for i := 1 to #Y do
		t := CP[i];
		Y[i, j] := &*[t[k](xq[k]): k in [1 .. #t]];
	    end for;
	end for;
//"Final coerce to CR"; time
	X := [];
	for c in Y do
	    x := CR![v: v in c];
	    x`IsCharacter := true;
	...
	X := [];
	for t in CartesianProduct(CT) do
	    c := [];
	    for u in C do
		x := Gf(u[3]);
		v := &*[t[k](Pfi[k](x)): k in [1 .. #t]];
		Append(~c, v);
	    end for;
	    Append(~X, CR!c);
	end for;
    ...
    Sort(~X, func<a, b | Degree(a) - Degree(b)>);

    return X;
end function;

function MyCharacterTable(G: Characters := [])

    C := Classes(G);
    if #KnownIrreducibles(G) eq #ClassesData(G) then
	return CharacterTable(G);
    end if;

    has_socle := func<G | Type(G) in {GrpPerm, GrpPC}>;

    done := false;
    if #G gt 1000 and has_socle(G) and not IsSimple(G) and Socle(G) eq G then
	D := MinimalNormalSubgroups(G);
	if &*[Order(x): x in D] eq #G then
	    P, Pf, Pfi := DirectProduct(D);
	    l, Gf := IsIsomorphic(G, P);

	    if l then
		vprint ZReps:
		    "DIR PROD CHAR TABLE:", [CompositionFactors(x): x in D];
		vtime ZReps: _ := char_table_dp(G, D, P, Pf, Pfi, Gf);
		done := true;
	    end if;
	end if;
    end if;

    if false and not done then
	P := PrimeBasis(#G);
	bestp := 0;
	bestQ := 0;
	bestQf := 0;
	for p in P do
	    Q, Qf := pCoreQuotient(G, p);
	    if Q eq G then
		continue;
	    end if;
	    if bestQ cmpeq 0 or #Q gt #bestQ then
		bestp := p;
		bestQ := Q;
		bestQf := Qf;
	    end if;
	end for;
	if bestQ cmpne 0 then
	    Q := bestQ;
	    Qf := bestQf;
	    vprintf ZReps:
		"CHAR TABLE: use pCoreQuotient(G, %o) of size %o (index %o)\n",
		bestp, #Q, #G div #Q;
	    IndentPush();
	    vprint ZReps: "Get char table of Q";
	    vtime ZReps: XQ := MyCharacterTable(Q);
	    CR := CharacterRing(G);
	    C := Classes(G);
	    vprintf ZReps: "Lift %o characters\n", #XQ;
	    vtime ZReps: Y := [CR![c(Qf(t[3])): t in C]: c in XQ];
Characters := [];
	    vprintf ZReps: "Get char table with %o + %o = %o given chars\n",
		#Y, #Characters, #Y + #Characters;
	    vtime ZReps: X := CharacterTable(G: Characters := Y cat Characters);
	    IndentPop();
	end if;
    end if;

    return CharacterTable(G: Characters := Characters);

end function;

function MyRationalCharacterTable(G)
    _:= MyCharacterTable(G);
    return RationalCharacterTable(G);
end function;
...
function MySchurIndex(c)
	...
	"c:", c;
	"c Magma:", c: Magma;
	"CT index:", Index(MyCharacterTable(G), c);
	error "FAIL";
    end try;
end function;

intrinsic MyCT(G::Grp: Characters := []) -> []
{Internal}
    return MyCharacterTable(G: Characters := Characters);
end intrinsic;
...
tuples, together with the rational character table of G}
    ...
    ***************************************************************************/
	...
	vtime ZReps: rats, ratsi := MyRationalCharacterTable(G);
...
//vprint ZReps: G: Magma;
    ...
    ***************************************************************************/
    ...
    vtime ZReps: full_char_table := MyCharacterTable(G);
	...
	WantChars := {c: c in rats | proper_deg(c) in WantCharDegrees};
...
// "want", i;
		...
		l, DBM := RepsDBGet(G, c);
		if l then
		    vprint ZReps: "GOT DB module:", DBM;
		    I[i] := DBM;
		    IC[i] := c;
	    ...
	    want := #rats;
	    R, C := RepsDBGet(G: MaxDegree := MaxDegree);

	    if #R gt 0 then
		vprint ZReps: "Got DB R:", R;
		vprint ZReps: "Got DB C:", C;
		...
		for i := 1 to #rats do
		    ...
		    //vprintf ZReps: "i: %o, cms: %o, J: %o\n", i, cms, J;
		    if #J eq 1 then
			j := J[1];
			if IsDefined(R, j) then
			    vprintf ZReps: "USE DB for i: %o, char: %o\n",
				i, c;
			    I[i] := R[j];
			    IC[i] := c;
...
// assert charz(c) eq cz;
    ...
    ***************************************************************************/
    ...
    procedure current_state(
	I, IS, other_method_done, WantChars, ~seen, ~other_method_ind
    )
	ID := [];
	other_method_ind := [];
	for i := 1 to #rats do
	    ...
	    elif true /*Degree(rats[i]) le DixonCharLimit*/ then
		if i notin other_method_done then
		    Append(~other_method_ind, i);
		end if;
	    end if;
	end for;
	vprint ZReps: "Current module dims:", ID;
	vprint ZReps: "Current Schur indices:", IS;
	vprint ZReps: "Other method try indices:", other_method_ind;
	if #WantChars gt 0 then
...
DixonEasySplitLimit := 10;
	...
	for SM in SR do
		...
		include_mod(
		    ~rats_schur_indices, ~I, ~IC, ~IS, ~Q_new, ~Qchars,
		    ~history, ~extra, ~extrac, ~extras, ~WantChars, M, si, c, hh
		);
		IndentPop();
	    end if;
	end for;
...
//"Subgroups:", L;
    ...
    ***************************************************************************/
	...
	for H in LIS do
	    ...
	    vtime ZReps: HX := MyCharacterTable(H);
	    Hrats, Hratsi := MyRationalCharacterTable(H);
		...
		if IndHRepLimit gt 0 and Degree(hc) gt IndHRepLimit then
		    continue;
		end if;
...
//"cc:", cc;
		...
		current_state(
		    I, IS, other_method_done, WantChars,
		    ~seen, ~other_method_ind
		);
	    end if;
	end for;
...
*/
    ...
    ***************************************************************************/
    ...
    HasComplexRepExt, ComplexRepExt := IsIntrinsic("ComplexRepExt");
    HasComplexRepExt, ComplexRepExtTest := IsIntrinsic("ComplexRepExtTest");
	...
	while true do
	    ...
	    wd := [Degree(rats[t[1]]): t in e];
	    mwd := Max(wd);
	    //sd := [Degree(rats[t[1]]): t in char_split];
	    msd := d;

//"mwd:", mwd;
//"msd:", msd;
WD_OTHER_DIM := 500;
WD_OTHER_SPLIT_DIM := 1000;

/*
WD_OTHER_DIM := 4;
WD_OTHER_SPLIT_DIM := 10;
*/

Hreps_MAX_SPLIT_DIM := 40;

    function get_Hreps(I, H, max_dim)
	vprintf ZReps: "Get Hreps up to split dim %o (H order %o)\n",
	    max_dim, #H;

	if assigned H`MeataxeSkipCharacterTable then
	    sct := H`MeataxeSkipCharacterTable;
	else
	    sct := false;
	end if;
	H`MeataxeSkipCharacterTable := true;

	LH := [];
	vtime ZReps: for i := 1 to #I do
	    if not IsDefined(I, i) then continue; end if;
	    M := I[i];
	    d := Dimension(M);
	    if d gt max_dim then continue; end if;

	    MH := Restriction(M, H);

	    IndentPush();
	    LH cat:= IntegralDecomposition(MH);
	    IndentPop();
	end for;
	H`MeataxeSkipCharacterTable := sct;

	vprint ZReps: "Got Hreps dims:", [Dimension(x): x in LH];

	return LH;
    end function;

	    if
1 eq 1 and UseDynExtInd and
		not Blackbox and
		mwd gt WD_OTHER_DIM and
		msd gt WD_OTHER_SPLIT_DIM and
		#other_method_ind gt 0
	    then
		i := other_method_ind[1];
		Include(~other_method_done, i);

		c := rats[i];
		cd := Degree(c);
		vprint ZReps: "[TRY OTHER METHOD FIRST]";
		...
		*/
		...
		vprintf ZReps: "Try direct induction for index %o, deg %o\n",
		    i, cd;
		vprint ZReps: "Character:", c;

		good, ComplexRepIndTest := IsIntrinsic("ComplexRepIndTest");
		if good then
		    IndentPush();
		    vtime ZReps: good, cc := ComplexRepIndTest(c);
		    IndentPop();
		end if;

		if good then
		    vprint ZReps: "FOUND DIRECT IND; cc:", cc;
		    vprint ZReps: "Get module for cc";

		    H := Group(Parent(cc));
		    vtime ZReps: Hreps := get_Hreps(I, H, Hreps_MAX_SPLIT_DIM);

		    ITIME := Cputime();
		    if HasComplexRepExt then
			"Try ext first";
			IndentPush();
			vtime ZReps: good, Mcc := ComplexRepExt(
			    cc: Recurse, UseInduction:=UseInduction
			);
			IndentPop();
		    else
			good := false;
			...
			//"Orig Mcc:"; Info(Mcc);
			vtime ZReps: 
			    Mcc := ExpandZ(Mcc: DoClean := false);
			//"Integral Mcc:"; Info(Mcc);
		    else
			"Fail ext test; use main algorithm";
			IndentPush();
			vtime ZReps:
			    Mcc := GModule(
				cc, Z: UseInduction := false,
				GivenReps := Hreps, UseLI := UseLI,
				NewLIS := NewLIS
			    );
			IndentPop();
		    end if;
		    vprintf ZReps: "Got Mcc:", Mcc;
		    M := Induction(Mcc, G);
		    ITIME := Cputime(ITIME);

		    get_si(~rats_schur_indices, i, ~si);
		    hh := <0, Dimension(M), "dindr", #H, ITIME>;
		    IndentPush();
		    include_mod(
			~rats_schur_indices, ~I, ~IC, ~IS, ~Q_new, ~Qchars,
			~history, ~extra, ~extrac, ~extras, ~WantChars,
		...
		vprintf ZReps: "Fail direct ind test";
		vprintf ZReps: "Try extension for index %o, deg %o\n",
		    i, cd;
		vprint ZReps: "Character:", c;

		if HasComplexRepExt then
		    IndentPush();
		    vtime ZReps: good, H := ComplexRepExtTest(c);
		    IndentPop();
		else
		    good := false;
		end if;
		...
		if good then
		    vprintf ZReps: "FOUND EXT; H index %o\n", Index(G, H);
		    vprintf ZReps: "Get module for cc";

		    ITIME := Cputime();
		    IndentPush();
		    vtime ZReps: 
			good, M := ComplexRepExt(c: H := H, Recurse);
		    IndentPop();
		    vprintf ZReps: "Got M:", M;
		    ITIME := Cputime(ITIME);

		    get_si(~rats_schur_indices, i, ~si);
		    hh := <0, Dimension(M), "ext", Index(G, H), ITIME>;
		    IndentPush();
		    include_mod(
			~rats_schur_indices, ~I, ~IC, ~IS, ~Q_new, ~Qchars,
			~history, ~extra, ~extrac, ~extras, ~WantChars,
...
*/
	...
	*/
	...
	vprint ZReps: "Char split indices:",
	    [<t[1], t[2]>: t in char_split];
	vprint ZReps: "Char split degrees:",
	    [<Z!Degree(rats[t[1]]), t[2]>: t in char_split];
	vprint ZReps: "Want indices:", [t[1]: t in e];
	vprint ZReps: "Want degrees:", [Degree(rats[t[1]]): t in e];
	...
	*/
	...
	if t eq PERM then
	    ...
	    */
		...
		if UseExt and HasComplexRepExt then
...
*/
		...
		include_mod(
		    ~rats_schur_indices, ~I, ~IC, ~IS, ~Q_new, ~Qchars,
		    ~history, ~extra, ~extrac, ~extras, ~WantChars, M,
		    B[i], c, hh
		);
		IndentPop();
	    else
...
function module_from_char(chi, UseInduction: GivenReps := [])
    ...
    G := Group(Parent(chi));
    C, CI := MyRationalCharacterTable(G);
    X := MyCharacterTable(G);
...
intrinsic GModule(
    chi::AlgChtrElt, Z::RngInt: UseInduction, GivenReps := []
) -> ModGrp
{An irreducible G-module over Z which affords the minimal integral
character containing chi};

    M := module_from_char(chi, UseInduction: GivenReps := GivenReps);
    require M cmpne 0:
