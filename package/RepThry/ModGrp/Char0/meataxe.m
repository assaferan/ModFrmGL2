...
BBPrime_MIN_DIM := 10;
		...
		if BBPrime gt 0 then
		    vprint ZMeataxe: "Endo:", EndomorphismRing(Mi);
		    vprint ZMeataxe: "Endo centre dim:", endo_centre_dim(Mi);
		end if;

		if
		    BBPrime gt 0 and
		    endo_centre_dim(Mi) ge BBPrime_MIN_DIM
		then
		    vprint ZMeataxe: "BBPrime case; p:", BBPrime;
		    F := GF(BBPrime);
		    Mp := ChangeRing(Mi, F);
		    vprint ZMeataxe: "Get IndecomposableSummands of:", Mp;
		    vtime ZMeataxe: I := IndecomposableSummands(Mp);
		    vprint ZMeataxe: I;
		if 1 eq 0 then
		    I2 := [I[1]];
		    for i := 2 to #I do
			m := I[i];
			if not exists{
			    j: j in [1 .. #I2] | IsIsomorphic(I2[j], m)
			}
			then
			    Append(~I2, m);
			end if;
		    ...
		    m := &+I2;
		    vprint ZMeataxe: "I2:", I2;
		    vprint ZMeataxe: "Sum module:", m;
		else
		    m := I[1];
		end if;
		    mor := Morphism(m, Mp)*Matrix(F, Morphism(Mi, M));
		    m`modular_morphism := mor;
		    m`modular_orig_M := Mi;
		    S := [m];
		    B := [1];
		end if;
