...
intrinsic SaveGroup(G::GrpMat, file::MonStgElt)
{"}

    C := Classes(G);
    X := CharacterTable(G);
    fprintf file, "G := %m;\n", G;
    fprintf file, "G`Order := %o;\n", #G;
    fprintf file, "LMGInitialize(G);\n";
    fprintf file,
	"C := SequenceToConjugacyClasses([car<Integers(), Integers(), G> |\n";
    for i := 1 to #C-1 do
	t := C[i];
	fprintf file, "%O,\n", <t[1],t[2],Matrix(t[3])>, "MagmaContext";
    end for;
    t := C[#C];
    fprintf file, "%O\n", <t[1],t[2],Matrix(t[3])>, "MagmaContext";
    fprintf file, "]);\n";
