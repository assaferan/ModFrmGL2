 ...
 */
    ...
    fl, si := HasAttribute(chi, "SchurIndices");
    if fl then
	if #si eq 0 then return 1, si; end if;
	i := LCM([t[2]: t in si]);
	return i, si;
    end if;
    Q := Rationals();

    Ghash := GroupOrder(Parent(chi));
    if DegreeBound eq -1 then DegreeBound := Ilog(100,Ghash)+1; end if;
    q, loc, cyclics, do_mult := Schur_index_bounds(chi:
	    Easy := Easy, DegreeBound := DegreeBound);

    L, U := Explode(q);
    if forall{t:t in loc|t[2] eq t[3]} then
	si := [<t[1],t[2]>:t in loc|t[2] ne 1];
	chi`SchurIndices := si;
	return U, si;
    elif U eq L and Easy then
