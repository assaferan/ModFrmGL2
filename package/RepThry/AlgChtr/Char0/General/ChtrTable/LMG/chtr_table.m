...
ProcessCharacters := procedure(~CP, C, mask, sqr_lev:LLLSort := false)
    ...
    */
	...
	if CP`CSBlocks[i]`LLLcount mod LLLfreq eq 0 then
	    ...
	    CP`CSBlocks[i]`Reds cat:= J;
	    CP`CSBlocks[i]`Gram := m;
	    CP`CSBlocks[i]`Trace := Trace(m);
	end if;
    end for;
...
vprintf Chtr: "CS = %o, %o irreducible(s) known (out of %o)\n", [b`MaxRank: b in
 CP`CSBlocks], #KnownIrreducibles(G), #cl;
	...
	for p in el_primes do
     ...
     vprintf Chtr, 2: "Calling LMGSylow on centralizer, p =", p, i, c[1];
     if GetVerbose("Chtr") gt 1 then C:Minimal; end if;
     Syl_timer := -Cputime();
	    S := LMGSylow(C,p);
     Syl_timer +:= Cputime();
     vprintf Chtr, 2: "LMGSylow in %o secs\n", Syl_timer;
...
{Character Table of finite group G}
    flag := GrpMatUseLMG(G);
    SetGrpMatLMG(G, true);
    T := ChtrTableWrapper(G:Al:=Al,ClassSizeLimit:=0, Check:=Check);
    SetGrpMatLMG(G, flag);
    return T;
end intrinsic;
