...
procedure compute_canonical_forms(~data, k)
	...
	t := Cputime();
	data`canon[k] := {@ zmat_from_mat_sort(HadamardCanonicalForm(m : Al := "nauty")) :
						    m in data`stored[k] @};
...
canonical form already}
	...
	if Canonical then
	    ...
	    m1 := HadamardCanonicalForm(m : Al := "nauty");
	    x_stored, x_canon := Explode(zmats_from_mat(m1));
	else
	    x_stored := zmat_from_mat(m);
	    x_canon  := zmat_from_mat_sort(HadamardCanonicalForm(m : Al := "nauty"));
	end if;
