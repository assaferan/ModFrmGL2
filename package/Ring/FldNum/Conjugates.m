...
intrinsic Conjugate
(a::FldAlgElt[FldRat], s::[RngIntElt]: Prec := GetKantPrecision() ) -> FldElt
{Given an element 'a' in a number field K and a sequence s containing 1
 positive integer, this returns the image of 'a' under the complex embedding 
 determined by that integer}

  require #s eq 1 :
  "The second argument should be a sequence containing 1 integer";

   if Type(a) eq FldCycElt then
    // necess spec case, as Conjugate(FldCycElt,RngIntElt) means something else
