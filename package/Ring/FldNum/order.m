...
/* MW: no idea why not using "NEW" code here... ?
if false then // NEW
...
printf "PowerProductSimplify (denom): "; time
  ...
  // h := [Universe(g)| ];
  //  for i := 2 to #g do bool, hi := IsPower(g[i], n);
  //    assert bool; Append(~h, hi); end for;
...
// MW, Aug 2018:
//  RankBounds(EllipticCurve(x^3 - 4668852241*x))
// gives a non-integral alpha
...
{Computes the (p, S)-Selmer group of the field containing the prime ideals in S}
  ...
  require forall{x : x in S | IsPrime(x) and not IsZero(x) } :
