...
function RayResidueRingN(I,oo)  R,m:=RayResidueRing(I,oo);
 if Order(R) gt 1 and Order(R.1) eq 1 then // solve some Magma nuttiness...
...
declare attributes GrpDrchNF:
 ...
 ResidueMap,ooReal,Labels,ambient,issubgroup,inclmap;
...
intrinsic SubConstr
 ...
 S`ambient:=G`ambient; S`issubgroup:=true;
 S`AbGrp,mp:=sub<G`ambient`AbGrp|
                [G`ambient`AbGrp!Eltseq(G`ambient!x) : x in E]>;
...
intrinsic '+'(G::GrpDrchNF,H::GrpDrchNF) -> GrpDrchNF {Add}
 ...
 GS:=G`ambient`AbGrp; HS:=H`ambient`AbGrp; T:=H`ambient;
...
intrinsic 'eq'(x::GrpDrchNFElt,y::GrpDrchNFElt) -> BoolElt {eq}
 if x`Parent`ambient ne y`Parent`ambient then return false; end if;
 return Eltseq((x`Parent`ambient)!x) eq Eltseq((y`Parent`ambient)!y);
 end intrinsic;
...
intrinsic IsCoercible(G::GrpDrchNF,e::.) -> BoolElt,GrpDrchNFElt {Coerce}
 if Type(e) eq GrpDrchNFElt then P:=e`Parent;
  ...
  if G eq P then return true,G!Eltseq(e); end if; // same intern rep
  if P`issubgroup then
   return IsCoercible(G,P`ambient!Eltseq(P`ambient`AbGrp!e`Element)); end if;
  if G`ambient eq e`Parent then b,chi:=IsCoercible(G`AbGrp,e`Element);
...
intrinsic '^'(e::GrpDrchNFElt,i::RngIntElt) -> GrpDrchNFElt {Power}
 ...
 x`Element:=i*e`Element; return x; end intrinsic;
...
intrinsic '*'(x::GrpDrchNFElt,y::GrpDrchNFElt) -> GrpDrchNFElt {Multiply}
 ...
 if I eq J and A`ooReal eq B`ooReal then // could have I,J 'eq' but not TES
  ...
  if not A`issubgroup and not B`issubgroup then
   z:=New(GrpDrchNFElt); z`Parent:=x`Parent; G:=A`AbGrp;
...
intrinsic '#'(G::GrpDrchNF) -> RngInt {The order of G.}
 return #G`AbGrp; end intrinsic;
...
intrinsic Eltseq(chi::GrpDrchNFElt) -> SeqEnum {For internal use.} 
 return Eltseq(chi`Element); end intrinsic;
...
intrinsic IsTrivialOnUnits(chi::GrpDrchNFElt) -> BoolElt
{Returns whether a Dirichlet character is trivial on the units}
 G:=chi`Parent; U,mp:=UnitGroup(G`NF);
 return &and[0 eq Z!'@'(mp(u),chi : Raw) : u in Generators(U)]; end intrinsic;
...
intrinsic IsIdentity(chi::GrpDrchNFElt) -> BoolElt
{True iff chi is the trivial character}
 return &and[x eq 0: x in Eltseq(chi)]; end intrinsic;

intrinsic IsTrivial(chi::GrpDrchNFElt) -> BoolElt {"}//"
 return &and[x eq 0: x in Eltseq(chi)]; end intrinsic;

intrinsic IsOdd(chi::GrpDrchNFElt) -> BoolElt
{True iff the Dirichlet character chi satisfies chi(-1)=-1}
 return chi(-1) eq -1; end intrinsic;

intrinsic IsEven(chi::GrpDrchNFElt) -> BoolElt
{True iff the Dirichlet character chi satisfies chi(-1)=+1}
 return chi(-1) eq 1; end intrinsic;
...
intrinsic Conductor(chi::GrpDrchNFElt) -> RngOrdIdl, SeqEnum
...
function Ioo_extend(chi,H) rj:=H`ResidueMap; RJ:=Domain(rj);
 ...
 D:=chi`Parent; ri:=D`ResidueMap; RI:=Domain(ri); HS:=H`ambient;
...
{Extend the given Dirichlet character to the given Dirichlet group}
 ...
 if H`issubgroup then chi:=H`ambient!chi; end if;
...
{Decomposition of a given Dirichlet character as a
 ...
 if G`issubgroup then CHI:=G`ambient!chi; else CHI:=chi; end if;
...
declare attributes FldNum: rrr_cache;

procedure rrr_ideal_catalogue(n,D,K) // abiilty to use tilde in procedure
 if n in Keys(K`rrr_cache) then Append(~K`rrr_cache[n],D);
 else K`rrr_cache[n]:=[* D *]; end if; end procedure;

function rrr_with_cache(I,oo) K:=NumberField(Order(I)); n:=Norm(I);
 KEYS:=Keys(K`rrr_cache); DATA:=(n in KEYS) select K`rrr_cache[n] else [];
 for data in DATA do if oo ne data[2] or I ne data[1] then continue; end if;
  return data[3],data[4],data[5],data[6]; end for;
 RRR,mp:=RayResidueRingN(I,oo); A,p:=Dual(RRR:UseSameGroup);
 rrr_ideal_catalogue(n,<I,oo,RRR,mp,A,p>,K); return RRR,mp,A,p; end function;
// problem is that I and J can be 'eq' with RRR's in different presentations
...
{Returns the dual character group for (O/Ioo)^*}
 ...
 G:=New(GrpDrchNF); G`ambient:=G; G`issubgroup:=false; MO:=MaximalOrder(K);
 _:=ClassGroup(MO); _:=IndependentUnits(MO); SetOrderUnitsAreFundamental(MO);
 G`Modulus:=I; G`ooReal:=oo; G`NF:=NumberField(Order(I));
 if not assigned K`rrr_cache then K`rrr_cache:=AssociativeArray(Z); end if;
  RRR,G`ResidueMap,G`AbGrp,G`Pairing:=rrr_with_cache(I,oo);
 //  R,G`ResidueMap:=RayResidueRingN(I,oo);
 //  G`AbGrp,G`Pairing:=Dual(R : UseSameGroup);
 m:=Modulus(Codomain(G`Pairing));
...
intrinsic '@'(e::FldNumElt,chi::GrpDrchNFElt : Raw:=false) -> RngElt {"} //"
 ...
 if G`issubgroup then return '@'(e,G`ambient!chi : Raw:=Raw); end if;
 ...
 u:=G`Pairing((G`NF!e) @@ G`ResidueMap,chi`Element);
 if Raw then return u; end if;
 m:=Modulus(Parent(u)); r:=Integers()!u; g:=Gcd(m,r);
 if assigned chi`Zeta then ans:=chi`Zeta^((r*chi`ZetaOrder) div m);
...
intrinsic DirichletCharacter(DG::GrpDrchNF,B::Tup : RequireGenerators:=true)
 ...
 if DG`issubgroup then RG:=RequireGenerators;
  r,K:=DirichletCharacter(DG`ambient,B : RequireGenerators:=RG);
