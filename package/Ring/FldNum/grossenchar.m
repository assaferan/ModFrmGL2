...
function cyclo_comp(raw,P) C:=ComplexField(P); i:=C.1;
...
procedure grossenideal_catalogue(GI,K) n:=Norm(GI[1]); // universe could differ
...
{Given a Hecke character along with a compatible infinity-type,
 ...
 if psi`Parent`issubgroup then
  return Grossencharacter(psi`Parent`ambient!psi,T); end if;
...
{Given a Hecke character along with a compatible infinity-type and
 ...
 if psi`Parent`issubgroup then
  return Grossencharacter(psi`Parent`ambient!psi,chi,T); end if;
 if chi`Parent`issubgroup then
  return Grossencharacter(psi,chi`Parent`ambient!chi,T); end if;
 K:=psi`Parent`NF; I:=psi`Parent`Modulus; oo:=psi`Parent`ooReal;
 ...
 require #Set([&+t : t in T]) eq 1: "All oo-type parts must have same weight";
 b,S:=check_1modI_is_OK(I,T); require b: S;
 b,S:=check_chi_is_OK(chi,T); require b: S; ensure_field(K);
 return make_grossencharacter(chi,psi,I,&+T[1],T); end intrinsic;
...
{Given a Grossencharacter and a prime ideal, return the local root number}
 ...
 "Grossencharacter and P must be defined over same number field";
 if not gr`field_is_cm then
  return root_no(AssociatedPrimitiveCharacter(gr`hecke),P :
		 Precision:=Precision); end if;
...
{Given a Grossencharacter and a place, return the local root number}
 ...
 TYPE:=Sort(gr`type[w]); ans:=TYPE[2]-TYPE[1]; // q-p, but need to recip...
...
(gr::GrossenChar : Precision:=GetPrecision(0.5), AA:=false) -> SeqEnum
{Given a Grossencharacter, return the root numbers at bad places}
 ...
 if AA then U:=AssociativeArray(); for x in A do U[x[1]]:=x[2]; end for;
  return U; end if; return A; end intrinsic;

intrinsic RootNumbers
(psi::GrpHeckeElt : Precision:=GetPrecision(0.5), AA:=false) -> SeqEnum
...
{Given a Grossencharacter, find its twist by a Hecke character
 ...
 H:=Parent(psi`hecke); if H`issubgroup then H:=H`ambient; end if;
...
{Given a Grossencharacter, return its Tate twist by z}
 ...
 if not GR`field_is_cm then return TateTwist(GR`hecke,-GR`wt+z); end if;
 return Grossencharacter(GR`hecke,GR`dirich,[[u[1]-z,u[2]-z] : u in GR`type]);
 end intrinsic;
...
{Given a Hecke character, return its Tate twist by z as a Grossencharacter}
 /* if z eq 0 then return psi; end if; */ // very delibrate, must change type!
