...
intrinsic Generators(M::ModDed : Minimal:= false) -> []
	...
	for i in [1..n] do
	    ...
	    assert i eq n or ok;
	    if ok then 
		Append(~Gens, (F ! x) * ChangeRing(p[2], F)); 
	    else 
		Gens cat:= [ (F ! x) * ChangeRing(p[2], F) : x in Generators(p[1]) ]; 
	    end if;
	end for;
    else
	P:= PseudoBasis(M);
	Gens:= [ (F ! x) * ChangeRing(p[2], F) : x in Generators(p[1]), p in P ];
    end if;
    return Gens;
end intrinsic;
...
intrinsic AbsoluteBasis(M::ModDed) -> []
{An absolute basis for the module M}
  P:= PseudoBasis(M);
