...
declare attributes GrpHecke:
 ...
 rcgmap,ooReal,Labels,ambient,issubgroup,inclmap;

declare attributes GrpHeckeElt: Parent,Element,cond,dr,ooC,Zeta,ZetaOrder;
// Element is now in AbGrp of Parent, not an Eltseq
...
{Print Hecke character}
 G:=psi`Parent; S:=Eltseq(psi`Element);
...
intrinsic Print(psi::GrpHeckeElt,level::MonStgElt) {Print Hecke character}
 name:="$"; G:=psi`Parent; S:=Eltseq(psi`Element);
 if not assigned(G`Name) then NAME:="$"; else NAME:=G`Name; end if;
 I:=[i : i in [1..#S] | S[i] ne 0];
 if #I eq 0 then printf "1";
 elif assigned G`Labels then
...
intrinsic 'in'(e::.,G::GrpHecke) -> BoolElt {in}
 ...
 if e`Parent`ambient ne G`ambient then return false; end if;
 return G`ambient`AbGrp!e`Element in G`AbGrp; end intrinsic;
...
intrinsic SubConstr
 ...
 S:=New(GrpHecke); S`NF:=G`NF; S`Modulus:=G`Modulus; S`ooReal:=G`ooReal;
 S`rcgmap:=G`rcgmap; S`ambient:=G`ambient; A:=G`ambient`AbGrp;
 S`AbGrp,mp:=sub<A|[A!Eltseq(G`ambient!x) : x in E]>; S`issubgroup:=true;
 if #S`AbGrp eq #G`AbGrp then // simplify, try to fix Z/2 x Z/3 vs Z/6 prob
 ...
 S`inclmap:=map<S->G|x:->G!Eltseq(G`AbGrp!mp(x`Element))>;
...
function is_compatible(G,H)
 return G`NF eq H`NF and G`Modulus eq H`Modulus and G`ooReal eq H`ooReal;
 end function;

intrinsic '+'(G::GrpHecke,H::GrpHecke) -> GrpHecke {Add}
 require is_compatible(G,H): "Groups must have same modulus to add them";
 GS:=G`ambient`AbGrp; HS:=H`ambient`AbGrp;
 assert IsIdentical(GS,HS); T:=H`ambient;
...
intrinsic 'meet'(G::GrpHecke,H::GrpHecke) -> GrpHecke {meet}
 require is_compatible(G,H): "Groups must have same modulus to intersect them";
 GS:=G`ambient`AbGrp; HS:=H`ambient`AbGrp;
 assert IsIdentical(GS,HS); T:=H`ambient;
 if #GS eq 1 then return sub<G|[]>; end if;
 if #HS eq 1 then return sub<H|[]>; end if;
 S:=sub<T|[T!Eltseq(HS!g) : g in Generators(G`AbGrp meet H`AbGrp)]>;
 return S; end intrinsic;

intrinsic 'eq'(G::GrpHecke,H::GrpHecke) -> BoolElt {eq}
 if not is_compatible(G,H) then return false; end if;
 assert IsIdentical(G`ambient`AbGrp,H`ambient`AbGrp);
 return G`AbGrp eq H`AbGrp; end intrinsic;

intrinsic 'subset'(G::GrpHecke,H::GrpHecke) -> BoolElt {subset}
 if not is_compatible(G,H) then return false; end if;
 assert IsIdentical(G`ambient`AbGrp,H`ambient`AbGrp);
 return G`AbGrp subset H`AbGrp; end intrinsic;

intrinsic 'eq'(x::GrpHeckeElt,y::GrpHeckeElt) -> BoolElt {eq}
 A:=x`Parent`ambient; B:=y`Parent`ambient; if A ne B then return false; end if;
 return Eltseq(A!x) eq Eltseq(B!y); end intrinsic;

intrinsic IsCoercible(G::GrpHecke,e::.) -> BoolElt, GrpHeckeElt {Coerce}
 if Type(e) eq GrpHeckeElt then P:=e`Parent;
  if IsTrivial(e) then return true,G.0; end if;
  if G eq P then return true,G!Eltseq(e); end if; // same internal rep
  if P`issubgroup then
   return IsCoercible(G,P`ambient!Eltseq(P`ambient`AbGrp!e`Element)); end if;
  if G`ambient eq P then b,chi:=IsCoercible(G`AbGrp,e`Element);
 ...
 if Type(e) eq SeqEnum then x:=New(GrpHeckeElt);
  x`Parent:=G; x`Element:=G`AbGrp!e; return true,x; end if;
...
intrinsic '*'(x::GrpHeckeElt,y::GrpHeckeElt) -> GrpHeckeElt {"} //"
 ...
 if I eq J and A`ooReal eq B`ooReal then // could have I,J 'eq' but not TES
  assert IsIdentical(A`ambient`AbGrp,B`ambient`AbGrp);
  if not A`issubgroup and not B`issubgroup then
   z:=New(GrpHeckeElt); z`Parent:=A; G:=A`AbGrp;
   z`Element:=x`Element+y`Element; return z;
  else S:=A`ambient; z:=(S!x)*(S!y); return (A+B)!z; end if;
...
intrinsic Eltseq(psi::GrpHeckeElt) -> SeqEnum {Eltseq}
 return Eltseq(psi`Element); end intrinsic;
...
declare attributes FldNum: rcg_cache;

procedure rcg_ideal_catalogue(n,D,K) // abiilty to use tilde in procedure
 if n in Keys(K`rcg_cache) then Append(~K`rcg_cache[n],D);
 else K`rcg_cache[n]:=[* D *]; end if; end procedure;

function rcg_with_cache(I,oo) K:=NumberField(Order(I)); n:=Norm(I);
 KEYS:=Keys(K`rcg_cache); DATA:=(n in KEYS) select K`rcg_cache[n] else [];
 for data in DATA do if oo ne data[2] or I ne data[1] then continue; end if;
  return data[3],data[4],data[5],data[6]; end for;
 RCG,mp:=RayClassGroup(I,oo); A,p:=Dual(RCG:UseSameGroup);
 rcg_ideal_catalogue(n,<I,oo,RCG,mp,A,p>,K); return RCG,mp,A,p; end function;
// problem is that I and J can be 'eq' with RCG's in different presentations

intrinsic HeckeCharacterGroup
(I::RngOrdIdl,oo::SeqEnum[RngIntElt] : Target:=0) -> GrpHecke
{Character group on ideals (rather than K-elements),
 ...
 H`ambient:=H; H`issubgroup:=false; MO:=MaximalOrder(H`NF);
 _:=ClassGroup(MO); _:=IndependentUnits(MO); SetOrderUnitsAreFundamental(MO);
 if not assigned K`rcg_cache then K`rcg_cache:=AssociativeArray(Z); end if;
 RCG,H`rcgmap,H`AbGrp,H`Pairing:=rcg_with_cache(I,oo);
 // RCG,H`rcgmap:=RayClassGroup(I,oo);
 // H`AbGrp,H`Pairing:=Dual(RCG : UseSameGroup);
 m:=Modulus(Codomain(H`Pairing)); H`CycloOrder:=m;
...
intrinsic '@'(J::RngOrdFracIdl,psi::GrpHeckeElt : Raw:=false) -> RngElt {"}//"
 ...
 if H`issubgroup then return '@'(J,H`ambient!psi : Raw:=Raw); end if;
 ...
 u:=H`Pairing(J@@H`rcgmap,psi`Element); if Raw then return u; end if;
 m:=Modulus(Parent(u)); r:=Integers()!u; g:=Gcd(m,r);
 if assigned psi`Zeta then ans:=psi`Zeta^((r*psi`ZetaOrder) div m);
 else ans:=CyclotomicField(m div g).1^(r div g); end if;
 if Type(ans) eq FldCycElt and (ans eq 1 or ans eq -1) then ans:=Z!ans; end if;
 return ans; end intrinsic; // users want 1 and -1 in Z it seems...
...
{Given a Hecke character and a root of unity in a ring,
 ...
 c:=Order(psi); require o mod c eq 0: "Order(psi) must divide Order(ring elt)";
...
{Extend a Hecke character to the given Hecke group}
 ...
 I:=H`Modulus; J:=P`Modulus; HS:=H`ambient;
 require I subset J:
...
intrinsic HeckeCharacter(HG::GrpHecke,B::Tup : RequireGenerators:=true)
 ...
 if HG`issubgroup then
  r,K:=HeckeCharacter(HG`ambient,B : RequireGenerators:=RequireGenerators);
...
{For a nonzero field element, return the associated quadratic Hecke character}
