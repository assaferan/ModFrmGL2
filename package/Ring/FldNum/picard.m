...
The map has an inverse map, sending an ideal to its ideal class.}
  ...
  F := Conductor(o);  
  if Type(F) eq RngIntElt then F := ideal<Integers()|F>; end if;
  om := MaximalOrder(o);
  Fm := om!!F;
  C, mC := cg(om);
  ...
  if Order(C) gt 1 then
    ...
    for i in [1..#Generators(C)] do
     ...
     cocoeff := CoprimeRepresentative(mC(C.i),Minimum(Fm)*om);
     //JK: to be sure avoid all prime numbers in Fm
...
/*
for id in generators_in_o do
 oid := MultiplicatorRing(id);
 assert o eq oid;
end for;
*/ 
