...
intrinsic EulerGamma() -> FldReElt
{Create Euler's constant gamma in the default real field}
    return EulerGamma(RealField());
end intrinsic;

intrinsic EulerGamma(k::RngIntElt) -> FldReElt
{Create Euler's constant gamma in the real field of precision k}
    require k gt 0: "Argument must be positive";
    return EulerGamma(RealField(k));
end intrinsic;

intrinsic LinearRelation(Q::SeqEnum[FldReElt] : Al:="LLL") -> SeqEnum
{This has been deprecated, use IntegerRelation instead.}
 require #Q ge 2: "Sequence must have at least 2 elements";
 C:=ComplexField(Precision(Universe(Q)));
 return LinearRelation(ChangeUniverse(Q,C)); end intrinsic;

intrinsic LinearRelation(Q::ModTupFldElt[FldRe] : Al:="LLL") -> SeqEnum
{This has been deprecated, use IntegerRelation instead.}
 require Degree(Q) ge 2: "Vector must be degree at least 2";
 C:=ComplexField(Precision(BaseRing(Q)));
 return LinearRelation(ChangeRing(Q,C)); end intrinsic;
