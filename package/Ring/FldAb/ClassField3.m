...
function maximal_abel(F, g, do_abel)
  ...
  if true then // CF: we may skip this...  but it gives smaller values...
               // MW: actually, we can't just work with FF
    FF := MaximalAbelianSubfield(Kr);
    vprint ClassField, 2: "MaximalAbelianSubfield is ", FF;
    c, c_inf := Conductor(FF);
    c := Parent(c)!Discriminant(MaximalOrder(Kr));
  // MW fix, need bad primes of Disc, but in QNF, not Q, geh
  else
    c := Discriminant(MaximalOrder(Kr)); // MW: former code, not over Q
  end if;
  m := MaximalOrder(k);
  ...
  map_mgC_rcg:=InducedMap
                (mgC,rcg`Map,map<Parent(1*M)->Parent(1*M)| x :->x>,Minimum(c));
  ker := Kernel(map_mgC_rcg);
  if IsAbsoluteField(Kr) then
...
{Computes the genus field, i.e.
 the maximal abelian extension of the BaseField of F that is inside F}

  k := BaseField(F);
  g := check_All_Over(k, All, Over);
  if #g eq 0 then return MaximalAbelianSubfield(k); end if; // MW, was "true"
