...
{the completion at P up to a precision of Prec together with the embedding}
  ...
  require (RamificationDegree(P) eq 1) or IsMaximal(O) : "For ramified primes the order must be maximal";
...
{Completion of K at P}
  ...
  O := Order(P);
  require (RamificationDegree(P) eq 1) or IsMaximal(O) :
...
{Completion of K at P}
  ...
  require IsFinite(P) : "The place must be a finite place of the field";
  P := Ideal(P);
  O := Order(P);
  require NumberField(O) eq _NumberField(K): 
