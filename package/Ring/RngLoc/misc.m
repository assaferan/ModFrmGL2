...
function padic_gamma(x) assert Valuation(x) ge 0; K:=Parent(x);
 if x eq 0 then return K!1; end if;
 if Valuation(x) eq Infinity() then return K!1; end if;
 p:=Prime(K); v:=AbsolutePrecision(x); // v should be finite
 o:=v; if p eq 2 and o eq 2 then o:=1; end if; O:=pAdicField(p,o);
 if p eq 2 then v:=v+1; end if;
 L:=pAdicField(p,v); x:=ChangePrecision(L!x,v); // assumes x is nonzero
...
// if AbsolutePrecision of x is Infinity(), then use ring precision?
...
{Compute the p-adic Gamma function}
 ...
 if prec eq Infinity() then prec:=Precision(Parent(x)); end if;
 K:=pAdicField(Prime(Parent(x)),prec);
...
{Compute the p-adic Gamma function}
 ...
 prec:=AbsolutePrecision(x);
 if prec eq Infinity() then prec:=Precision(Parent(x)); end if;
 K:=pAdicField(Prime(Parent(x)),prec); // prec could still be Infinity()
