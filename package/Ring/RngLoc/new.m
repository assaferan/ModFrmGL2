...
//"vstar",[Valuation(coeffs[i]) / i : i in [1..#coeffs]];
    vals := [Valuation(coeffs[i]) / i : i in [1..#coeffs] | not IsZero(coeffs[i])];
    if #vals eq 0 then
	vals := [Valuation(coeffs[i]) / i : i in [1..#coeffs]];
    end if;
    res := Min(Min(vals), Precision(Universe(coeffs)));
//"vstar",res;
...
elt_invariants:= procedure(Phi, traces_Phi, ~phi)
  ...
  phi`passes_newton_test := // MW May 2017: changed to 'ge' from 'eq' here ?
    phi`v_star ge Valuation(TrailingCoefficient(phi`chi)) / Degree(phi`chi);
...
// computed
    ...
    while true do
        ...
        vprint RoundFour,1:"R4| deg(phi) =", Degree(phi`elt),"and v*(phi) =",
                            phi`v_star;
        vprint RoundFour,2:"R4| E =",E,"and F =",F; 
        /* Step a *********************************************************/
        if not phi`v_star gt phi_old_v_star then
          error "Factorization: Insufficient prec in Round Four main loop";
        end if;
        if not Degree(phi`elt) le E*F then
          error "Factorization: Degree(phi) too large in Round Four main loop";
        end if;
	...
	/* Step b [increase E] *******************************************/
	    ...
	    E := s * E;
            ...
            if E*F eq Degree(Phihat) then // Phi is irreducible
              vprint RoundFour,1:
                     "R4b E * F =",E,"*",F,"= deg(Phi) =",Degree(Phihat),
                                 "  =>   Phi is irreducible";    
              if with_certificates then
                ...
                if Pi`v_star ne 1/E then
                  error "Factorization: Insufficient precision "*
                        " in computation of ramified certificate";
                end if;
                ...
                if Gamma`f ne F then
                  error "Factorization: Insufficient precision "*
                        "in computation of unramified certificate";
                end if;
                return [rec<FACTOR| factor := Phi, multiplicity := 1, 
                                   F:=F, Gamma := Gamma`elt, 
	...
	/* Step c **********************************************************/
        ...
        gamma := elt((phi`elt * poly_inv`elt) mod Phihat,
		     poly_inv`denval-poly`denval);
	...
	/* Step f ************************************************************/
        ...
        if F mod gamma`f ne 0 then // increase F
          ...
          if E * LCM(F, gamma`f) eq Degree(Phihat) then // Phi is irreducible
             F := LCM(F, gamma`f);         
             vprint RoundFour,1:"R4f E * lcm(F,F_gamma) =",E,"*",F,
	                        "= deg(Phi) =",Degree(Phihat),
                                "  =>   Phi is irreducible";    
             if with_certificates then
	  ...
	  /* Step g *********************************************************/
	    ...
	    OF:=F; F := LCM(F, gamma`f);         
	    vprint RoundFour,1:"R4 increasing F from",OF,"to", F,
	       "and resetting E to 1";
            beta := kappa;
            //beta`elt := beta`elt+pi^(beta`denval+1)*PR.1;
            vprint RoundFour,5:"R4g searching beta with F_beta = F =",F,
                               "and chi_beta irred w/ deg(Phi) =",Degree(Phi); 
            newPhi := elt_chi(Phihat, traces_Phihat, beta);
            //while not is_squarefree(newPhi) do

            if Discriminant(Phihat) ne 0 then
              // we can use the disc for testing whether newPhi is squarefree
	      // MW (2016 Sep1): umm no we can't - prec prob can still exist!
	      //
	      // MW (Mar 2018): hmm, Gonzalo Tornaria sends an example where
	      // the Discriminant is 0, but it's "squarefree" ??!
	      // ergo, include both conditions...
	      //
	      // while Discriminant(newPhi) eq 0 do
 	      while Discriminant(newPhi) eq 0 or not is_squarefree(newPhi) do
              ...
              // better also use the gcd for testing whether newPhi is sqfree
	      // as we have a precision problem with the discriminant already
              while not is_squarefree(newPhi) do
                vprint RoundFour,5:"R4g beta := beta + pi*x";
                beta`elt := beta`elt+pi^(beta`denval+1)*PR.1;
                newPhi := elt_chi(Phihat, traces_Phihat, beta);
          ...
          // no increase of F // continue in main loop
        /* Step h ********************************************************/
          vprint RoundFour,5:
