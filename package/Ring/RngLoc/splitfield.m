...
//! does not construct embeddings
  ...
  k<z>:=ResidueClassField(Integers(B));
  kP:=PrimeField(k);
  deg:=Degree(k,kP);
  repeat
    resext:=PR(Z)!MinimalPolynomial(z,kP);
    z:=Random(k);
  until Degree(resext) eq deg;
...
procedure SplitStep(~F: tame:=false)
  ...
  repeat
    ...
    if tame then
      d:=Denominator
       (VectorContent([v[1] eq Infinity() select 1 else v[1]: v in val]));
...
procedure pAdicSplit(~F,~r: tame:=false)
  repeat
    ...
    vprintf ArtRep,2: "pAdicSplit prec=%o (d=%o e=%o f=%o) %o\n",
      ...
      Type(F) eq RngUPolElt
       select Degree(F) else DelSpaces({* Degree(d[1]): d in F *});
...
function ComputeSplittingField
(f,p: prec:=20, tame:=false, std:=true, Gal:=false, squarefree:=false)
  ...
  if Type(B) in [RngInt,FldRat] 
    then vroots:=[v[1]: v in ValuationsOfRoots(f,p) | v[1] ne Infinity()];
         pi:=p;
         scaleval:=#vroots eq 0 select 0 else Floor(Min(vroots));
    else vroots:=[v[1]: v in ValuationsOfRoots(f)   | v[1] ne Infinity()];
         pi:=UniformizingElement(B);
         prec:=Precision(B);
         scaleval:=#vroots eq 0 select 0 else Min(0,Floor(Min(vroots)));
  end if;
