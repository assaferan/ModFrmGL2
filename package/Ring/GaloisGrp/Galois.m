...
function _GetGaloisCacheLevel() I:=ideal<_Z|_Cache>; // more paranoia
 return assigned I`CacheLevel select I`CacheLevel else 0; end function;
...
function UseCache() 
  Cache := ideal<_Z|_Cache>;
  return assigned Cache`UseCache and Cache`UseCache;
// MW, July 2018, paranoia, but it seems to have been unassigned?
end function;

_SetGaloisCacheLevel(0);

// Actually, I think these were being removed with "clear" ?
...
function prec_sufficient(prec_stored, prec_required)
 ...
 if Type(prec_stored) ne Type(prec_required) then
    assert Type(prec_stored) eq Tup;
    prec_required := <prec_required, prec_required>;
 end if;   
...
// We may have a special function for computing roots:
  if not assigned S`f or assigned S`GetRoots then
    ...
    r := S`Roots[n];
    assert2 CheckRoot(S, r, Prec);
    assert Type(Prec) eq RngIntElt and Precision(r) ge Prec or (Type(Prec) eq Tup and Precision(r)[1] ge Prec[1] and Precision(r)[2] ge Prec[2]);
    if Scaled and assigned S`Scale and not IsOne(S`Scale) then
      if assigned S`BaseMap then
        r *:= S`BaseMap(S`Scale, Prec);
      else
        r *:= S`Scale;
...
// in case a linear transformation is proven to be useless, we force degree >= 2.
    ...
    if Type(all_tschirni) eq SetEnum or Characteristic(R) gt 0 then
	...
	assert Type(all_tschirni) eq SeqEnum;
	if #all_tschirni eq 0 then
	    tschirni := PolynomialRing(R).1^(degree_bound div 2) + 1;
	else 
	    tschirni := all_tschirni[#all_tschirni];
	    for i in [0 /*#all_tschirni mod Degree(tschirni)*/ .. Degree(tschirni) + 1] do
		assert Coefficient(tschirni, i) le coeff_bound;
		if Coefficient(tschirni, i) eq coeff_bound then
		    tschirni -:= coeff_bound*Parent(tschirni).1^i;
		else
		    tschirni +:= Parent(tschirni).1^i;
		    if i gt 0 then
			tschirni +:= 1;
		    end if;
		    break;
	...
	assert tschirni notin all_tschirni;
    end if;
    return tschirni;
end function;
...
/* In case of lack of implementation just return the trivial element. */
  ...
  prec := 2;
  while #Set(p) ne #p do
// We need more precision to detect the Frobenius 
   r := [ GaloisRoot(i, S: Prec := prec) : i in [1..n]];
   rt := [ FrobeniusImage(x) : x in r];
   p := [ Position(rt, x) : x in r];
   prec := 2 * prec;
  end while;
  assert not 0 in p;
  vprintf GaloisGroup, 2: "Generator of used local group:\n%o\n", Sym(#p)!p,"Maximal";
  vprintf GaloisGroup, 2: "Found with precision %o\n",prec div 2;
 else
...
//CF        if #cos eq 2 then return i, tschirni, cos; end if;
		...
		if #all_tschirni ge 3*Degree(G) then
		  ...
		  error "way too many transforms in shortcut!";
            ...
            if #cos eq 1 and assigned additional then
...
// L_embed_pol is an integral polynomial that gives an primitive element of L when evaluated at a SCALED GaloisRoot
   ...
   repeat
    ...
    if exists{x : x in SS`Roots | WeakValuation(Evaluate(ff, x)) lt 0} then
     ...
     elif SS`Type eq "Complex" then
       need_more_prec := exists(x){ x : x in SS`Roots | 
       WeakValuation(Evaluate(ff, x)) le 2*WeakValuation(Evaluate(ffs, x))};
     else
       need_more_prec := exists(x){ x : x in SS`Roots | 
       Valuation(Evaluate(ff, x)) le 2*WeakValuation(Evaluate(ffs, x))};
     end if;
...
// "No good polynomial found";
 ...
 vprint GaloisGroup,3:"Resolvent polynomial",pol,"found. Building GaloisData...";
 SS:=InternalGaloisDataCreate(S`Prime);
 for i in GetAttributes(MakeType("GaloisData")) do
 ...
 SS`CycleTypes := Set(sl);
 SS`DescentChain := [];
 if assigned SS`CycleTypes then 
...
//returns true is starting group is proven, false otherwise
  ...
  if assigned S`CycleTypes then
    dogo := InternalDivisorOfGroupOrder(S`CycleTypes);
  else
    dogo := Degree(K);
  end if;
...
/* Get intersection of WreathProducts without deep subfield inspection: */
  ...
  if Order(G) eq dogo then
       delete S`Subfields;
       vprint GaloisGroup,1:"Starting group reached lower bound of order",Order(G);
       return G, [], [], proven;
  end if;
...
// It would be better to get and test the shapes before we build up all the GaloisData.
        ...
        if assigned SS`CycleTypes then 
          dogo_sf := InternalDivisorOfGroupOrder(SS`CycleTypes);
        else
          dogo_sf := Degree(G_start);
        end if;
 ...
 // ToDo: Find a better sieve at this point
        ...
        H := GenericStauduhar(G_start, SS, Type(max_comp) eq RngIntElt select max_comp else 1, 
                              func<X|[ x`subgroup : x in MaximalSubgroups(X:IsTransitive)]>, 
                              func<X| (Order(X) mod dogo_sf eq 0) and
...
intrinsic EnterStauduhar(G::GrpPerm,S::GaloisData:Subfields := false,Transitive := false, Proven := false, DivisorOfOrder := 1) -> GrpPerm, [], GaloisData
{Enter Galois Group algorithm. Transitive: Result is known to be transitive. Subfields: All subfields are encoded in G.
 ...
 if assigned S`CycleTypes then 
   return GenericStauduhar(G, S, Type(max_comp) eq RngIntElt select max_comp else 1, 
          ...
          ShortOK := false, Subfields := Subfields,  DivisorOfOrder := DivisorOfOrder); 
 else
   return GenericStauduhar(G, S, Type(max_comp) eq RngIntElt select max_comp else 1, 
          func<X|[ x`subgroup : x in MaximalSubgroups(X:IsTransitive:= Transitive)]>,
          func<X| true>:
...
intrinsic EnterStauduhar(G::GrpPerm, K::FldNum:Subfields:=false, DivisorOfOrder := 1) -> GrpPerm, [], GaloisData
...
intrinsic GaloisGroup(f::RngUPolElt[RngOrd]: Ring := false, 
  ...
  require Degree(f) gt 0: "Polynomial must not be constant";
  A := SimpleExtension(AbsoluteOrder(CoefficientRing(f)));
  f := Polynomial(A, f);
...
function galois_group_for_all(f, prime, ShortOK, lf, lF, llf, ff);
	...
	if assigned S`Permutation then    
	  ...
	  if S`Type eq "Q(t)" then
	      f2 := Polynomial(S`Ring, [S`BaseMap(x, <5, 5>) : x in Eltseq(f)]);
	      assert &and[ [Valuation(y) : y in Eltseq(Evaluate(f2, x))][1] gt 0 : x in S`Roots];
	  else
	      if assigned S`BaseMap then
	...
	Sall := InternalGaloisGroupProductSetup(S, ff, lf);
	  ...
	  if Sall`Type eq "Q(t)" then
	      f2 := Polynomial(Sall`Ring, [Sall`BaseMap(x, <5, 5>) : x in Eltseq(f)]);
	      lG := [];
	  end if;

	for i in [Sall`Type eq "Q(t)" select 1 else 2 .. #lf] do
	      S := galois_data_copy_for_factor(Sall, lf[i][1]);
	    if Type(lF[1]) eq FldFun then
	      SC := GaloisData(lF[i]);
	      assert not assigned S`KnownSubgroup;
	      //S`KnownSubgroup := SC`KnownSubgroup;
	      S`CycleTypes := SC`CycleTypes;
		if assigned SC`DescentChain then
		    S`DescentChain := SC`DescentChain;
		end if;
...
//"S`Roots = ", S`Roots;
    ...
    if S`Type eq "Q(t)" then
	f1 := Polynomial([S`Ring!S`BaseMap(x, <1, 5>) : x in Eltseq(f)]);
      assert &and[[Valuation(y) : y in Eltseq(Evaluate(f1, x))][1] gt 0 : x in S`Roots];
    else
	if assigned S`BaseMap then
    ...
    */
	...
	pr +:= Degree(llf[j][1]);
	  ...
	  if assigned S`BaseMap then
		...
		if S`Type eq "Q(t)" then
		  _, mmrfb := ResidueClassField(S`Ring);
		  mrfb := mmrfb*mrfb;
		  Append(~r, Roots(Polynomial(F, Polynomial(rfb, [mrfb(S`BaseMap(c, <1, 5>)) : c in Coefficients(llf[j][1])])), F)[1][1]);
		else
	  ...
	  if S`Type eq "Q(t)" then
	    S`max_comp := Parent(S`max_comp)!([Maximum(Coefficient(S`max_comp, i), Coefficient(sl`max_comp, i)) : i in [0 .. Minimum(RelativePrecision(S`max_comp), RelativePrecision(sl`max_comp))-1]] cat [Coefficient(S`max_comp, i) : i in [Minimum(RelativePrecision(S`max_comp), RelativePrecision(sl`max_comp))-1 .. RelativePrecision(S`max_comp)-1]]);
	  else
...
//[Valuation(Evaluate(f1, x)) : x in S`Roots];
    ...
    if assigned S`BaseMap then
    if S`Type eq "Q(t)" then
	g := Polynomial([S`BaseMap(c, <5, 25>) : c in Coefficients(f)]);
    else
...
"Reducible GetRoots to include linears and multiplicities";
	    assert #SS`Roots eq Degree(f);
	    if Type(Prec) eq RngIntElt then
		if Precision(Representative(SS`Roots)) ge Prec then
		    return;
		end if;
	    elif Precision(Representative(SS`Roots))[1] ge Prec[1] and Precision(Representative(SS`Roots))[2] ge Prec[2] then
		return;
	    end if;
		...
		if Degree(llf[j][1]) eq 1 then
		    ...
		    */
		    ...
		    for k in [s .. llf[j][2]] do
			Append(~SS`Roots, SS`BaseMap(-TrailingCoefficient(llf[j][1]), Prec));
			//Roots(Polynomial(SS`Ring, [SS`BaseMap(c, Prec) : c in Coefficients(llf[j][1])]))[1][1]);
		    end for;
		elif llf[j][2] ge 2 then
		    for k in [2 .. llf[j][2]] do
    ...
    return Image(h), [GaloisRoot(n, S : Prec := S`Type eq "Q(t)" select <1, 5> else 1) : n in [1 .. #S`Roots]], S;
end function;

import "GalRelRest.m" : GetGaloisData;
...
{}
	...
	ff := Polynomial([S1`mKQ(x) : x in Eltseq(f)]);
	assert IsSquarefree(ff);
	KK := NumberField(ff : Check := false, DoLinearExtension);
//ff, KK;
	S := GetGaloisData(KK);// : Prime := S1`S`Prime);//, Ring := S1`S);
	S := InternalGalQt_ComputeRoots(f, S1`Prime, <1, 5> : RatPrime := S`Prime);
	delete S`KnownSubgroup;
	base_map := map<CartesianProduct(FieldOfFractions(CoefficientRing(f)), CartesianProduct(Integers(), Integers())) -> S`Ring | t :-> S`BaseMap(t[1], t[2])>;
    else
...
//S`Ring;
    ...
    if assigned S1`IsInt and not assigned S`IsInt then
	function IsInt(x, B, SS : EasyNonInt := false)
	    ...
	    if not f and SS`Type ne "Q(t)" then
	      vprint GaloisGroup, 1: "no int - ring error";
	      return false, _;
	    end if;
//"Product setup IsInt";
	    return S1`IsInt(x, B, SS`Type eq "Q(t)" select SS else S1 : EasyNonInt := EasyNonInt);
	end function;
	S`IsInt := IsInt;
    end if;

    if assigned S1`max_comp and not assigned S`max_comp then
...
function galois_data_copy_for_factor(S, f)
	...
	assert IsSquarefree(ff);
	KK := NumberField(ff : Check := false, DoLinearExtension := true);
	//SC`S := GaloisData(KK: Prime := S`S`Prime);
	SC`S := galois_data_copy_for_factor(S`S, ff);
	SC`S`Ring := S`S`Ring;
	SC`S := GetGaloisData(KK : Prime := S`S`Prime, Ring := SC`S);
	//SC := InternalGalQt_ComputeRoots(Polynomial(FieldOfFractions(CoefficientRing(f)), f), S`Prime, <1, 5> : S := SC);
	//SC`S`GetRoots(SC`S);
...
//assigned lG[1][2]`GetRoots;
    if S`Type eq "Q(t)" or assigned lG[1][2]`GetRoots then
      procedure GetRoots(SS:Prec := 20)
        if Type(Prec) eq RngIntElt then
	    if Precision(Representative(SS`Roots)) ge Prec then
	      return;
...
//[Valuation(Evaluate(f1, x)) : x in SS`Roots];
	    ...
	    if SS`Type eq "Q(t)" then
	      assert &and[[Valuation(y) : y in Eltseq(Evaluate(f2, x))][1] gt 0 : x in SS`Roots];
	    else 
      ...
      if #non_coprime gt 0 then
	  procedure GetRoots(SS:Prec := 20)
//"Non coprime GetRoots";
	    r := Representative(SS`Roots);
	    if Type(Prec) eq RngIntElt and Precision(r) ge Prec or (Type(Prec) eq Tup and Precision(r)[1] ge Prec[1] and Precision(r)[2] ge Prec[2]) then
//"Non coprime GetRoots";
//Precision(Representative(SS`Roots)),Prec;
	      return;
	    end if;
	    // This needs to be done here for the reducible case because of the 
	    // different splitting field setup to the irreducible case
    ...
    S`Scale := #scales eq 0 select 1 else &*(scales);
    if #non_coprime gt 0 then 
    scales := [lG[x][2]`Scale : x in non_coprime | assigned lG[x][2]`Scale];
    Snc`Scale := #scales eq 0 select 1 else &*(scales);
    end if;
...
//ResidueClassField(CoefficientRing(lG[i][2]`Ring));
      ...
      if S`Type eq "Q(t)" then
	  assert &and[IsWeaklyZero(Evaluate(MinimalPolynomial(mF1(x)), F!mF1(x))) : x in lG[i][2]`Roots];
	  assert &and[[Valuation(y) gt 0 : y in Eltseq(Evaluate(Polynomial(S`Ring, MinimalPolynomial(mF1(x))), S`Ring!F!mF1(x)))][1] : x in lG[i][2]`Roots];
      else 
	  ...
	  f2 := Polynomial(F, Polynomial(F1, [mF1(x) : x in Eltseq(f1)]));
	  if S`Type eq "Q(t)" then
	      assert &and[[Valuation(y) : y in Eltseq(Evaluate(f1, x))][1] gt 0 : x in lG[i][2]`Roots];
	      assert &and[Valuation(Eltseq(Evaluate(f2, mF1(x)))[1]) gt 0: x in lG[i][2]`Roots];
	      assert &and[Valuation(Eltseq(Evaluate(f2, mF1(x)))[1]) gt 0: x in r1];
	  else 
	      assert &and[Valuation(Evaluate(f1, x)) gt 0: x in lG[i][2]`Roots];
	      assert &and[Evaluate(f2, mF1(x)) eq 0: x in lG[i][2]`Roots];
	      assert &and[Evaluate(f2, mF1(x)) eq 0: x in r1];
	  end if;  
//"lG[i][2]`Roots ok", i;
      end if;
      ...
      if S`Type eq "Q(t)" then
	  assert &and[&and[Valuation(y) gt 0: y in Eltseq(Evaluate(f2, S`Ring!x))] : x in r1];
	  assert &and[&and[Valuation(y) gt 0 : y in Eltseq(Evaluate(f2, S`Ring!x))] : x in ChangeUniverse(r1, F1)];
      else
	  ...
	  assert &and[IsWeaklyZero(Evaluate(f2, x)) : x in r1];
      else
	  assert &and[Valuation(Evaluate(f2, S`Ring!x)) gt 0: x in r1];
      ...
      if assigned lG[i][2]`BaseMap then
	  assert &and[IsWeaklyZero(x) : x in Coefficients(f1 - Polynomial(S`Base, f2))];
      end if;
//"r1 roots ok", i;
      r cat:= r1;
      if i in non_coprime then
...
//end for;
    ...
    if S`Type eq "Q(t)" then
      assert &and[&and[Valuation(y) gt 0: y in Eltseq(Evaluate(f1, x))] : x in S`Roots];
    else
	assert &and[Valuation(Evaluate(f1, x)) gt 0 : x in S`Roots];
    end if;
...
//"Product max_comp :", max_comp;
    ...
    when "Q(t)":
	assert &and[Valuation(x[2]`max_comp) eq 0 : x in lG];
	max_comp := [Eltseq(x[2]`max_comp) : x in lG];
	max_max_comp := Maximum([#x : x in max_comp]);
	max_comp := [x cat [0 : i in [1 .. max_max_comp-#x]] : x in max_comp];
	max_comp := [Maximum([x[i] : x in max_comp]) : i in [1 .. #max_comp[1]]];
	max_comp := Parent(lG[1][2]`max_comp)!max_comp;
	if #non_coprime gt 0 then
	max_compnc := [Eltseq(lG[x][2]`max_comp) : x in non_coprime];
	max_max_compnc := Maximum([#x : x in max_compnc]);
	max_compnc := [x cat [0 : i in [1 .. max_max_compnc-#x]] : x in max_compnc];
	max_compnc := [Maximum([x[i] : x in max_compnc]) : i in [1 .. #max_compnc[1]]];
	max_compnc := Parent(lG[1][2]`max_comp)!max_compnc;
	end if;
    end case;
