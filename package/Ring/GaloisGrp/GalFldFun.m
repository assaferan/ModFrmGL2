...
intrinsic InternalGalQt_ComputeRoots(f::RngUPolElt[FldFunRat], P::RngUPolElt, 
  ...
  if S cmpeq false then
    if RatPrime cmpne false then 
	KQ := NumberField(Order(RatPrime));
    else
	KQ := NumberField(P);
    end if;
...
//Derivative(P);
//Evaluate(Derivative(P), KQ.1);
//KQ.1, KQ;
//1/Evaluate(Derivative(P), KQ.1);
...
/*
if false and not IsExport() then
"This is non-export printing";
SS`Ring;
Denominator(x);
if x ne 0 then
SS`Ring!([S`BaseMap(xx, p[2]) : xx in Eltseq(Numerator(h(Numerator(x))))]);
SS`Ring!([S`BaseMap(xx, p[2]) : xx in Eltseq(Numerator(h(Denominator(x))))]);
    SS`Ring!inner(Numerator(h(Numerator(x)))) ,
           inner(Numerator(h(Denominator(x)))),
...
*/
  ...
  if pAdic then
    ...
    pRoots := func<|[GaloisRoot(i, S:Prec := prec[2]) : i in [1..Degree(f)]]>();
    Sc := SS`BaseMap(Scale, prec);
    g := Polynomial([S`BaseMap(x, prec[2]) : x in Eltseq(Derivative(r_ff))]);
    Rmap := hom<Parent(Sc) -> R | Coercion(CoefficientRing(Parent(Sc)), CoefficientRing(R))*Coercion(CoefficientRing(R), R), R.1>;
    ff := Polynomial([ Rmap(SS`BaseMap(x, prec)): x in Eltseq(f)]);
...
//  "Pre-Verify roots:", [ [Valuation(y) : y in Eltseq(Evaluate(ff, x))] : x in pRoots];
    assert &and[ [Valuation(y) : y in Eltseq(Evaluate(ff, x))][1] gt 0 : x in pRoots];
//    assert &and[Valuation(Evaluate(ff, x)) gt 0 : x in pRoots];
...
//      if EasyNonInt then return true, 1; end if;
      ...
      Den_p := den_p^(2*vx+1);
      for i in [1..#x] do
        if assigned SS`S`IsInt then
          f, t := SS`S`IsInt(x[i]*Den_p, B[2], SS`S);
        else
          f, t := IsInt(x[i]*Den_p, B[2], SS`S);
        end if;
...
/*    //this gives bounds in Q(t)
    ...
    t0 := 0;
    R := Integers(KQs);
gg := Polynomial(KQ,[((R!Numerator(x)) mod (R.1-t0)) / ((R!Denominator(x)) mod (R.1 - t0)) : x in Eltseq(gf)]);
//"gg : ", gg;
    while Degree(gg) ne Degree(f) or not IsSquarefree(gg)  do
      t0 +:= 1;
//      "Substitute!! (KQ)";
gg := Polynomial(KQ,[((R!Numerator(x)) mod (R.1-t0)) / ((R!Denominator(x)) mod (R.1 - t0)) : x in Eltseq(gf)]);
    end while;
  ...
  if not assigned SS`GetRoots then
...
pr;
h_KP_Fqt(sum_roots);
Degree(Eltseq(SS`f)[Degree(SS`f)]) - Degree(SS`Scale);
pr/Degree(SS`Prime);
*/
...
function galois_group(f, prime, next_p, ShortOK, Ring, ProofEffort)
    ...
    if false and Characteristic(Pff) eq 0 then
	error "Polynomial must be irreducible when characteristic is 0";
    end if;

    assert Ring cmpeq false;
...
//Prime, #primes, char_multiplier*n + 1, min_deg, #other_primes;
...
//Prime;
  ...
  if cfe and DimensionOfExactConstantField(K) eq Degree(K) and Degree(K) gt 1 then
    if Characteristic(K) gt 0 then
...
function get_primes_rel(f, max_fact_deg, sfp)
	...
	ff := CoefficientRing(f)!LCM([Denominator(x, MF) : x in Coefficients(f)])*f;
    else 
	ff := f;
    end if;
    n := Degree(f);
...
{Return the galois group of the function field K}
    ...
    require IsSimple(K) : "Function field must be a simple extension";
    require Characteristic(K) gt 0 : "Function field must have positive characteristic";
...
{Return the galois group of the polynomial f}
    require Characteristic(CoefficientRing(f)) gt 0 : "Galois group calculations over non-rational function fields are currently only available for positive charactertistic";
...
{Return the galois group of the polynomial f}
    ...
    require Characteristic(A) gt 0 : "Galois group calculations over non-rational function fields are currently only available for positive charactertistic";
    f := Polynomial(A, f);
