...
// "Skip Prime ",Norm(p[1])," because of subfield discriminants";
  ...
  if Ring cmpne false then
    ...
    if Prime cmpne false then
      ...
      l := 5;//CF: TODO not good criteria!!!
      u := 9;// it used to be u := 10 which is also bad.
...
//S`BaseMap(k.1, 20);
  ...
  if assigned S`Roots then
    ...
    S`Roots := [ InternalEasyCompletionsUse(K.1, h, 20) : h in S`RootData];
    ff := Polynomial([S`BaseMap(x, 5) : x in Coefficients(DefiningPolynomial(K))]);
    assert &and[Valuation(Evaluate(ff, x)) gt 0 : x in S`Roots];
  end if;
...
//"GalRelRest IsInt";
    ...
    if not f then
      assert Parent(x) cmpeq SS`Ring or Integers(Parent(x)) eq SS`Ring; 
      assert IsCoercible(SS`Ring, x);
    ...
    assert IsWeaklyEqual(SS`BaseMap(c, pr), SS`Ring!x);
...
// "Starting Group in GalRelRest.m";
...
intrinsic Conjugates(x::FldAlgElt[FldAlg] : Precision:=0) -> []
  ...
  if Precision gt 0 then
    con := [Evaluate(x, p : Precision:=Precision) : p in places];
  else
    con := [Evaluate(x, p) : p in places];
  end if;
...
function galois_splitting_field(f, Galois, Roots, Stab, AllAuto, Chain, Name, Inv, MaxBound, MinBound)
  assert Type(CoefficientRing(f)) in {FldRat, RngInt, FldNum, FldOrd, RngOrd, FldCyc, FldQuad};
...
intrinsic GaloisSplittingField(f::RngUPolElt[FldAlg[FldRat]]:Galois := false, Roots := true,
                              Stab := true, AllAuto := false, Chain := false,
                              Name := false, Inv := false, 
                              MaxBound := Infinity(), MinBound := 1)
             -> FldNum, [], GrpPerm, []
...
intrinsic GaloisSplittingField(f::RngUPolElt[RngOrd[RngInt]]:Galois := false, Roots := true,
                              Stab := true, AllAuto := false, Chain := false,
                              Name := false, Inv := false, 
                              MaxBound := Infinity(), MinBound := 1)
             -> FldNum, [], GrpPerm, []
...
intrinsic GaloisSplittingField(f::RngUPolElt[FldAlg[FldAlg]]:Galois := false, Roots := true,
                              Stab := true, AllAuto := false, Chain := false,
                              Name := false, Inv := false, 
                              MaxBound := Infinity(), MinBound := 1)
             -> FldNum, [], GrpPerm, []
{Computes a splitting field for f (as a tower) and returns the roots in it}
    f := Polynomial(AbsoluteField(CoefficientRing(f)), f);
    return galois_splitting_field(f, Galois, Roots, Stab, AllAuto, Chain, Name, Inv, MaxBound, MinBound);
end intrinsic;

intrinsic GaloisSplittingField(f::RngUPolElt[RngOrd[RngOrd]]:Galois := false, Roots := true,
                              Stab := true, AllAuto := false, Chain := false,
                              Name := false, Inv := false, 
                              MaxBound := Infinity(), MinBound := 1)
             -> FldNum, [], GrpPerm, []
{Computes a splitting field for f (as a tower) and returns the roots in it}
    f := Polynomial(AbsoluteOrder(CoefficientRing(f)), f);
    return galois_splitting_field(f, Galois, Roots, Stab, AllAuto, Chain, Name, Inv, MaxBound, MinBound);
end intrinsic;
...
//  print "Need ", n, "roots of unity";
  ...
  r := [GaloisRoot(i,S:Scaled := false) : i in [1..Degree(G)]];
  delete S`Subfields;
  delete S`SubfieldLattice;
...
// hmm, AutomorphismGroup is (much) slower in the one example I tried
//    AG,_,mp:=AutomorphismGroup(K); assert IsCyclic(AG) and #AG ne 1; i,#AG;
//    GENS:=[x : x in AG | Order(x) eq Order(AG)]; sig:=mp(GENS[1]);
// should be nothing more than arbitrary choices of cube roots, but...
      A:=Automorphisms(K); sig:=A[#A]; // Deg(K)=#A is prime...
      zeta:=Roots(CyclotomicPolynomial(Degree(K)),k)[1][1];
      K:=CyclicToRadical(K,sig(K.1),zeta);
    end if;
    k := K;                                       
  end for;
