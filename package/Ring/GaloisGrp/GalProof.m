...
import "Galois.m" : get_frobenius, PrecisionFromBound;
...
function Qt_bound(b0, S, ind)
 assert S`Type eq "Q(t)";
    prec_b0 := AbsolutePrecision(b0);
    b0 := ChangePrecision(Parent(b0), Maximum(Precision(Parent(b0)), prec_b0*ind))!b0;
    ChangePrecision(~b0, Infinity());
    b0 := b0^ind;
    ChangePrecision(~b0, prec_b0*ind);
    b0 *:= Max([Binomial(ind,e) : e in [0..ind]]); 
 return b0;
end function;

 /*
 elif S`Type eq "F_q(t)" then
 else 
     b0 := Max([b0^i * Binomial(ind,i) : i in [0..ind]]);
 end if;
 */
...
function BuildSetResolvent(f,n,S: U_test := false)
 ...
 repeat
     if S`Type eq "Q(t)" then
	sep_prec := <sep_prec[1]+1, sep_prec[2]+1>;
     else
      sep_prec := sep_prec + 1;
     end if;
  all_tschirni := {};
...
function ProofStepBySetResolvent(G, U, f, S, n) 
 ...
 // Uses MSum, so invariant is \sum x_i
...
// a check of this factor will improve U0
   ...
   r_deg := #orb_u[k];
assert r_deg eq &+[#l : l in loc_orbs];
   if S`Type eq "Q(t)" then
       // constant coeff of resolvent has largest deg coeff as prod of roots
       // there are r_deg evaluations of the invariant
       // there are n roots added in the evaluations of the invariant
       // adding roots should only affect the coeffs not the degree
       // tschirn(root) are the summands
       // use the maximum binomial coefficient to contribute to the Q bound
       b0 := (n*Evaluate(tschirn, S`max_comp));
       if not true then
       prec_b0 := AbsolutePrecision(b0);
       b0 := ChangePrecision(Parent(b0), Maximum(Precision(Parent(b0)), AbsolutePrecision(b0)*r_deg))!b0;
b0;
       ChangePrecision(~b0, Infinity());
//b0 := &+[(b0)^e * Binomial(r_deg,e) : e in [0..r_deg]]; 
prec_b0;
       b0 := b0^r_deg;
b0;
Degree(res), r_deg;
       ChangePrecision(~b0, 2*prec_b0*r_deg);
AbsolutePrecision(b0);
       b0 *:= Max([Binomial(r_deg,e) : e in [0..r_deg]]);
AbsolutePrecision(b0);
       b0 := b0^2;
b0;
       end if;
       b0 := Qt_bound(b0, S, r_deg);
	b0 := b0^2;		// why??
       prec := PrecisionFromBound(b0, S);
   //elif S`Type eq "Fq(t)" then
   else
       b0 := Max([(Evaluate(tschirn,S`max_comp)*n)^e * Binomial(r_deg,e) : e in [0..r_deg]]); 
       prec := 5 + Ceiling(Log(b0)/Log(S`Prime));
   end if;
   change_prec := prec;
   if Type(change_prec) eq RngIntElt then
    change_prec := <change_prec, change_prec>;
   end if;

   if sep_prec cmpeq 1 then //or sep_prec cmpeq <1, 1> then
    ff,pi := ResidueClassField(S`Ring);  
    rt := [pi(GaloisRoot(i,S:Prec := sep_prec)) : i in [1..Degree(f)]]; 
    rt := [Evaluate(tschirn,a) : a in rt];
    ff0,pi0 := ResidueClassField(S`Base); 
    fac0 := &*[PolynomialRing(ff0)!&*[Polynomial([-&+[pi0(rt[j]) : j in aa],1]) : aa in a] : a in loc_orbs];
    res_loc := PolynomialRing(ChangePrecision(S`Base,change_prec[2]))!res;
...
//    if not IsExport() then printf"N"; end if;
    ...
    rt := [Evaluate(tschirn,a) : a in rt]; 
    fac0 := &*[//PolynomialRing(ChangePrecision(S`Base,change_prec[2]))!
               PolynomialRing(ChangePrecision(S`Ring,change_prec[1]))!
...
prec0 := PrecisionFromBound(Index(G, U)^2, S);
prec00 := prec0;
if S`Type eq "Q(t)" then
    prec00 := prec0[1];
end if;
 ...
 repeat
  ...
  prec00 := prec00 + 1;
  if S`Type eq "Q(t)" then
    prec0 := <prec0[1]+1, prec0[2]+1>;
else
    prec0 := prec00;
end if;
  tschirn0 := tschirn;
  tschirn := get_tschirni(S, 3, Minimum(Degree(G), 2+#all_tschirni), all_tschirni,inv`GalInvType);
 until Max([Valuation(val_s[i] - val_s[j]) : i,j in [1..#val_s] | i lt j]) lt prec00 - 1; 
 ...
 tschirn := tschirn0;
 if S`Type eq "Q(t)" then
    b0 := S`Bound(tschirn, inv, 1); //Index(G, U));
    b0 := Qt_bound(b0, S, Index(G, U));
    //b0 := b0^2;
    /*
    prec_b0 := AbsolutePrecision(b0);
    b0 := ChangePrecision(Parent(b0), Maximum(Precision(Parent(b0)), 2*prec_b0*Index(G, U)))!b0;
    ChangePrecision(~b0, Infinity());
    b0 := b0^Index(G, U);
    ChangePrecision(~b0, 2*prec_b0*Index(G, U));
    b0 *:= Max([Binomial(Index(G, U),e) : e in [0..Index(G, U)]]); 
    b0 := b0^2;
    */
 elif S`Type eq "F_q(t)" then
    b0 := S`Bound(tschirn, inv, Index(G,U));
 else 
 ...
 if S`Type eq "Q(t)" then
     rt := [Evaluate(tschirn,GaloisRoot(i,S:Bound := b0)) : i in [1..Degree(G)]];
 else 
...
// first multiply the orbits of the local group
 if not ISA(Type(S`Ring), RngSer) and NumberOfOperations(inv, "*") gt Degree(S`Ring) then
...
// Cast the local factors down to the base: 
 res_loc := &*[//PolynomialRing(ChangePrecision(S`Base,prec[2]))!
	    PolynomialRing(ChangePrecision(S`Ring,prec[1]))!a :  a in fac_l];
 co := Coefficients(res_loc);
 co2 := [];
 for a in co do
  bool, e2 := IsInt(a, b0, S);
  //e2 := Integers()!a;
...
function ProofStepGeneral(G, U, S, U_res)
 ...
 if S`Type eq "Q(t)" then
    b0 := S`Bound(tschirn, inv, 1);
    b0 := Qt_bound(b0, S, Index(G, U_res));
    //b0 := b0^2;
    /*
    prec_b0 := AbsolutePrecision(b0);
    b0 := ChangePrecision(Parent(b0), Maximum(Precision(Parent(b0)), 2*prec_b0*Index(G, U_res)))!b0;
    ChangePrecision(~b0, 2*prec_b0*Index(G, U_res));
    b0 := b0^Index(G, U_res);
    b0 *:= Max([Binomial(Index(G, U_res),e) : e in [0..Index(G, U_res)]]); 
    b0 := b0^2;
    */
 elif S`Type eq "F_q(t)" then
    b0 := S`Bound(tschirn, inv, Index(G,U_res));
 else 
     b0 := Bound(inv,Evaluate(tschirn, S`max_comp));
...
if false then 
    S`DescentChain;
    dc := [<Index(dc[i][1], dc[i+1][1]), i+1> : i in [1 .. #dc - 1]] where dc := S`DescentChain;
    Append(~dc, <Index(Sym(Degree(f)), S`DescentChain[1][1]) div 2, 1>);
    Sort(~dc);
    Reverse(~dc);
dc;
 res := NewGaloisProof(S`DescentChain[#dc gt 0 select dc[1][2] else #S`DescentChain][1],S);
 else 
