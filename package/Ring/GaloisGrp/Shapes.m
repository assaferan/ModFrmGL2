...
{Divisor of order of Galois group of an irreducible polynomial with cycle types as in Patterns}
 ...
 for p := 1 to n do 
  if IsPrime(p) then
   sel_p := [p_seq[i] : i in [1..#p_seq] | LCM_seq[i] mod p eq 0];
   if #sel_p eq 0 then continue p; end if;
   syl_pat := { &cat [ [p^e : j in [1..b div p^e]] where e is Valuation(b,p) : b in a] : a in sel_p};
   syl_fix_pat := { Sort([b div m : k in [1..m], b in a ]) where m is Minimum(a) : a in syl_pat};
// Cycle types in the p-Sylow-Subgroup of the 1-point-stabilizer
// Extract the longest cycle of each pattern.
   long_cyc := SetToSequence({[b : b in a | b eq m] where m is Maximum(a) : a in syl_fix_pat});   
   fac1, ind := Max([a[1] : a in long_cyc]);
   len := &+long_cyc[ind];
   fac2 := Max([1] cat [a[1] : a in long_cyc | &+a ne len]);
   t3 := t3 * fac1 * fac2;
// Lemma 16 of Elsenhans/Klueners for 1-point-stabilizer   
  end if;
 end for;
 t1 := LCM(LCM_seq);
...
/* t1 factor of group order. 
   t2 factor of order of 1-point-stabilizer.  */
end intrinsic;
