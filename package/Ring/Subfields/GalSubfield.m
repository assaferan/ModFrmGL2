...
/* Pretest based on factorization patterns.
   ...
   Idea of the approach: See section 4.2  */

function count_block_sum(bl,n)
 cnt := [1] cat [0 : i in [1..n]];
 last := 1;
 for i := 1 to #bl do
  last := last + bl[i];
  for j := last to bl[i] + 1 by -1 do
   cnt[j] := cnt[j] + cnt[j - bl[i]]; 
  end for;  
 end for;
 return cnt; 
end function;
...
function possible_block_sizes_iterative(ct: start := false)
 n := &+ct;
 if start cmpeq false then
  start := [a : a in Divisors(n) | a ne 1 and a ne n];
 end if;
 for i := 1 to #ct do // Teste i-ten Zykel
  if i ne Index(ct,ct[i]) then continue i; end if; // Mehrfach gleiche Laenge 
  bewiesen := [a : a in start | ct[i] mod a eq 0];
  offen := [a : a in start | ct[i] mod a ne 0];
  for m in Divisors(ct[i]) do // m Bloecke auf einem orbit
   if #offen eq 0 then continue i; end if; 
   bls := count_block_sum([ct[j] div m : j in [1..#ct] | 
                        j ne i and
                        ct[j] mod m eq 0],n div m);
   offen_2 := [];
   for a in offen do
    if (a*m le n) and (ct[i] le m * a) and 
	   (bls[a - ct[i] div m +1] gt 0) then
     Append(~bewiesen, a); 
    else
	 Append(~offen_2,a);
    end if;	 
...
function PossibleBlockSizes(Patterns)
 Patterns := SetToSequence(Patterns);
 ls := {&+a : a in Patterns};
 assert #ls eq 1; 
 start := false;
 for i := 1 to #Patterns do
  start := possible_block_sizes_iterative(Patterns[i]: start := start);
  if #start eq 0 then return start; end if;
 end for;
 return start;
end function;
...
procedure FindPrime(p, s : full := true)
  //if assigned s`Prime then s`Prime;end if;
...
//"0 : ", p;
  ...
  D := Lcm(D, Denominator(s`fs_alpha_inv)); 
//D;
  d := 1;
  ...
  if (Type(k) ne FldRat) then
     f_sub := DefiningPolynomial(k); 
// We are in the relative number field case. The function fields have the UseGalois
     scale_bf := LCM([Denominator(CoefficientRing(k)!x) : x in 
	              Eltseq(Polynomial(CoefficientRing(k), f_sub)/LeadingCoefficient(f_sub))]);
     eq_order := Order([(k.1*scale_bf)^i : i in [0..Degree(k)-1]] : Order, Verify := false);
  else
     scale_bf := 1;
  end if;
...
// Relative number field case: Construct the primes above p without a maximal order computation:
      co_spez := Coefficients(Polynomial(GF(p), f_sub)); 
      spez := Polynomial([co_spez[i] * GF(p)!scale_bf^(#co_spez-i) : i in [1..#co_spez]]);
      assert Degree(spez) eq Degree(f_sub);
      fac := Factorization(spez);
      fac := [a[1] : a in fac | a[2] eq 1];
      lp := [ideal<eq_order | {p, Evaluate(PolynomialRing(Integers())!a, k.1 * scale_bf) } > : a in fac]; 
//      lp := [ x : x in Support(MaximalOrder(k)*p)];
      //lp := [x[1] : x in Decomposition(MaximalOrder(k), p)];
    end if;
    for P in lp do
// k : Magma; P : Magma;  Norm(P);
//"Hallo 1";
      C, cm := Completion(k, P);
//"Hallo 2";
	...
	mR := cm*mR;
      ...
      if Gcd(f,Derivative(f)) ne 1 then
         continue;
      end if;
      fac_di := DistinctDegreeFactorization(f); 
      degs := [ a[1] : j in [1..Degree(a[2]) div a[1]], a in fac_di];
...
//lf;
	...
	if #PossibleBlockSizes(cycletypes) eq 0 then
...
function lattice_create(K)
  ...
  AddAttribute(SetEnum, "fs_alpha");                 // f'(alpha)  
  AddAttribute(SetEnum, "fs_alpha_inv");             // 1/f'(alpha) 


  s`fs_alpha := Evaluate(Derivative(DefiningPolynomial(K)),K.1);
  s`fs_alpha_inv := 1/ s`fs_alpha;
...
function BlockFromPrimitiveElement(s, elt, deg)
    ...
    h := s`Top!elt;
    h := Eltseq(h);
    if not assigned s`Prime then FindPrime(Degree(K),s); end if; // MW instafix
    p := s`Prime[1];
    if not ISA(Type(p), RngElt) then
	p := Minimum(p);
    end if;
