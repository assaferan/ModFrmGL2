...
intrinsic One(W::RngUPolTwst) -> RngUPolTwstElt 
   {The identity element of R.}
   return W!1;
end intrinsic;
...
{A right gcd g such that a = x * g and b = y * g for some x,y.}
  
  W := Parent(a);
  require Parent(b) eq W : "Polynomials must be from the same ring";
  ANS:=New(RngUPolTwstElt); ANS`Parent:=W; // MW, can't reuse "a" w/ attribute
  repeat
    _,c := Quotrem(a,b);
    a := b;
    b := c;
  until c eq W!0 or Degree(c) eq 0 and IsWeaklyZero(LeadingCoefficient(c));
  ANS`Element := Normalize(a`Element);
  return ANS;
end intrinsic;
