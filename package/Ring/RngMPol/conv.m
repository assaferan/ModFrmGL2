...
*/
	...
	*/
	...
	if Degree(F) eq 1 or Degree(mf) eq 1 then
	    ...
	    PF := PrimeField(F);
	    pre := S ! Eltseq(Generator(F, PF), SS);
	    cfi := hom<F -> S | pre>;
	end if;
	r := Rank(R);
	...
	when RngFunFrac:
	    ...
	    u := <
		[],
		[S!PI.i: i in [1 .. Rank(PI)]],
		[S!BPI.i: i in [1 .. Rank(BPI)]]
	    >;
	when FldFun:
/*
"S:", S;
...
*/
	...
	for i := #L to polyi + 1 by -1 do
		...
		/*
		Previous step used [FV[j]: j in [transv + 1 .. transv + tv]]
		as images of inner hom f from the rat func field BPI.
		That will be ignored by the hom nf below from S.
		So make sure the images match the gens of S.
		*/

		ag := {@ S!PI.i: i in [1 .. av] @};
		tg := {@ S!BPI.i: i in [1 .. av] @};

		ai := algv - av + 1;
		ti := transv + 1;
		ims := [];
		for i := 1 to Rank(S) do
		    j := Index(ag, S.i);
		    if j gt 0 then // S.i = S!PI.i
			Append(~ims, APV[ai]);
			ai +:= 1;
		    else
			j := Index(tg, S.i); // S.i = S!BPI.i
			Append(~ims, FV[ti]);
			ti +:= 1;
		    end if;
		end for;
		...
		algv -:= av;
		// Leave transv as before (already changed by prev step)
	    when RngMPolRes, FldFunFracSch:
		/*
