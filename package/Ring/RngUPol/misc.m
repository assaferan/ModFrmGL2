...
of its coefficient ring/field}
    ...
    // handle case where L is a RngOrd or FldAlg
    try
        LL := NumberField(L);
        L := LL;
    catch E
    ...
    // The code below is not correct unless L is an extension of K
    // obtained by adjoining the root of a single defining poly
    try
        l := DefiningPolynomial(L);
        ok := true;
    catch E
        ok := false;
    end try;
    require ok and Ngens(L) eq 1 and IsZero(Evaluate(l,L.1)) :
        "The coefficient ring must be an extension of its base ring " *
        "obtained by adjoining the root of a single defining poly";

    P2<x, y> := PolynomialRing(K, 2);
    g := &+[
        MultivariatePolynomial(
            P2, Polynomial(K, Eltseq(L!Coefficient(f,i))), 2
        ) * x^i : i in [0..Degree(f)]
    ];
    chi := UnivariatePolynomial(
    Resultant(
        MultivariatePolynomial(P2, Polynomial(K, l), 2),
        g,
        2
    ));

    return Polynomial(BaseRing(CoefficientRing(P)), chi);
