...
intrinsic AutomorphismGroup(M::Mtrx : RowColours := 0, ColColours := 0, Al := "Default") -> GrpPerm
{The group of row and column permutations of M that leave m invariant}
    ...
    if Al eq "Leon" or
	(Type(R) eq FldFin and (IsPrime(#R) and #R le 255 or #R eq 4)
...
intrinsic IsIsomorphic(M1::Mtrx, M2::Mtrx : LeftRowColours := 0, RightRowColours := 0, LeftColColours := 0, RightColColours := 0, Al := "Default") -> BoolElt, GrpPermElt
{Find a permutation of the rows and columns that takes M1 to M2}
    ...
    if Al eq "Leon" or (Type(R1) eq FldFin and
	(IsPrime(#R1) and #R1 le 255 or #R1 eq 4)
...
{True if the character tables X and Y are equal up to permuting rows and
columns}
    if #X ne #Y then return false,_; end if;
