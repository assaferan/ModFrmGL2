...
import "generics.m": SymplecticSL2, OrthogSL2, l3qdim6, u3qdim6, l2q3dim8,
        ...
        s6qdim13, o5qdim13, s6qsympdim14, omplus6qdim14, omminus6qdim14,
        s6qsymporth14, om5qdim14, g2qdim14, sl3qdim15a, su3qdim15a,
        sl3qdim15b, su3qdim15b, sl5qdim15, su5qdim15, l6qdim15, u6qdim15,
        l4qdim15, u4qdim15, sl4qdim16, su4qdim16, l4q2dim16l, l4q2dim16u,
        sp4qdim16, l2q4dim16, s4q2dim16;
import "SpinRepsAuts.m": HalfSpinPlus, HalfSpinMinus, SpinCircle;
 ...
 */
 ...
 if type eq "O" then
   ...
   if IsEven(q) then
     error "Field must have odd order for type \"O\"";
   end if;
   sign := 0;
 end if;
 ...
 if 9 in classes and d gt 17 then
   "*WARNING*: Aschbacher Class 9 subgroups are not returned for dimensions";
   "           greater than 16. List of subgroups will be incomplete.";
 end if;
 ...
 //Make matrix C for making conjugates of subgroups under GL/GU/GSp
 if type eq "L" then
   C := GLMinusSL(d, qq);
 elif type eq "U" then
   C := GUMinusSU(d,q);
...
//In fact, we probably want this for larger d whenever q mod 4 eq 2.
  ...
  /* DFH, 24/6/16 O^+(4,q) wr S_2 is not maximal in dimension 16 */
               if d ne 16 then
	 ...
	 if (e eq 1 and p mod 24 in {17,23}) then
	   ...
	   if all then nconj := 6;
         ...
         if e eq 1 and (p mod 8 in {1,7} or
           ...
           //2.S5 (p mod 8 in {1,7}) or 2.A5 o.w. 
           _LRS := Read(c9lib cat "sl25d6");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:normaliser:=normaliser);
         ...
         if sign eq 1 then
          ...
          special:=special, general:=general, normaliser:=normaliser);
           ...
           if e mod 3 eq 0 then
             S :=sub< GL(d,q) |
                         ChevalleyGroup("3D",4,GF(q)),ScalarMatrix(8,-1) >;
             S:=S^TransformForm(S);
             if all then
               if p eq 2 then
                 asmax cat:= [S, S^CS];
       ...
       if type eq "L" then
         ...
         if e eq 1 and p mod 3 eq 1 then
           //S63
            _LRS := Read(c9lib cat "s63d13");
            _LR := eval _LRS;
            S := ReduceOverFiniteField(_LR,qq:general:=general);
            if all then nconj :=  GCD(q-1,13);
              asmax cat:= GConjugates(S[1],C,nconj);
            else Append(~asmax, S[1]);
            end if;
         end if;
         if e eq 1 and p mod 5 eq 1 then
           //U34
            _LRS := Read(c9lib cat "u34d13");
            _LR := eval _LRS;
            S := ReduceOverFiniteField(_LR,qq:general:=general);
            if all then nconj :=  GCD(q-1,13);
              asmax cat:= GConjugates(S[1],C,nconj);
         ...
         if e eq 1 and q mod 28 in {1,3,5,9,11,13,15,17,19,23,25,27} and
                                                     not q in {3,5} then
           //A14 for q mod 28 in {\pm5,\pm11,\pm13}, S14 ow
           _LRS := Read(c9lib cat "a14d13");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
                      special:=special,general:=general,normaliser:=normaliser);
           if all and q mod 28 in {1,3,9,19,25,27} then
             asmax cat:= [S[1],S[1]^CS];
           else Append(~asmax, S[1]);
           end if;
         end if;
         if q eq 3 or q eq 5 then
           _LRS := Read(c9lib cat "a15d14");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
                    special:=special,general:=general,normaliser:=normaliser);
           if all and q eq 3 then
             asmax cat:= [S[1],S[1]^CS];
           else Append(~asmax, S[1]);
           end if;
         end if;
         if e eq 1 and q mod 5 in {2,3} and not q eq 2 then 
           //L2_25
           _LRS := Read(c9lib cat "l225d13");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
                    special:=special,general:=general,normaliser:=normaliser);
           Append(~asmax, S[1]);
         end if;
         if e eq 1 and q mod 12 in {1,3,5,11} and not q eq 3 then
            //L33 if q mod 12 in {3,5}, L33.2 ow
           _LRS := Read(c9lib cat "l33d13");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
                    special:=special,general:=general,normaliser:=normaliser);
           if all and q mod 12 in {1,11} then
             asmax cat:= [S[1],S[1]^CS];
           else Append(~asmax, S[1]);
           end if;
         end if;
         if (e eq 1 and q mod 5 in {1,4}) or
            (e eq 2 and q mod 5 in {2,3} and not q eq 2) then
            //S45
           _LRS := Read(c9lib cat "s45d13");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
                    special:=special,general:=general,normaliser:=normaliser);
           if all then asmax cat:= [S[1],S[1]^CS];
       ...
       if type eq "L" then
         if novelties then continue; end if;
         if e eq 1 and q mod 24 in {1,3,7,19} then
           //Sp63
           _LRS := Read(c9lib cat "sp63d14");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq : general:=general);
            if all then nconj :=  GCD(q-1,14);
              asmax cat:= GConjugates(S[1],C,nconj);
            else Append(~asmax, S[1]);
            end if;
         end if;
         if e eq 1 and q mod 4 eq 1 then
           //Sp63
           _LRS := Read(c9lib cat "sz8d14");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq : general:=general);
             if all then nconj :=  GCD(q-1,14);
               asmax cat:= GConjugates(S[1],C,nconj);
               dual := sub< Generic(S[1]) |
                              [Transpose(S[1].i)^-1 : i in [1..Ngens(S[1])]] >;
               asmax cat:= GConjugates(dual,C,nconj);
             else Append(~asmax, S[1]);
             end if;
         end if;
       elif type eq "U" then
         if novelties then continue; end if;
         if e eq 1 and q mod 24 in {5,11,17,23} then
           //Sp63
           _LRS := Read(c9lib cat "sp63d14");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq :
         ...
         if e eq 1 and q mod 4 eq 3 then
           //Sp63
           _LRS := Read(c9lib cat "sz8d14");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq :
         ...
         if novelties then
           if e eq 1 and q mod 104 in {3,27,29,35,43,51,101,77,75,69,61,53} then
              //Sl213a
             _LRS := Read(c9lib cat "sl213d14a");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq : normaliser:=normaliser);
             Append(~asmax, S[1]);
           end if;
           if e eq 1 and q mod 40 in {11,19,21,29} then
              //2J2
             _LRS := Read(c9lib cat "2j2d14");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq : normaliser:=normaliser);
             Append(~asmax, S[1]);
           end if;
         ...
         if (e eq 1 and p mod 8 in {1,7}) or
                            (e eq 2 and p mod 8 in {3,5} and not p eq 3) then
           //2A7
           _LRS := Read(c9lib cat "2a7d14");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq : normaliser:=normaliser);
           if all then asmax cat:= [S[1], S[1]^C];
           else Append(~asmax, S[1]);
         ...
         if e eq 1 and q mod 8 in {1,7} or
                     q mod 104 in {5,11,19,21,37,45,99,93,85,83,67,59} then
            //Sl213a.2 for q mod 8 in {1,7}, SL213a ow 
           _LRS := Read(c9lib cat "sl213d14a");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq : normaliser:=normaliser);
           if all and q mod 8 in {1,7} then
             asmax cat:= [S[1], S[1]^C];
           else Append(~asmax, S[1]);
           end if;
         end if;
         if (e eq 1 and p mod 24 in {1,23}) or
            (e eq 1 and p mod 24 in {11,13} and p ne 13) or
            (e eq 2 and p mod 24 in {5,7,17,19}) then
            //Sl213b for p mod 24 in {11,13}, SL213b.2 ow
           _LRS := Read(c9lib cat "sl213d14b");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq : normaliser:=normaliser);
           if  p mod 24 in {1,23} then
             if all then asmax cat:= [S[1], S[1]^C, S[2], S[2]^C];
             else asmax cat:= [S[1], S[1]^C];
             end if;
           elif p mod 24 in {11,13} then asmax cat:= [S[1], S[1]^C];
           else 
             if all then asmax cat:= [S[1], S[1]^C, S[2], S[2]^C];
             else asmax cat:= [S[1], S[1]^C];
             end if;
         ...
         if q eq 4 or
              (e eq 1 and p mod 29 in {1,4,5,6,7,9,13,28,25,24,23,22,20,16}) or
              (e eq 2 and p mod 29 in {2,3,8,10,11,12,14,27,26,21,19,18,17,15}
                      and p ne 2) then
           //(S)L2_29
           _LRS := Read(c9lib cat "sl229d14");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq : normaliser:=normaliser);
           if all and q ne 4 then asmax cat:= [S[1], S[1]^C];
           else Append(~asmax, S[1]);
           end if;
         end if;
         if e eq 1 and (p mod 8 in {1,7} or p mod 40 in {3,13,27,37}) then
           //2J2.2 if p mod 8 in {1,7}, 2J2 ow
           _LRS := Read(c9lib cat "2j2d14");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq : normaliser:=normaliser);
           if all and  p mod 8 in {1,7} then asmax cat:= [S[1], S[1]^C];
           else Append(~asmax, S[1]);
           end if;
         end if;
         if p ne 2 then
           //2.L2q
           S := s6qsympdim14(q:normaliser:=normaliser);
           Append(~asmax, S);
         end if;
         if p ge 17 then
           //2.L2q
         ...
         if sign eq 1 then
           if novelties then
             if e eq 1 and
                q mod 156 in {1,25,49,61,121,133,43,55,79,103,127,139} then
               //L2_13b
               _LRS := Read(c9lib cat "l213d14b");
               _LR := eval _LRS;
               S := ReduceOverFiniteField(_LR,qq :
                   special:=special, general:=general, normaliser:=normaliser);
               if all and q mod 4 eq 1 then
             ...
             if q eq 4 then
               ...
               S := ReduceOverFiniteField(_LR,qq :
                   special:=special, general:=general, normaliser:=normaliser);
               //only two conjugatess, no four as in Anna's thesis
               if all then asmax cat:= [S[1], S[1]^CS];
               else Append(~asmax, S[1]);
               end if;
             end if;
           ...
           if e eq 1 and q mod 60 in {1,7,19,31,43,49,13,37} and p ne 7 then
              //A7 if q mod 60 in {13,37} A7.2 ow
             _LRS := Read(c9lib cat "a7d14a");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
                   special:=special, general:=general, normaliser:=normaliser);
             if all and q mod 60 in {1,49} then
               asmax cat:= [S[1], S[1]^CS, S[1]^CG, S[1]^(CS*CG),
                         S[1]^CN, S[1]^(CN*CS), S[1]^(CN*CG), S[1]^(CN*CG*CS)];
             elif all then
               asmax cat:= [S[1], S[1]^CG, S[1]^CN, S[1]^(CG*CN)];
             else Append(~asmax, S[1]);
             end if;
           end if; 
           if e eq 1 and q mod 60 in {1,17,19,23,31,47,49,53} then
              //A_15
             _LRS := Read(c9lib cat "a15d14");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
                   special:=special, general:=general, normaliser:=normaliser);
             if all and q mod 4 eq 1 then
               asmax cat:= [S[1], S[1]^CS, S[1]^CN, S[1]^(CN*CS)];
             elif all then
               asmax cat:= [S[1], S[1]^CN];
             else Append(~asmax, S[1]);
             end if;
           ...
           if q eq 2 then
              //A_16
             _LRS := Read(c9lib cat "a16d15");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
                   special:=special, general:=general, normaliser:=normaliser);
             Append(~asmax, S[1]);
           end if;
           if e eq 1 and q mod 156 in {1,5,11,25,41,43,47,49,55,59,61,71,79,83,
                    89,103,119,121,125,127,133,137,139,149} then
              //L2_13a
             _LRS := Read(c9lib cat "l213d14a");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
                   special:=special, general:=general, normaliser:=normaliser);
             if all and q mod 4 eq 1 then
           ...
           if (e eq 1 and q mod 156 in {1,47,59,71,83,119,41,89,125,137,149})
                                                                or q eq 2 then
              //L2_13b
             _LRS := Read(c9lib cat "l213d14b");
             _LR := eval _LRS;
           ...
           if e eq 1 and q mod 24 in {1,13,7,19} then
              //G23
             _LRS := Read(c9lib cat "g23d14");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
                   special:=special, general:=general, normaliser:=normaliser);
             if all and q mod 24 eq 1 then
               asmax cat:= [S[1], S[1]^CS, S[1]^CG, S[1]^(CS*CG),
                         S[1]^CN, S[1]^(CN*CS), S[1]^(CN*CG), S[1]^(CN*CG*CS)];
             elif all and q mod 24 eq 13 then
               asmax cat:= [S[1], S[1]^CG, S[1]^CN, S[1]^(CG*CN)];
             elif all then
               asmax cat:= [S[1], S[1]^CG];
             else Append(~asmax, S[1]);
           ...
           if (e eq 1 and p mod 60 in {1,19,31,49}) or
              (e eq 2 and p mod 60 in {7,13,17,23,37,43,47,53}) then
              //J2
             _LRS := Read(c9lib cat "j2d14");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
                   special:=special, general:=general, normaliser:=normaliser);
             if all and p mod 60 in {19,31} then
               asmax cat:= [S[1], S[1]^CG, S[1]^CN, S[1]^(CG*CN)];
             elif all then
               asmax cat:= [S[1], S[1]^CS, S[1]^CG, S[1]^(CS*CG),
                         S[1]^CN, S[1]^(CN*CS), S[1]^(CN*CG), S[1]^(CN*CG*CS)];
           ...
           if p eq 2 and IsEven(e) then
             //Omega^\pm(6,q)
             S := omplus6qdim14(q:normaliser:=normaliser);
             if all then
               asmax cat:= [S, S^CS];
             else Append(~asmax, S);
             end if;
             S := omminus6qdim14(q:normaliser:=normaliser);
             if all then
               asmax cat:= [S, S^CS];
             else Append(~asmax, S);
             end if;
           end if;
           if q mod 12 in {1,7} or (p eq 2 and IsEven(e)) then
             //S6q
             S := s6qsymporth14(q:
                   special:=special, general:=general, normaliser:=normaliser);
             if all and IsOdd(q) then
               asmax cat:= [S, S^CG, S^CN, S^(CG*CN)];
             elif all then asmax cat:= [S, S^CS];
             else Append(~asmax, S);
             end if;
           end if;
           if q mod 20 in {1,3,7,9} and q ne 3 then
             //Om5q
             S := om5qdim14(q:
                   special:=special, general:=general, normaliser:=normaliser);
             if all then
           ...
           if q mod 12 in {1,7} then
             //G2q
             S := g2qdim14(q: normaliser:=normaliser);
             if all and q mod 4 eq 1 then
               asmax cat:= [S, S^CS, S^CG, S^(CS*CG),
                         S^CN, S^(CN*CS), S^(CN*CG), S^(CN*CG*CS)];
             elif all then
               asmax cat:= [S, S^CG, S^CN, S^(CG*CN)];
             else Append(~asmax, S);
             end if;
           ...
           if novelties then
             if e eq 1 and
                 q mod 156 in {23,35,95,107,131,155,17,29,53,77,101,113} then
               //L2_13b
               _LRS := Read(c9lib cat "l213d14b");
               _LR := eval _LRS;
               S := ReduceOverFiniteField(_LR,qq :
           ...
           if (e eq 1 and p mod 60 in {11,17,29,41,53,59,23,47} and p ne 7) or
                                                                  q eq 4  then
              //A7 if q mod 60 in {23,47} A7.2 ow
             _LRS := Read(c9lib cat "a7d14a");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
                 special:=special, general:=general, normaliser:=normaliser);
             if all and q eq 4 then asmax cat:= [S[1], S[1]^CS];
             elif all and q mod 60 in {11,59} then
               asmax cat:= [S[1], S[1]^CS, S[1]^CG, S[1]^(CS*CG),
                         S[1]^CN, S[1]^(CN*CS), S[1]^(CN*CG), S[1]^(CN*CG*CS)];
             elif all then
               asmax cat:= [S[1], S[1]^CG, S[1]^CN, S[1]^(CG*CN)];
           ...
           if e eq 1 and q mod 60 in {7,11,13,29,37,41,43,59} then
              //A_15
             _LRS := Read(c9lib cat "a15d14");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
           ...
           if e eq 1 and q mod 156 in {7,17,19,23,29,31,35,37,53,67,73,77,85,
                               95,97,101,107,109,113,115,131,145,151,155} then
              //L2_13a
             _LRS := Read(c9lib cat "l213d14a");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
           ...
           if e eq 1 and
                  q mod 156 in {37,73,85,97,109,145,7,19,31,67,115,151} then
              //L2_13b
             _LRS := Read(c9lib cat "l213d14b");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
           ...
           if (e eq 1 and q mod 24 in {5,17,11,23}) or q eq 2 then
              //G23
             _LRS := Read(c9lib cat "g23d14");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
                   special:=special, general:=general, normaliser:=normaliser);
             if all and q mod 24 eq 23 then
               asmax cat:= [S[1], S[1]^CS, S[1]^CG, S[1]^(CS*CG),
                         S[1]^CN, S[1]^(CN*CS), S[1]^(CN*CG), S[1]^(CN*CG*CS)];
             elif all and q mod 24 eq 11 then
               asmax cat:= [S[1], S[1]^CG, S[1]^CN, S[1]^(CG*CN)];
             elif all and q ne 2 then
               asmax cat:= [S[1], S[1]^CG];
             elif all then
               asmax cat:= [S[1], S[1]^CS];
             else Append(~asmax, S[1]);
           ...
           if (e eq 1 and p mod 60 in {11,29,41,59}) or q eq 5 then
              //J2
             _LRS := Read(c9lib cat "j2d14");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
                   special:=special, general:=general, normaliser:=normaliser);
             if all and p mod 60 in {11,59} then
               asmax cat:= [S[1], S[1]^CS, S[1]^CG, S[1]^(CS*CG),
                         S[1]^CN, S[1]^(CN*CS), S[1]^(CN*CG), S[1]^(CN*CG*CS)];
             elif all then
               asmax cat:= [S[1], S[1]^CG, S[1]^CN, S[1]^(CG*CN)];
           ...
           if q mod 12 in {5,11} or (p eq 2 and IsOdd(e)) then
             //S6q
             S := s6qsymporth14(q:
                   special:=special, general:=general, normaliser:=normaliser);
             if all and IsOdd(q) then
           ...
           if q mod 20 in {11,13,17,19} then
             //Om5q
             S := om5qdim14(q:
                   special:=special, general:=general, normaliser:=normaliser);
             if all then
       ...
       if type eq "L" then
         if novelties then
           if e eq 1 and q mod 12  eq 7 then
             //3A7
             _LRS := Read(c9lib cat "3a7d15a");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq : general:=general);
             if all then nconj :=  GCD(q-1,15);
               asmax cat:= GConjugates(S[1],C,nconj);
             else Append(~asmax, S[1]);
             end if;
           end if;
           if p gt 2 then
             S := sl3qdim15b(q:general:=general);
             if all then nconj := GCD(q-1,5);
               asmax cat:= GConjugates(S,C,nconj);
             else Append(~asmax, S);
             end if;
         ...
         if e eq 1 and q mod 31 in {1,2,4,5,7,8,9,10,14,16,18,19,20,25,28} then
           //L2_31
           _LRS := Read(c9lib cat "l231d15");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq : general:=general);
           if all then nconj :=  GCD(q-1,15);
             asmax cat:= GConjugates(S[1],C,nconj);
         ...
         if e eq 1 and q mod 3 eq 1 then
           //3L34
           _LRS := Read(c9lib cat "3l34d15");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq : general:=general);
           if all then nconj :=  GCD(q-1,15);
             asmax cat:= GConjugates(S[1],C,nconj);
         ...
         if e eq 1 and q mod 3 eq 1 then
           //3U43 (not U33 as in Anna's thesis)
           _LRS := Read(c9lib cat "3au43d15");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq : general:=general);
           if all then nconj :=  GCD(q-1,15);
             asmax cat:= GConjugates(S[1],C,nconj);
         ...
         if q eq 3 then
           //M12
           _LRS := Read(c9lib cat "m12d16");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq : general:=general);
           Append(~asmax, S[1]);
         end if;
         if p gt 3 then
           S := sl3qdim15a(q:general:=general);
           if all then nconj := GCD(q-1,5);
             asmax cat:= GConjugates(S,C,nconj);
           else Append(~asmax, S);
           end if;
         ...
         if novelties then
           if e eq 1 and q mod 12  eq 5 then
             //3A7
             _LRS := Read(c9lib cat "3a7d15a");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq : general:=general);
           ...
           if p gt 2 then
             S := su3qdim15b(q:general:=general);
             if all then nconj := GCD(q+1,5);
               asmax cat:= GConjugates(S,C,nconj);
             else Append(~asmax, S);
             end if;
         ...
         if e eq 1 and
                 q mod 31 in {3,6,11,12,13,15,17,21,22,23,24,26,27,29,30} then
           //L2_31
           _LRS := Read(c9lib cat "l231d15");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq : general:=general);
         ...
         if e eq 1 and q mod 3 eq 2 and q ne 2 then
           //3L34
           _LRS := Read(c9lib cat "3l34d15");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq : general:=general);
         ...
         if p gt 3 then
           S := su3qdim15a(q:general:=general);
           if all then nconj := GCD(q+1,5);
             asmax cat:= GConjugates(S,C,nconj);
           else Append(~asmax, S);
           end if;
         ...
         if novelties then continue; end if;
         if e eq 1 and not q in {2,7} then
           //S7
           _LRS := Read(c9lib cat "a7d15");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
                    special:=special,general:=general,normaliser:=normaliser);
           if all then
         ...
         if e eq 1 and not q in {2,17} then
           //A16 (p = 3,5 mod 8), S16 ow
	 _LRS := Read(c9lib cat "a16d15");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
                    special:=special,general:=general,normaliser:=normaliser);
         ...
         if (e eq 1 and p mod 17 in {1,16}) or (e eq 2 and p mod 17 in {4,13})
             or (e eq 4 and p mod 17 in {2,8,9,15})
             or (e eq 8 and p mod 17 in {3,5,6,7,10,11,12,14}) then
            //L_2(16)
           _LRS := Read(c9lib cat "l216d15");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
                    special:=special,general:=general,normaliser:=normaliser);
           if all then
             asmax cat:= [S[1],S[1]^CS,S[2],S[2]^CS];
           else asmax cat:= [S[1],S[2]];
           end if;
         end if;
         if (e eq 1 and p mod 29 in {1,4,5,6,7,9,13,16,20,22,23,24,25,28}) or  
           (e eq 2 and p mod 29 in
                      {2,3,8,10,11,12,14,15,17,18,19,21,26,27} and p ne 2) then
           //L2_29
           _LRS := Read(c9lib cat "l229d15");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
                    special:=special,general:=general,normaliser:=normaliser);
           if all then
         ...
         if q eq 3 then
           //L34.2_2
           _LRS := Read(c9lib cat "l34d35");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
                    special:=special,general:=general,normaliser:=normaliser);
           S15 := [s: s in S | Dimension(s) eq 15][1];
           Append(~asmax, S15);
         end if;
         if e eq 1 and not q in {2,3} then
           //S62
           _LRS := Read(c9lib cat "s62d15");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
       ...
       if type eq "L" then
         ...
         if e eq 1 and p mod 11 in {1,3,4,5,9} and p ne 3 then
           //2.A_11
           _LRS := Read(c9lib cat "2a11d16");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq: general:=general);
           if all then nconj := GCD(16,q-1);
             asmax cat:= GConjugates(S[1],C,nconj);
           else Append(~asmax, S[1]);
           end if;
         end if;
         if e eq 1 and p mod 66 in {1,25,31,37,49,5,23,47,53,59} then
           //M_12
           _LRS := Read(c9lib cat "m12d16");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq: general:=general);
         ...
         if e eq 1 and IsSquare(GF(31)!q) and not q in {2,31} then
           //SL(2,31)
           _LRS := Read(c9lib cat "sl231d16");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq: general:=general);
           if all then nconj := GCD(16,q-1);
             asmax cat:= GConjugates(S[1],C,nconj);
         ...
         if e eq 1 and q mod 78 in {1,55,61,29,35,53} then
           //L33
           _LRS := Read(c9lib cat "l33d16");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq: general:=general);
           if all then nconj := GCD(16,q-1);
             asmax cat:= GConjugates(S[1],C,nconj);
         ...
         if p eq 3 then
           S := sl4qdim16(q:general:=general);
           if all then nconj := IsOdd(e) select 2 else 4;
             asmax cat:= GConjugates(S,C,nconj);
           else Append(~asmax, S);
           end if;
         end if;
         //L(4,q^2)
         S := l4q2dim16l(q:general:=general);
         if all then nconj := IsEven(q) select 1 else
                                                 q mod 4 eq 3 select 2 else 4;
           asmax cat:= GConjugates(S,C,nconj);
         else Append(~asmax, S);
         end if;
         //SpinPlus(10,q)
         S := HalfSpinPlus(10,q:Automorphisms:=true);
         w := PrimitiveElement(GF(q));
         scal := general select ScalarMatrix(16,w)
                 else ScalarMatrix(16, w^((q-1) div GCD(16,q-1)) );
         if IsEven(q) then
           S := sub< Generic(S) | S, scal >;
           Append(~asmax, S);
         else
           ng := Ngens(S);
           nconj := q mod 4 eq 3 select 2 else 4;
           if general then
             S := sub< Generic(S) | S, scal >;
           elif q mod 8 in {3,5,7} then
             assert Determinant(S.ng) eq 1;
             S := sub< Generic(S) | S, scal >;
           elif q mod 16 eq 9 then
             assert Determinant(S.ng) eq -1;
             S := sub< Generic(S) | [S.i : i in [1..ng-1]], S.ng^2, scal >;
           else
             S := sub< Generic(S) | [S.i : i in [1..ng-1]], scal >;
           end if;
           if all then asmax cat:= GConjugates(S,C,nconj);
           else Append(~asmax, S);
           end if;
         end if;
       elif type eq "U" then
         if novelties then
       /*
           // update: M_12.2 < HSpinMinus(10,2).gamma so not a novelty
           if q eq 2 then
             //M_12
             _LRS := Read(c9lib cat "m12d16");
             _LR := eval _LRS;
        ...
        */
         ...
         if q eq 7 then
           //4.M_22
           _LRS := Read(c9lib cat "4m22d16f49");
           _LR := eval _LRS;
           S := _LR`G;
           S := S^TransformForm(S);
           w := PrimitiveElement(GF(49));
           if normaliser then
             S := sub< Generic(S) | S.1, S.2, ScalarMatrix(16,w) >;
           else
             S := sub< Generic(S) | S.1, S.2, ScalarMatrix(16,w^6) >;
           end if;
           if all then nconj := 8;
             asmax cat:= GConjugates(S,C,nconj);
           else Append(~asmax, S);
         ...
         if e eq 1 and p mod 66 in {17,29,35,41,65,7,13,19,43,61} then
           //M_12
           _LRS := Read(c9lib cat "m12d16");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
         ...
         if e eq 1 and not IsSquare(GF(31)!q) and not q in {2,31} then
           //SL(2,31)
           _LRS := Read(c9lib cat "sl231d16");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
         ...
         if (e eq 1 and q mod 78 in {17,23,77,25,43,49}) or
            (e eq 2 and p mod 13 in {2,5,6,7,8,11}) then
           //L33
           _LRS := Read(c9lib cat "l33d16");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
           ...
           if all then nconj := GCD(16,q+1);
             asmax cat:= GConjugates(S[1],C,nconj);
             if e eq 1 then asmax cat:= GConjugates(S[2],C,nconj); end if;
           else
             Append(~asmax, S[1]);
             if e eq 1 then Append(~asmax, S[2]); end if;
           end if;
         end if;
         if p eq 3 then
           S := su4qdim16(q:general:=general);
           if all then nconj := GCD(16,q+1);
             asmax cat:= GConjugates(S,C,nconj);
           else Append(~asmax, S);
           end if;
         end if;
         S := l4q2dim16u(q:general:=general,normaliser:=normaliser);
         if all then nconj := IsEven(q) select 1 else
                                                 q mod 4 eq 1 select 2 else 4;
           asmax cat:= GConjugates(S,C,nconj);
         else Append(~asmax, S);
         end if;
         //SpinMinus(10,q)
         S := HalfSpinMinus(10,q:Automorphisms:=true);
         S := S^TransformForm(S);
         w := PrimitiveElement(GF(q^2)); z:=w^(q-1);
         scal := normaliser select ScalarMatrix(16,w)
                 else general select ScalarMatrix(16,z)
                 else ScalarMatrix(16, z^((q+1) div GCD(16,q+1)) );
         if IsEven(q) then
           S := sub< Generic(S) | S, scal >;
           Append(~asmax, S);
         else
           ng := Ngens(S);
           nconj := q mod 4 eq 1 select 2 else 4;
           if general then
             S := sub< Generic(S) | S, scal >;
           elif q mod 8 in {1,3,5} then
             assert Determinant(S.ng) eq 1;
             S := sub< Generic(S) | S, scal >;
           elif q mod 16 eq 7 then
             assert Determinant(S.ng) eq -1;
             S := sub< Generic(S) | [S.i : i in [1..ng-1]], S.ng^2, scal >;
           else
             S := sub< Generic(S) | [S.i : i in [1..ng-1]], scal >;
         ...
         if q eq 7 then
           //2A8
           _LRS := Read(c9lib cat "2a8d24");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:normaliser:=normaliser);
           S := [s: s in S | Degree(s) eq 16];
           Append(~asmax, S[1]);
         end if;
         if e eq 1 and q gt 3 and q ne 17 then
            //2L217a
           _LRS := Read(c9lib cat "sl217d16a");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:normaliser:=normaliser);
           if all and q mod 12 in {1,11} then asmax cat:= [S[1], S[1]^C];
           else Append(~asmax, S[1]);
           end if;
         end if;
         if (e eq 1 and p mod 36 in {1,35,17,19} and p ne 17) or
            (e eq 3 and p mod 36 in {5,7,11,13,23,25,29,31}) then
            //2L217b
           _LRS := Read(c9lib cat "sl217d16b");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:normaliser:=normaliser);
           if p mod 36 in {1,35} then
             if all then asmax cat:= [S[1], S[1]^C, S[2], S[2]^C, S[3], S[3]^C];
             else asmax cat:= [S[1], S[2], S[3]];
             end if;
           elif p mod 36 in {17,19} then
             asmax cat:= [S[1], S[2], S[3]];
           elif p mod 36 in {11,13,23,25} then
             if all then asmax cat:= [S[1], S[1]^C, S[2], S[2]^C, S[3], S[3]^C];
             else Append(~asmax, S[1]);
             end if;
           else //p mod 36 in {5,7,29,31}
             if all then asmax cat:= [S[1], S[2], S[3]];
             else Append(~asmax, S[1]);
             end if;
           end if;
         end if;
         if not p in {2,5} then
           //Sp(4,q)
           S := sp4qdim16(q:normaliser:=normaliser);
           Append(~asmax, S);
         end if;
         if p ge 17 then
           //2.L2q
         ...
         if sign eq 1 then
           if novelties then
             if e eq 1 and q mod 17 in {1,2,4,8,9,13,15,16} then
               //L2_17a
               _LRS := Read(c9lib cat "l217d16a");
               _LR := eval _LRS;
               S := ReduceOverFiniteField(_LR,qq :
                   special:=special, general:=general, normaliser:=normaliser);
               if all and p ne 2 then
           ...
           if e eq 1 and q mod 17 in {1,2,4,8,9,13,15,16} then
             ...
             S := ReduceOverFiniteField(_LR,qq :
                   special:=special, general:=general, normaliser:=normaliser);
             if all and q ne 2 then
               asmax cat:= [S[1], S[1]^CS, S[1]^CN, S[1]^(CN*CS)];
             else Append(~asmax, S[1]);
             end if;
           end if;
           if e eq 1 and not q in {2,5,11} then
              //2.A_10
             _LRS := Read(c9lib cat "2a10d16");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
                   special:=special, general:=general, normaliser:=normaliser);
             if all then
               if q mod 10 in {1,9} then
                 asmax cat:= [S[1], S[1]^CS, S[1]^CG, S[1]^(CS*CG),
                         S[1]^CN, S[1]^(CN*CS), S[1]^(CN*CG), S[1]^(CN*CG*CS)];
               else asmax cat:= [S[1], S[1]^CS, S[1]^CG, S[1]^(CS*CG)];
               end if;
             else Append(~asmax, S[1]);
             end if;
           end if;
           if p mod 17 in {1,2,4,8,9,13,15,16} and
                 ((e eq 1 and p mod 9 in {1,8}) or
                  (e eq 3 and p mod 9 in {2,4,5,7}))then
             //L2_17b
             _LRS := Read(c9lib cat "l217d16b");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
                 special:=special, general:=general, normaliser:=normaliser);
             if all and p eq 2 then
               asmax cat:= [S[1], S[1]^CS, S[1]^CN, S[1]^(CN*CS)];
             elif all then
               if p eq 2 then asmax cat:= [S[1], S[2], S[3]];
               else 
                 asmax cat:= [S[1], S[1]^CS, S[1]^CN, S[1]^(CN*CS)];
                 asmax cat:= [S[2], S[2]^CS, S[2]^CN, S[2]^(CN*CS)];
                 asmax cat:= [S[3], S[3]^CS, S[3]^CN, S[3]^(CN*CS)];
               end if;
             elif e eq 1 then
               asmax cat:= [S[1], S[2], S[3]];
             else Append(~asmax, S[1]);
           ...
           if q ne 2 then
             //L(2,q^4).4
             S := l2q4dim16(q: normaliser:=normaliser);
             if all then
               if IsOdd(q) then
                 asmax cat:= [S, S^CS, S^CG, S^(CS*CG)];
               else asmax cat:= [S, S^CS];
           ...
           if p ne 2 then
             //S(4,q^2).2
             S := s4q2dim16(q: normaliser:=normaliser);
             if all then asmax cat:= [S, S^CS, S^CG, S^(CS*CG)];
             else Append(~asmax, S);
             end if;
           end if;
           //Spin(9,q)
           S := SpinCircle(9,q : Automorphisms:=normaliser);
           if normaliser then
             w := PrimitiveElement(GF(q));
             S := sub< Generic(S) | S, ScalarMatrix(16,w) >;
           end if;
           if IsOdd(q) then asmax cat:= [S, S^CS, S^CG, S^(CS*CG)];
           else Append(~asmax, S);
           end if;
         else //sign eq -1
           if novelties then continue; end if;
           ...
           if e eq 1 and q mod 17 in {3,5,6,7,10,11,12,14} and q ne 3 then
              //A_17
             _LRS := Read(c9lib cat "a17d16");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
           ...
           if e eq 1 and q mod 17 in {3,5,6,7,10,11,12,14} then
             //L2_17a
             _LRS := Read(c9lib cat "l217d16a");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
           ...
           if p mod 17 in {3,5,6,7,10,11,12,14} and
                 ((e eq 1 and p mod 9 in {1,8}) or
                  (e eq 3 and p mod 9 in {2,4,5,7}))then
             //L2_17b
             _LRS := Read(c9lib cat "l217d16b");
             ...
             S := ReduceOverFiniteField(_LR,qq :
                 special:=special, general:=general, normaliser:=normaliser);
             if all then
               asmax cat:= [S[1], S[1]^CN, S[2], S[2]^CN, S[3],S[3]^CN];
             elif e eq 1 then asmax cat:= [S[1], S[2], S[3]];
             else Append(~asmax, S[1]);
             end if;
           end if;
         end if;
       ...
       //only type "O" arises
       if type eq "O" then
         if e eq 1 and not q in {2,3,19} then
           //S18
	 _LRS := Read(c9lib cat "a18d17");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
         ...
         if  ((e eq 1 and p mod 15 in {1,14}) or
             (e eq 2 and p mod 15 in {4,11}) or
             (e eq 4 and p mod 15 in {2,7,8,13})) and p ne 2 then
           //L_2(16)c
           _LRS := Read(c9lib cat "l216d17c");
           _LR := eval _LRS;
           S := ReduceOverFiniteField(_LR,qq:
                    special:=special,general:=general,normaliser:=normaliser);
           if all then asmax cat:= [S[1],S[1]^CS];
