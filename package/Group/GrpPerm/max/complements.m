freeze;
//edited by Derek Holt 30/11/18 to allow GrpMat as well as GrpPerm
...
//========================================================================================
intrinsic Complements(G::Grp, M::Grp : Presentation := 0) -> SeqEnum
{Given a finite permutation or matrix group G, with normal subgroup M,
...
the empty sequence is returned}
  ...
  if not ISA(Category(G), GrpPerm) and not ISA(Category(G), GrpMat) then
    error "First argument for Complements must be a perm- or mat-group";
  end if;
  if not ((Category(M) eq Category(G)) and IsNormal(G,M)) then
...
//========================================================================================
intrinsic HasComplement(G::Grp, M::Grp : Presentation := 0) -> BoolElt, GrpPerm
{Given a finite permutation or matrix group G, with normal subgroup M,
if M has a complement in G return true, otherwise false. A single complement
is also returned.}

  if not ISA(Category(G), GrpPerm) and not ISA(Category(G), GrpMat) then
    error "First argument for HasComplement must be a perm- or mat-group";
  end if;
  if not ((Category(M) eq Category(G)) and IsNormal(G,M)) then
    error "Second argument for HasComplement must be a normal subgroup of first";
  end if;
  require Presentation cmpeq 0 or Type(Presentation) eq GrpFP: "Presentation must be an FP-group";
  if not (Presentation cmpeq 0) and Ngens(G) ne Ngens(Presentation) then
    error "Presentation and G must have corresponding generators";
...
//========================================================================================
intrinsic RefineSection(G::Grp, M::Grp, N::Grp) -> SeqEnum
{Given a finite permutation or matrix group G, with normal subgroups N and M,
...
for all i, and L[#L] = M}
  ...
  if not ISA(Category(G), GrpPerm) and not ISA(Category(G), GrpMat) then
    error "First argument for RefineSection must be a perm- or mat-group";
  end if;
  if not ((Category(M) eq Category(G)) and IsNormal(G,M)) then
    error
       "Second argument for RefineSection must be a normal subgroup of G";
  end if;
  if not ((Category(N) eq Category(G)) and IsNormal(G,N)) then
    error
       "Third argument for RefineSection must be a normal subgroup of G";
  end if;
  if not N subset M or N eq M then
    error
     "Second argument for RefineSection must be a proper subgroup of third";
  end if;
...
intrinsic Complements(G::Grp, M::Grp, N::Grp: Presentation:=0) -> SeqEnum
{Given a finite permutation or matrix group G, with normal subgroups N and M,
such that N < M, if M/N has a complement in G/N return a list of
representatives of the conjugacy classes of complements in G/N. If M/N does
not have a complement in G/N, the empty sequence is returned}
  ...
  if not ((Category(M) eq Category(G)) and IsNormal(G,M)) then
    error
       "Second argument for Complements must be a normal subgroup of G";
  end if;
  if not ((Category(N) eq Category(G)) and IsNormal(G,N)) then
    error
       "Third argument for Complements must be a normal subgroup of G";
  end if;
  if not N subset M or N eq M then
    error
     "Second argument for Complements must be a proper subgroup of third";
  end if;
  require Presentation cmpeq 0 or Type(Presentation) eq GrpFP: "Presentation must be an FP-group";
  if not (Presentation cmpeq 0) and Ngens(G) ne Ngens(Presentation) then
    error "Presentation and G must have corresponding generators";
...
intrinsic HasComplement(G::Grp, M::Grp, N::Grp: Presentation := 0) -> SeqEnum
{Given a finite permutation group G, with normal subgroups N and M,
such that N < M, if M/N has a complement in G/N return true, otherwise false.
A single complement is also returned}
  ...
  if not ((Category(M) eq Category(G)) and IsNormal(G,M)) then
    error
       "Second argument for HasComplement must be a normal subgroup of G";
  end if;
  if not ((Category(N) eq Category(G)) and IsNormal(G,N)) then
    error
       "Third argument for HasComplement must be a normal subgroup of G";
  end if;
  if not N subset M or N eq M then
    error
     "Second argument for HasComplement must be a proper subgroup of third";
  end if;
  require Presentation cmpeq 0 or Type(Presentation) eq GrpFP: "Presentation must be an FP-group";
  if not (Presentation cmpeq 0) and Ngens(G) ne Ngens(Presentation) then
    error "Presentation and G must have corresponding generators";
  ...
  fl, C := section_complements(G, M, N: comps:=1, presentation := Presentation );
  if fl then
     return fl, C[1];
  else
     return fl, _;
...
intrinsic Supplements(G::Grp, M::Grp) -> SeqEnum
{Given a finite permutation or matrix group G, with soluble normal subgroup M,
if M has a supplement in G return a list of representatives of the conjugacy
classes of supplements in G. If M does not have a supplement in G, the empty
sequence is returned}

  if not ISA(Category(G), GrpPerm) and not ISA(Category(G), GrpMat) then
    error "First argument for Supplements must be a perm- or mat-group";
  end if;
  if not ((Category(M) eq Category(G)) and IsNormal(G,M)) then
    error "Second argument for Supplements must be a normal subgroup of first";
  end if;
...
//========================================================================================
intrinsic HasSupplement(G::Grp, M::Grp) -> BoolElt, GrpPerm
{Given a finite permutation or matrix group G, with soluble normal subgroup M,
if M has a supplement in G return true, otherwise false. A single
supplement is also returned.}

  if not ISA(Category(G), GrpPerm) and not ISA(Category(G), GrpMat) then
    error "First argument for HasSupplement must be a perm- or mat-group";
  end if;
  if not ((Category(M) eq Category(G)) and IsNormal(G,M)) then
    error "Second argument for HasSupplement must be a normal subgroup of first";
  end if;
...
//========================================================================================
intrinsic Supplements(G::Grp, M::Grp, N::Grp) -> SeqEnum
{Given a finite permutation or matrix group G, with normal subgroups N and M,
...
in G/N, the empty sequence is returned}
  ...
  if not ((Category(M) eq Category(G)) and IsNormal(G,M)) then
    error
       "Second argument for Supplements must be a normal subgroup of G";
  end if;
  if not ((Category(N) eq Category(G)) and IsNormal(G,N)) then
    error
       "Third argument for Supplements must be a normal subgroup of G";
  end if;
  if not N subset M or N eq M then
    error
     "Second argument for Supplements must be a proper subgroup of third";
  end if;
...
intrinsic HasSupplement(G::Grp, M::Grp, N::Grp) -> SeqEnum
{Given a finite permutation or matrix group G, with normal subgroups N and M,
such that N < M and M/N is soluble, if M/N has a supplement in G/N return true,
otherwise false.  A single supplement is also returned}
  ...
  if not ((Category(M) eq Category(G)) and IsNormal(G,M)) then
    error
       "Second argument for HasSupplement must be a normal subgroup of G";
  end if;
  if not ((Category(N) eq Category(G)) and IsNormal(G,N)) then
    error
       "Third argument for HasSupplement must be a normal subgroup of G";
  end if;
  if not N subset M or N eq M then
    error
     "Second argument for HasSupplement must be a proper subgroup of third";
  end if;
  
  fl, Supp := section_supplements(G, M, N: supps:=1 );
  if fl then
     return fl, Supp[1];
  else
     return fl, _;
