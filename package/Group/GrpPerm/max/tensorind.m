...
/* DFH: 24/6/16 also include generator of centre */
  scal := SL(d, q)!ScalarMatrix(d, z^((q-1) div GCD(q-1,d)) );
  return sub<SL(d, q)|gens, out, new_diag*(s^-1), scal>;
end function;
...
/* DFH: 24/6/16 also include generator of centre */
  scal := SL(d, q^2)!ScalarMatrix(d, z^((q^2-1) div GCD(q+1,d)) );
  grp:= sub<SL(d, q^2)|gens, out, new_diag*(s^-1), scal>;
