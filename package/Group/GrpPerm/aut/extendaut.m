...
/* Following patch added by DFH 1/10/16 to deal with case en GR`holgens has
 * repeated generators. I would not have expected that to happen, so there
 * there could be a problem somewhere.
 */
    for i in [2..nliftgens] do
      if (GR`holgens)[i] in { (GR`holgens)[j] : j in [1..i-1] } then
        for j in [i..nliftgens] do Prune(~GR`holgens); Prune(~GR`outimages);
        end for;
        nliftgens := i-1;
        break;
      end if;
