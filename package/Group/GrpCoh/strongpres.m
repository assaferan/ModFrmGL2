...
//get_mats := func<CM | [CM`get_mat(x): x in CM`mats]>;
get_mats := func<CM | CM`mats>; // for now
...
ZeroCohomologyGroupG := procedure(CM)
  ...
  X := #mats eq 0 select Matrix(K,d,d,[]) else
                   HorizontalJoin([x - 1: x in mats]);
//"csm X:", Parent(X);
  CM`csm := X;
  CM`Z0 := Nullspace(X);
//"csm NS dim:", Dimension(CM`Z0);
...
FirstCohomologyGroupG := procedure(CM)
  ...
  gm := get_mats(CM);
  MF := #gm eq 0 select GModule(CM`gf,[IdentityMatrix(CM`ring,CM`dim)])
                 else GModule(CM`gf, gm);
//"Get ComplementEquationsMatrix";
//SetVerbose("SparseMatrix",1);
//time
  CM`cem := ComplementEquationsMatrix(MF);
//"Get Nullspace of", Parent(CM`cem); "Dens:", Density(CM`cem); time
//"cem:", CM`cem;
  CM`Z1 := Nullspace(CM`cem);
//"NS Deg:", Degree(CM`Z1);
//"NS Dim:", Dimension(CM`Z1);
//"Get Im csm", Parent(CM`csm); "Dens:", Density(CM`csm); time
  CM`B1 := Image(CM`csm);
//"Im deg:", Degree(CM`B1);
//"Im dim:", Dimension(CM`B1);
...
function GetH1Dim(M, G, P, monomial, R, NZTest)
// G::GrpFP, P::GrpPerm, R::Rng: NZTest)
    ...
    */
	...
	ng := Ngens(G);
	d := Degree(P);
	assert Ngens(P) eq ng;
	A := 0;

	if monomial then
	    assert IsEven(d);
	    d div:= 2;
	    vprintf ModCoho: "H1Dim: monomial deg: %o, ngens: %o\n", d, ng;
	else
	    vprintf ModCoho: "H1Dim: perm deg: %o, ngens: %o\n", d, ng;
	end if;
    else
	R := BaseRing(M);
	/*
	...
	*/
	...
	vprintf ModCoho:
	    "H1Dim: general G-module dim: %o, ngens: %o\n", d, ng;
    end if;

    if A cmpne 0 then

	    //ker := KernelMatrix(&+[rand()*A.i - 1: i in [1 .. ng]]);

	rand_range := 2;
	rand := func< | Random({-rand_range .. rand_range} diff {0})>;
	vprint ModCoho: "Get first csm kernel"; vtime ModCoho:
	    ker := KernelMatrix(A.1 - 1);
	vprintf ModCoho: "First kernel dim: %o\n", Nrows(ker);

	vprint ModCoho: "Get full csm kernel"; vtime ModCoho:
	for i := 2 to ng do
	    if Nrows(ker) eq 0 then
		break;
	    end if;

	    pr := ker * (A.i - 1);
	    ker2 := KernelMatrix(pr);
	    if Nrows(ker2) lt Ncols(ker2) then
		ker := ker2 * ker;
	    end if;
	end for;

	csm_nullity := Nrows(ker);
	delete ker;
	csm_rank := d - csm_nullity;
	vprintf ModCoho: "Final csm nullity: %o, rank: %o\n",
	    csm_nullity, csm_rank;

    elif monomial then

	Z := IntegerRing();
	csm := VerticalJoin(
	    [Transpose(SparseMonomialMatrix(P.i) - IdentitySparseMatrix(Z, d)):
		i in [1 .. ng]]
	);
	csm := SparseMatrix(R, csm);

	vprint ModCoho: "Monomial csm:", csm; //Matrix(cem);
	vprint ModCoho: "Get csm rank"; vtime ModCoho:
	    csm_rank := Rank(csm);

	csm_nullity := Nrows(csm) - csm_rank;
	vprintf ModCoho: "csm rank: %o, nullity: %o, tr nullity: %o\n",
	    csm_rank, csm_nullity, Ncols(csm) - csm_rank;
    elif 1 eq 1 then
	csm_rank := d - #Orbits(P);
	vprintf ModCoho: "csm rank: %o\n", csm_rank;
    else
	vprint ModCoho: "Get csm"; vtime ModCoho:

	/*
	csm := HorizontalJoin(
	    [SparseMatrix(PermutationMatrix(R, P.i) - 1): i in [1 .. ng]]
	);
	*/
	csm := VerticalJoin(
	    [Transpose(SparseMatrix(PermutationMatrix(R, P.i) - 1)):
	    i in [1 .. ng]]
	);

	vprint ModCoho: "csm:", csm; //Matrix(cem);
	vprint ModCoho: "Get csm rank"; vtime ModCoho:
	    csm_rank := Rank(csm);
	...
	vprintf ModCoho: "csm rank: %o, nullity: %o, tr nullity: %o\n",
	    csm_rank, csm_nullity, Ncols(csm) - csm_rank;
	vprintf ModCoho: "#P Orbits: %o\n", #Orbits(P);

	assert csm_rank eq d - #Orbits(P);
    end if;

    vprint ModCoho: "Get ComplementEquationsMatrix"; vtime ModCoho:
    if M cmpeq 0 then
	cem := ComplementEquationsMatrix(G, P, R: Monomial := monomial);
    else
	cem := ComplementEquationsMatrix(M);
    end if;

//"Type cem:", Type(cem);
//"First cem:", Nrows(cem), Ncols(cem);

    if 0 eq 1 then
	"DO OLD";
	M := PermutationModule(P, R);
	M := GModule(G, [ActionGenerator(M, i): i in [1 .. ng]]: Check:=false);
	time cem0 := ComplementEquationsMatrix(M);
	//"OLD cem0 density:", Density(cem0);
	assert cem0 eq Matrix(cem);
    end if;

    if NZTest then
	vprint ModCoho: "Get block ranks";

	nr := #Relations(G);
	ranks := [];
	vtime ModCoho: for i := 1 to nr do
	    pos := (i - 1)*d + 1;
	    S := ColumnSubmatrix(cem, pos, d);
	    //S;
	    Sr := Rank(S);
	    Append(~ranks, Sr);
	end for;
	rs := &+ranks;
	vprint ModCoho: "Ranks:", ranks;
	vprintf ModCoho: "Rank sum: %o (nullity %o)\n",
	    rs, Min(Nrows(cem), Ncols(cem)) - rs;
	f := d - csm_rank;
	rhs := f + (ng - 1)*d;
	pass := rs lt rhs;
	vprintf ModCoho: "f: %o, ng: %o, d: %o, rhs: %o, PASS: %o\n",
	    f, ng, d, rhs, pass;

	if NZTest and pass then
	    return 1;
	end if;

	vprint ModCoho: "FAIL rank test";
    end if;

//Parent(cem); "cem:", cem; Matrix(cem);
    //cem := SparseMatrix(cem);
    func := Nrows;
    cem := Transpose(cem); func := Ncols;
    vprint ModCoho: "Density:", Density(cem);

//SetVerbose("SparseMatrix",1);

vprint ModCoho: "Get cem rank", Nrows(cem), Ncols(cem), Type(cem);

    if 1 eq 1 and Type(R) in {RngInt, FldRat} and M cmpne 0 then
	if 1 eq 1 then
	    cem := IntegralMatrix(cem);
	    K := GF(RandomPrime(23));
	    vprint ModCoho: "Get modular rank";
	    vtime ModCoho: cem_rank := Rank(Matrix(K, cem));
	    if func(cem) - cem_rank eq csm_rank then
		vprint ModCoho: "Proven trivial by modular rank";
	    else
		vprintf ModCoho: "Probably non-trivial (modular rank %o)\n",
		    cem_rank;
		vprint ModCoho: "Get Z rank";
		vtime ModCoho: cem_rank := Rank(cem);
	    end if;
	else
	    //"Use Classical Herm";
	    //cem := HermiteForm(cem: Al := "Classical");
	    vtime ModCoho: cem := EchRec(cem);
	    RemoveZeroRows(~cem);
	    cem_rank := Nrows(cem);
	end if;
    else
    ...
    cem_nullity := func(cem) - cem_rank;
vprintf ModCoho: "cem rank: %o, nullity: %o\n", cem_rank, cem_nullity;

    res := cem_nullity - csm_rank;
    assert res ge 0;

    vprintf ModCoho: "Final H1 dim: %o\n", res;
    return res;
end function;

/*
intrinsic H1Dimension(G::GrpFP, P::GrpPerm, R::Rng: NZTest := false) -> RngIntElt
{Dimension of H^1(G, PermutationModule(P, R))}
    return GetH1Dim(0, G, P, false, R, NZTest);
end intrinsic;
*/

intrinsic H1Dimension(G::GrpFP, phi::Map, R::Rng) -> RngIntElt
{Dimension of H^1(G, PermutationModule(P, R)),
where P is defined by images of generators of G under phi}
    ims := [G.i@phi : i in [1 .. Ngens(G)]];
    P := sub<Generic(Image(phi)) | ims>;
    return GetH1Dim(0, G, P, false, R, false);
end intrinsic;

intrinsic H1DimensionExteriorSquare(
    G::GrpFP, P::GrpPerm, R::Rng: NZTest := false
) -> RngIntElt
{Dimension of H^1(G, ExteriorSquare(PermutationModule(P, R)))}

    d := Degree(P);
    O := {@ [i, j]: i in [1 .. j - 1], j in [1 .. d] @};
    Md := #O;
    MS := Sym(2*Md);
    Mgens := [];
    for k := 1 to Ngens(P) do
	g := P.k;
	im := [];
	for o := 1 to Md do
	    p := O[o];
    ...
    MP := sub<MS | Mgens>;
//"MP:", MP;
    return GetH1Dim(0, G, MP, true, R, NZTest);
end intrinsic;

intrinsic H1DimensionExteriorSquare(G::GrpFP, phi::Map, R::Rng: NZTest := false) -> RngIntElt
{Dimension of H^1(G, ExteriorSquare(PermutationModule(P, R))),
where P is defined by images of generators of G under phi}
    ims := [G.i@phi : i in [1 .. Ngens(G)]];
    P := sub<Generic(Image(phi)) | ims>;
    return H1DimensionExteriorSquare(G, P, R: NZTest := NZTest);
end intrinsic;

intrinsic H1DimensionSymmetricSquare(
    G::GrpFP, P::GrpPerm, R::Rng: NZTest := false
) -> RngIntElt
{Dimension of H^1(G, SymmetricSquare(PermutationModule(P, R)))}

    d := Degree(P);
    //O := {@ {i, j}: i in [1 .. j - 1], j in [1 .. d] @};
    O := {@ {i, j}: i in [1 .. j - 0], j in [1 .. d] @};
    Md := #O;
    MS := Sym(Md);
    Mgens := [];
    for k := 1 to Ngens(P) do
    ...
    MP := sub<MS | Mgens>;
//"MP:", MP;
    return GetH1Dim(0, G, MP, false, R, NZTest);
end intrinsic;

intrinsic H1DimensionSymmetricSquare(G::GrpFP, phi::Map, R::Rng: NZTest := false) -> RngIntElt
{Dimension of H^1(G, SymmetricSquare(PermutationModule(P, R))),
where P is defined by images of generators of G under phi}
    ims := [G.i@phi : i in [1 .. Ngens(G)]];
    P := sub<Generic(Image(phi)) | ims>;
    return H1DimensionSymmetricSquare(G, P, R: NZTest := NZTest);
end intrinsic;

intrinsic HasNonZeroH1(G::GrpFP, P::GrpPerm, R::Rng) -> BoolElt
{Return whether H^1(G, PermutationModule(P, R)) is non-zero}
    return H1Dimension(G, P, R: NZTest) gt 0;
end intrinsic;

intrinsic HasNonZeroH1(G::GrpFP, phi::Map, R::Rng) -> BoolElt
{Return whether H^1(G, PermutationModule(P, R)) is non-zero,
where P is defined by images of generators of G under phi}
    ims := [G.i@phi : i in [1 .. Ngens(G)]];
    P := sub<Generic(Image(phi)) | ims>;
    return HasNonZeroH1(G, P, R);
end intrinsic;

//intrinsic H1Dimension(M::ModGrp: NZTest := false) -> RngIntElt
//{Dimension of H^1(Group(M), M)}
function H1Dimension_Mod(M: NZTest := false)
    return GetH1Dim(M, 0, 0, false, 0, NZTest);
//end intrinsic;
end function;

intrinsic H1Dimension(F::GrpFP, phi::Map, HM::ModGrp) -> RngIntElt
{Dimension of the first cohomology group of the module FM, where 
where F is a finitely presented group and phi is an epimorphism 
from F onto a finite group H and HM is a H-module that has been
lifted to FM} 
    rho := Representation(HM);
    mats := [F.i@phi@rho : i in [1 .. Ngens(F)]];
    FM := GModule(F, mats: Check := false);
    return H1Dimension_Mod(FM);
end intrinsic;
 ...
 */
  ...
  tb := Cputime();
  vprint ModCoho: "Done. Time to set up matrix:",tb-ta;
  ...
  // need its nullspace.
  vprint ModCoho, 2: "  Sparcity of matrix is:",
   RealField(3)!(#[x: x in Eltseq(X) | x ne 0]/(Nrows(X)*Ncols(X)));
  ns := Nullspace(X);
  ta := Cputime();
  vprint ModCoho: "Got Nullspace. Time for nullspace:",ta-tb;
  V1 := RSpace(K,Nrows(X));
  ...
  tb := Cputime();
  vprint ModCoho: "Calculated H2. Time since nullspace computation:", tb-ta;
