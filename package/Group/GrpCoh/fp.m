 ...
 */ 
  ...
  ng := #gens;
// "M:", M; "IsPermutationModule(M):", IsPermutationModule(M); "ng:", ng;
  if false and IsPermutationModule(M) then
    n := Dimension(M);
    perms := [];
    for i := 1 to ng do
	S := SparseMatrix(ActionGenerator(M, i));
	e := [Min(Support(S, i)): i in [1 .. n]];
	p := Sym(n)!e;
	Append(~perms, p);
    end for;
    mats := perms;
    get_mat := func<X | PermutationMatrix(BaseRing(M), X)>;
  else
      mats := [ R(gens[i]) : i in [1..ng] ];
      //"dens:", [Density(x): x in mats];
      get_mat := func<X | X>;
  end if;
//"Get invs"; time
  imats := [ m^-1 : m in mats ];
