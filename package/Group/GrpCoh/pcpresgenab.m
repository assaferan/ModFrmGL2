 ...
 */
  ...
  PtoF := function(w) //map word in P to corresponding word in F
   local s;
   if ng eq 0 then return F.0; end if;
   s:=ElementToSequence(w); return &*[F.i^s[i]:i in [1..ng]];
  end function;
  ...
  r`relpos := relpos;
  r`gf := quo<F|rels>;
  r`gftoG := ng eq 0 select hom< r`gf->P | [P.0] >
             else hom< r`gf->P | [P.i : i in [1..ng]] >;

  if ng ne 0 and not CheckRelationsA(r`gf,invar,mats:imats:=imats) then
 ...
 */
  ...
  if k eq 0 or Ncols(X) eq 0 then
  ...
  //for now, we lift the result back up the integers, since quotients of
  //spaces over finite integer rings are not well behave.
  if ng eq 0 then CM`B2 := CM`Z2;
  elif (invar[d] ne 0) then
