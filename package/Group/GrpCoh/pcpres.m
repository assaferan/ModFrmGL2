...
OldExtensionOfModulePCP := function(CM,seq)
/* old version using pQuotient - no longer used
 ...
 */
 local Z, P, p, mats, imats, ng, z2v, w, rels, rel, phi, H2, e, E;
  ...
  for i in [1..d] do for j in [i+1..d] do
    Append(~rels,(F.(ng+j),F.(ng+i)) = Id(F));
  end for; end for;
  ...
  for j in [2..ng] do for i in [1..j-1] do
    ...
    Append(~rels,F.i^-1*F.j*F.i = PtoF(P.i^-1*P.j*P.i)*w);
    colno +:= d;
  end for; end for;
  E := quo<F|rels>;
  return E;
...
OldStabilityTest2 := function(MR,g)
/* old version using pQuotient - no longer used
 ...
 */
  ...
  for h2genno in [1..Ngens(H2)] do
    ...
    E := OldExtensionOfModulePCP(MR`modcoho,seq);
    PtoE := hom<P->E | [E.i : i in [1..ng]]>; 
    PE, EtoPE := pQuotient(E,p,127:Print:=1);
 ...
 */
  ...
  for h2genno in [1..Ngens(H2)] do
    seq := [0 : i in [1..Ngens(H2)]]; seq[h2genno]:=1;
    PE :=  ExtensionOfModulePCP(MR`modcoho,seq);
    //PtoPE := hom<P->PE | [PE.i : i in [1..ng]]>; 
    PtoPE := map<P->PE | x :-> PE!(Eltseq(x) cat [0: i in [1..d]]) >;
    //PE, EtoPE := pQuotient(E,p,127);
    PEM := sub < PE | [PE.(ng+i) : i in [1..d]]>;
    //PtoPE := Ptopcp*PtoE * EtoPE;
    PtoPE := Ptopcp*PtoPE;
    // We need a matrix to convert an element of PEM from the
    // Magma representation to an element in the generators of our M.
    // To do this we multiply the magma word by cmat
    cmat := Matrix(K,d,d,
                 [ElementToSequence(PEM!(PE.(ng+i))) : i in [1..d]])^-1;
 ...
 */
  ...
  for l in [1..#S-1] do
    ...
    vprint ModCoho,1: #dcosreps, "double coset representatives";
