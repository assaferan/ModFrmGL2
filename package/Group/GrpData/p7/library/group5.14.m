...
Group5_14 := function (p: Process:=true, Select:=func<x|true>, Limit := 0, Single := 0)
...
list_len := 3*p+22;
if Single gt SBase + list_len then
    SBase +:= list_len;
else
mats:=[];
//get pc,pb
...
if A eq 0 then
  count:=count+1;
  if Single eq 0 or count eq Single - SBase then
  GR:=Group<a,b,c|(c,b),(c,a,a),(c,a,b),(c,a,c),a^p,b^p,c^p>;
  ProcessPresentation (~L, GR, p, class, ~Nmr: Process := Process, Select := Select);
  end if;
  if (limited and Nmr ge Limit) or (Single gt 0 and count eq Single - SBase)
  then
...
list_len := 5*p+13+Gcd(p-1,3)+Gcd(p-1,4);
if Single gt SBase + list_len then
    SBase +:= list_len;
else
mats:=[];
...
list_len := 6*p+8+2*Gcd(p-1,3)+Gcd(p-1,4)+Gcd(p-1,5);
if Single gt SBase + list_len then
    SBase +:= list_len;
else
mats:=[];
...
list_len := 5*p+13+2*Gcd(p-1,3)+Gcd(p-1,4);
if Single gt SBase + list_len then
    SBase +:= list_len;
else
//first get pc,pb
...
list_len := p^2+3*p-3+(p+2)*Gcd(p-1,3)+(p+1)*Gcd(p-1,4)+(p+1)*Gcd(p-1,5);
if Single gt SBase + list_len then
    SBase +:= list_len;
else
//get pc
...
list_len := 2*p^2+11*p+43+Gcd(p-1,4);
if Single gt SBase + list_len then
    SBase +:= list_len;
else
count:=0;
...
list_len := p^3+4*p^2+6*p+(p+5)*Gcd(p-1,3)+3*Gcd(p-1,4)+Gcd(p-1,5);
if Single gt SBase + list_len then
    SBase +:= list_len;
else
count:=0;
...
list_len := 2*(p^3+5*p^2/2+7*p+19/2+(p+4)*Gcd(p-1,4)/2);
if Single gt SBase + list_len then
    SBase +:= list_len;
else
...
list_len := 2*((p^4+p^3+4*p^2+p-1+(p^2+2*p+3)*Gcd(p-1,3)+(p+2)*Gcd(p-1,4))/2);
if Single gt SBase + list_len then
    SBase +:= list_len;
else
...
list_len := 2*p^2+11*p+27+Gcd(p-1,4);
if Single gt SBase + list_len then
    SBase +:= list_len;
else
...
list_len := p^3+2*p^2+6*p+10+(p+4)*Gcd(p-1,3);
if Single gt SBase + list_len then
    SBase +:= list_len;
else
count:=0;
...
list_len := p^3+(7*p^2+17*p+59+5*Gcd(p-1,3)+(p+1)*Gcd(p-1,4))/2;
if Single gt SBase + list_len then
    SBase +:= list_len;
else
count:=0;
...
list_len := 2*p^4+4*p^3+8*p^2+14*p+11+4*Gcd(p-1,3)+3*Gcd(p-1,4);
if Single gt SBase + list_len then
    SBase +:= list_len;
else
count:=0;
...
list_len :=
p mod 3 eq 1 select (p^5+p^4+p^3+p^2+7*p+10)/3+(p^2+p+1)*Gcd(p-1,4)/2
else p^5+p^4+p^3+p^2+p+2+(p^2+p+1)*Gcd(p-1,4)/2;
if Single gt SBase + list_len then
    SBase +:= list_len;
else
count:=0;
...
list_len := (2*p^5+2*p^4+2*p^3+2*p^2+14*p+17)/3;
if Single gt SBase + list_len then
    SBase +:= list_len;
else
...
list_len := 2*p^4+4*p^3+6*p^2+11*p+11+2*Gcd(p-1,3)+(p+1)*Gcd(p-1,4);
if Single gt SBase + list_len then
    SBase +:= list_len;
else
count:=0;
...
list_len := 2*p^3+6*p^2+7*p+7+(p+1)*Gcd(p-1,4);
if Single gt SBase + list_len then
    SBase +:= list_len;
else
count:=0;
...
list_len := (2*p^3+3*p^2+3*p+13-Gcd(p-1,3)+(p+1)*Gcd(p-1,4))/2;
if Single gt SBase + list_len then
    SBase +:= list_len;
else
...
*/
list_len :=
p mod 3 eq 1 select p^5+p^4+p^3+p^2+p+2+(p^2+p+1)*Gcd(p-1,4)/2 else
p^5/3+p^4/3+p^3/3+p^2/3+p+2+(p^2+p+1)*Gcd(p-1,4)/2;
...
list_len := 2*(p^5+p^4+p^3+p^2)/3+2*p+3;
if Single gt SBase + list_len then
    SBase +:= list_len;
else
val:={};
