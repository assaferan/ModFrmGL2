 ...
 *    Date      : $Date:: 2016-10-27 00:24:18 +1100 (Thu, 27 Oct 2016)       $:
 *    Last edit : $Author:: eobr007                                          $:
 *
 *    $Id:: classical-bb.m 3240 2016-10-26 13:24:18Z eobr007               $:
 *
 *****************************************************************************/
...
function CsabaWrapper(G, center, scalar, gens, type, d, q, g)
    flag, slp := ClassicalRewrite(G, gens, type, d, q, g);

    if IsTrivial(sub<Generic(center) | center, scalar>) and not flag
       or slp cmpeq false then
	return false, _;
    else
        return true, slp;
    end if;
...
function GeneralSLPMaps(node, ExtendedGroup, stdGens,
    ...
    iso := hom<ExtendedGroup -> node`LeafData`GoldCopy | g :->
	 Evaluate(MembershipTesting(node, func<h |
		CsabaWrapper(ExtendedGroup, center, node`Scalar, niceInputGens,
			     type, d, q, h)>, g), gens)>;
...
function RecogniseClassicalSmallDegree(G, S, d, q, type)
    ...
    flag, H := RecogniseSmallDegree(G, type, d, q: Hard := false, Verify := false);
