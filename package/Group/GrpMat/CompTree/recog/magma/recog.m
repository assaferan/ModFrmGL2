 ...
 *    Date      : $Date:: 2017-05-19 06:33:48 +1000 (Fri, 19 May 2017)       $:
 *    Last edit : $Author:: eobr007                                          $:
 *
 *    $Id:: recog.m 3251 2017-05-18 20:33:48Z eobr007                      $:
 *
 *****************************************************************************/
...
forward RecogniseGroup, FixCompTree, IsCompTree, Flatten, GetTreeLeaves,
    DisplaySubTreeNodes, DisplayNode, DecomposeGroup, ToRootNice, NameChange;
...
intrinsic CleanCompositionTree(G :: Grp)
    { Remove all data related to composition tree from G and also LMG data. }
...
intrinsic CompositionTreeFactorNumber(G :: Grp, x :: GrpElt) -> RngIntElt
    ...
    if Type (G) eq GrpMat then 
       ...
       assert IsCentral (G, g) and IsDivisibleBy(Order(tree[1]`Scalar), Order(g));
    end if;
    return 1;
end intrinsic;
...
intrinsic CompositionTreeSeries(G :: Grp) ->
    ...
    if Ngens (G) eq 0 then return [], [], [], [], [], [], []; end if;
...
intrinsic CompositionTreeCompositionFactors(G :: Grp) -> SeqEnum
    {compute composition factors without calling CompositionTreeSeries}
    ...
    for i in [1 .. #leaves] do
        grp := leaves[i]`Group;
	if Category(grp) eq GrpMat then
	  S := leaves[i]`LeafData`GoldCopy;
          if Category(S) eq GrpMat then
            if assigned leaves[i]`LeafData`Name then
              nametryct := 0;
              repeat nametryct +:= 1;
                issimp, name :=
                  SimpleGroupName(S : OmegaFactor:=100);
              until issimp or nametryct eq 5;
              if not issimp then
                error "Failed to show that composition factor is simple";
              end if;
              Append(~series, NameChange(name[1])); 
            end if;
          else series cat:= CompositionFactors(S);
          end if;
	
	  C := C9Centre(leaves[i]`NiceData`Group : NmrTries := 1000);
          Cord := FactoredOrder(C);
	  // Divide out by scalar flag
	  Cord := Cord / GCD(Cord, Factorisation(Order(leaves[i]`Scalar)));
          for f in Cord do
            for i in [1..f[2]] do Append(~series, <19, 0, f[1]>); end for;
          end for;
        else
          if Order(grp) gt 1 and Order(leaves[i]`Scalar) gt 1 then
            grp := quo< grp | leaves[i]`Scalar >;
          end if;
          series cat:= CompositionFactors(grp);
        end if;
    end for;

    return SequenceToCompositionFactors(series);
end intrinsic;
...
function RecogniseGroup(G :
	...
	vprintf CompositionTree, 1 : "Error of type %o during composition tree creation\n", err`Type;
	vprintf CompositionTree, 1 : "Full error info:\n%o\n", err;
