 ...
 *    Date      : $Date:: 2017-09-18 05:34:33 +1000 (Mon, 18 Sep 2017)       $:
 *    Last edit : $Author:: jbaa004                                          $:
 *
 *    $Id:: c9.m 3291 2017-09-17 19:34:33Z jbaa004                         $:
 *
 *****************************************************************************/
...
import "spin.m" : IsSpinGroup, SpinImage, SpinImageOfElement;
...
function IsElementInCoset(G, N, cosetReps, g : order := CentralOrder,
    NumberRandom := NormalSubgroupElements, Randomiser := RandomProcess(N))
    if exists(k){i : i in [1 .. #cosetReps] |
	     ElementInNormalSubgroup(G, N, cosetReps[i] / g : order := order,
...
function ConstructCosets(G, N : order := CentralOrder,
	...
	for i in [1 .. NumberOfGenerators(G)] do
	    ...
	    if not IsElementInCoset(G, N, cosetReps, g :
...
function CosetAction(G, N, reps, images, g : order := CentralOrder,
		     NumberRandom := NormalSubgroupElements,
		     Randomiser := RandomProcess(N))
    flag, coset := IsElementInCoset(G, N, reps, g : order := order,
				    NumberRandom := NumberRandom,
...
function ConstructOuterAutomorphisms(node, kernel, Randomiser, MaxCosets)
    ...
    // Construct actions of coset representatives
    repImages := [AlmostSimpleImage(node`Group, kernel, cosetReps, g :
			Randomiser := Randomiser, AllCosets := safe) :
		  g in cosetReps];

    vprintf CompositionTree, 3 :
	    "Coset reps image sequence found, depth %o\n", node`Depth;
	    
    return sub<Sym(#cosetReps) | genImages>, genImages,
	   cosetReps, repImages, safe;
end function;

function SpinGroupCheck(node, name, data)
    if name[1] cmpeq "B" then
	type := <"Omega", 2 * name[2] + 1, name[3]>;
    elif name[1] cmpeq "D" then
	type := <"Omega+", 2 * name[2], name[3]>;
    elif name[1] cmpeq "2D" then
	type := <"Omega-", 2 * name[2], name[3]>;
    else
	return false, _;
    end if;
    ...
    derivative := data[1];
    return IsSpinGroup(derivative, type[1], type[2], type[3]), type;
end function;

procedure SpinGroupHom(~node, type)
    vprintf CompositionTree, 3 :
	    "Group is a spin group, setup Spin image, depth %o\n", node`Depth;

    flag, H, data := SpinImage(node`Group, type[1], type[2], type[3]);
    assert flag;

    // Construct reduction
    reduction := hom<node`Group -> H | g :-> SpinImageOfElement(H, g, data)>;
    ...
    node`ActionMaps := rec<ActionMapsInfo |
		...
		FromKernelBatch := func<seq | seq>,
    	        BatchTest := func<seq | true>>;
	    
    // We already know the image
    node`Image`Group := H;
    node`Image`InputGens := [Function(reduction)(g) :
			     g in UserGenerators(node`Group)];
    // TODO Go directly to C8 checks in image?
    
    node`CBM := func<node | Identity(node`Group)>;
    // Always use presentation kernel in this case, since it will be fast.
    node`PresentationKernel := true;
...
procedure OuterAutHom(~node, image, kernel, genImages, cosetReps, repImages,
		      FoundAllCosets, Randomiser, leafData)
    vprintf CompositionTree, 3 :
	    "Group is not simple, setup outer automorphism image, "
	    cat "depth %o\n", node`Depth;

    // Construct reduction
    reduction := hom<node`Group -> image | g :->
		CosetAction(node`Group, kernel, cosetReps, repImages, g :
		Randomiser := Randomiser)>;
    reductionScalar := func<g | Function(reduction)(g),
				Identity(node`Group), Identity(node`Group)>;
	    
    // Test if element lies in some coset rep of the simple group
    test := func<g | IsElementInCoset(node`Group, kernel, cosetReps, g :
				      Randomiser := Randomiser)>;
...
procedure AlmostSimpleHom(~node, data)
	...
	isSpin, type := SpinGroupCheck(node, name, data[2]);
	if isSpin then
	    isNotSimple := true;
	else
	...
	if isNotSimple then
	    if isSpin then
		SpinGroupHom(~node, type);
	    else
		image, genImages, cosetReps, repImages, FoundAllCosets :=
			ConstructOuterAutomorphisms(node, kernel, Randomiser,
						    MaxCosets);
		if not IsTrivial(image) then
		    OuterAutHom(~node, image, kernel, genImages, cosetReps,
				repImages, FoundAllCosets,
				Randomiser, leafData);
		else
		    isNotSimple := false;
		end if;
