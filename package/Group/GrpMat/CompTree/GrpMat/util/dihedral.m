...
MyAreInvolutionsConjugate :=  function (G, x, wx, y, wy: Central := false, 
    ...
    if Central or Type(x) ne GrpMatElt then
	use_vec := false;
	fct := CentralOrder;
    else
	use_vec := true;
//"vec ord";
    ...
    R := Randomiser;
    i := 0;
//"MyAreInvolutionsConjugate"; time
    repeat
	p, slp_p := Random(R);
	...
	if use_vec then
	    ...
	    if IsEven(o) then
		r := 0;
//assert 'mod'(fct(prod), 2) eq 0;
	    else
		q, r := Quotrem(fct(prod), 2);
	    end if;
	else
	    q, r := Quotrem(fct(prod), 2);
	end if;
    ...
    if i ge MaxTries then
//printf "Fail after %o tries\n", i;
        return false, _, _; 
    else 
//printf "Odd order 2*%o+1 = %o found after %o tries\n", q, 2*q+r, i;
