 ...
 *    Date      : $Date:: 2016-07-10 07:22:42 +1000 (Sun, 10 Jul 2016)       $:
 *    Last edit : $Author:: eobr007                                          $:
 *
 *    $Id:: closure.m 3223 2016-07-09 21:22:42Z eobr007                    $:
 *
 *****************************************************************************/
...
function NormalClosureEngine(G, H, slpsG, slpsH, iterations)
	...
	u, slp_u := RandomSubProduct(gens, slps);
        ...
        // avoid identity + repetitions 
        c := u^g;
        if c ne c^0 and not (c in gens) then 
           Append(~gens, c);
...
function NormalClosureGeneration(G, H, slpsG, slpsH :
    SubgroupChainLength := 10, ErrorProb := 2^(-100), MaxGenerators := 50)
	...
	Minimum (SubgroupChainLength * eltsPerLevel, MaxGenerators));
end function;
