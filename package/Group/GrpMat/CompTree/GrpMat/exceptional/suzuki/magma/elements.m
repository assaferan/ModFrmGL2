 ...
 *    Date      : $Date:: 2017-08-27 10:08:16 +1000 (Sun, 27 Aug 2017)       $:
 *    Last edit : $Author:: jbaa004                                          $:
 *
 *    $Id:: elements.m 3274 2017-08-27 00:08:16Z jbaa004                   $:
 *
 *****************************************************************************/
...
intrinsic SzClassMap(G :: GrpMat) -> Map
	...
	stdCopyRepresentative(G`SuzukiReductionData`stdCopy,
			      G`SuzukiReductionData`iso(x),
			      WordGroup(G`SuzukiReductionData`stdCopy)))>;
end intrinsic;
...
function stdCopyRepresentative(G, g, wordGroup)
    ...
    if o eq 2 then
	assert WordGroup(G`RandomProcess) eq wordGroup;
	...
	if P.1 eq Vector(F, [0, 0, 0, 1]) then
	    ...
	    conj1 := getZeroMappingMatrix(G, P.1);
	end if;
	rep := g^conj1;
