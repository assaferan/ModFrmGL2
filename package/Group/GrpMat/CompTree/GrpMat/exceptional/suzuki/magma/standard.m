 ...
 *    Date      : $Date:: 2017-08-27 10:07:32 +1000 (Sun, 27 Aug 2017)       $:
 *    Last edit : $Author:: jbaa004                                          $:
 *
 *    $Id:: standard.m 3269 2017-08-27 00:07:32Z jbaa004                   $:
 *
 *****************************************************************************/
...
// Return lower triangular matrix (hence fixing the infinity point) in
// standard Sz that maps P (which must be in the standard ovoid)
// to the zero point
function getInfMappingMatrix(G, P)
    assert SuzukiStandardRecogniser(G);
    assert isOvoidPoint(P);
    V := VectorSpace(G);
    assert Normalise(P) ne V ! [1, 0, 0, 0];
    if not IsZero(P[4]) then
	F := CoefficientRing(G);
	// Standardise point
	P *:= P[4]^(-1);
	a := P[3];
	b := P[2];
	return getSuzukiSylowGen(F, a, b);
    else
	return Identity(G);
    end if;
end function;

// Return upper triangular matrix (hence fixing the zero point) in
// standard Sz that maps P (which must be in the standard ovoid)
// to the infinity point
function getZeroMappingMatrix(G, P)
    assert SuzukiStandardRecogniser(G);
    assert isOvoidPoint(P);
    V := VectorSpace(G);
    assert Normalise(P) ne V ! [0, 0, 0, 1];
	...
	a := P[3];
	b := P[2];
	return Transpose(getSuzukiSylowGen(F, b, a + b^(t + 1)));
    else
	return Identity(G);
    end if;
end function;
