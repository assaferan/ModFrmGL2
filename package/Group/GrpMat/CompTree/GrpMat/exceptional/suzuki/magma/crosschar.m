 ...
 *    Date      : $Date:: 2017-08-27 10:05:54 +1000 (Sun, 27 Aug 2017)       $:
 *    Last edit : $Author:: jbaa004                                          $:
 *
 *    $Id:: crosschar.m 3266 2017-08-27 00:05:54Z jbaa004                  $:
 *
 *****************************************************************************/
...
intrinsic SuzukiSmallFieldReduction(G :: GrpMat : CheckInput :=
    ...
    sz`RandomProcess := RandomProcessWithWords(sz : WordGroup := WordGroup(sz));
    assert SuzukiStandardRecogniser(sz);
    iso := hom<G -> sz | x :-> Function(homo)(x)>;
    inv := hom<sz -> G | x :-> Evaluate(slp, gens)
    where slp is SuzukiNaturalConstructiveMembership(H, x :
