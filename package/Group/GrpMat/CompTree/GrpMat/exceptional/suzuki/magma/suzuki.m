 ...
 *    Date      : $Date:: 2017-08-27 10:08:43 +1000 (Sun, 27 Aug 2017)       $:
 *    Last edit : $Author:: jbaa004                                          $:
 *
 *    $Id:: suzuki.m 3277 2017-08-27 00:08:43Z jbaa004                     $:
 *
 *****************************************************************************/
...
import "standard.m" : diagramAutomorphism, getSuzukiOrder, getZeroMappingMatrix;
import "membership.m" : getSuzukiRecognitionData;
...
intrinsic SuzukiReduction(G :: GrpMat : CheckInput := true, FieldSize := 0,
	...
	if Degree(sz) gt 4 then
	    ...
	    sz`RandomProcess := RandomProcessWithWords(
			sz : WordGroup := WordGroup(sz));
	else
    ...
    conjHomo, conjugator := SuzukiConjugacy(
	    sz : Randomiser := sz`RandomProcess, CheckInput := false);
    ...
    _ := SuzukiConjugacy(sz);    
    assert SuzukiStandardRecogniser(sz); 
    assert assigned sz`RandomProcess;
    assert WordGroup(sz`RandomProcess) eq WordGroup(sz);
    ...
    G`SuzukiReductionData`slpCoercion :=
	hom<sz`SuzukiRecognitionData`slpGroup -> W | UserGenerators(W)>;
    G`SuzukiReductionData`slpGroup := W;
    ...
    inv := hom<sz -> G | x :-> flag eq true select
	G`SuzukiReductionData`slpHomo(G`SuzukiReductionData`slpCoercion(slp))
	else flag where flag, slp is SuzukiNaturalConstructiveMembership(sz, x :
...
intrinsic SuzukiConstructiveMembership(G :: GrpMat, g :: GrpMatElt :
	...
	sz, element : CheckInput := false, Randomiser := sz`RandomProcess);
    if flag then
	return true, G`SuzukiReductionData`slpCoercion(slp);
    else
	return false, _;
    end if;
end intrinsic;
...
intrinsic SuzukiStabiliser(G :: GrpMat, P :: ModTupFldElt  :
    ...
    conj := getZeroMappingMatrix(G`SuzukiReductionData`stdCopy,
				 P * G`SuzukiReductionData`conjugator);
    S := Sz(field);
    stab := [<Function(G`SuzukiReductionData`slpHomoInv)(
		g^(conj^(-1) * G`SuzukiReductionData`conjugator^(-1))),
	      g^(conj^(-1) * G`SuzukiReductionData`conjugator^(-1))>
	     : g in [S.3, S.2]];
    return stab, 0;
end intrinsic;
...
intrinsic SuzukiPointStabiliser(G :: GrpMat : CheckInput := true,
    ...
    element1 := stabElts[1]`mat;
    w1 := stabElts[1]`slp;
    element2 := stabElts[2]`mat;
    w2 := stabElts[2]`slp;
