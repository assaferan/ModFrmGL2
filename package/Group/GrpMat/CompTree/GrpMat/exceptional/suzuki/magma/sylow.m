 ...
 *    Date      : $Date:: 2017-08-27 10:08:08 +1000 (Sun, 27 Aug 2017)       $:
 *    Last edit : $Author:: jbaa004                                          $:
 *
 *    $Id:: sylow.m 3273 2017-08-27 00:08:08Z jbaa004                      $:
 *
 *****************************************************************************/
...
import "standard.m" : getInfMappingMatrix, getZeroMappingMatrix,
       diagramAutomorphism, getSuzukiOrder, isOvoidPoint;
...
intrinsic SuzukiSylow(G :: GrpMat, p :: RngIntElt) -> GrpMat, SeqEnum
	...
	slps := [G`SuzukiReductionData`slpCoercion(slp) where
		 _, slp is SuzukiNaturalConstructiveMembership(S, g`mat :
			Randomiser := S`RandomProcess, CheckInput := false)
			: g in S`SuzukiRecognitionData`subDiagElts];
	GS := sub<Generic(G) | Evaluate(slps, UserGenerators(G))>;
	AssertAttribute(GS, "Order", q^2);
	return GS, slps;
    else
	...
	slp := G`SuzukiReductionData`slpCoercion(W ! slp^divisor);
	// Return Sylow subgroup
	GS := sub<Generic(G) | G`SuzukiReductionData`slpHomo(slp)>;
	assert Order(GS.1 : Proof := false) eq order;
	
	AssertAttribute(GS, "Order", order);
	return GS, [slp];
    end if;
end intrinsic;
...
function cyclicSylowConjugation(H, RH, SH)
	...
	// Find two ovoid points preserved by subgroup
	...
	b := getZeroMappingMatrix(H, points[2] * a);
	Append(~conjs, a * b);
    end for;
