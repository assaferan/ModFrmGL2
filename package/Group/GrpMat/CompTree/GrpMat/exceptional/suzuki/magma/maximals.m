 ...
 *    Date      : $Date:: 2017-08-27 10:07:49 +1000 (Sun, 27 Aug 2017)       $:
 *    Last edit : $Author:: jbaa004                                          $:
 *
 *    $Id:: maximals.m 3271 2017-08-27 00:07:49Z jbaa004                   $:
 *
 *****************************************************************************/
...
intrinsic SuzukiMaximalSubgroups(G :: GrpMat) -> [], []
    ...
    assert WordGroup(G`SuzukiReductionData`stdCopy) eq
	   Domain(G`SuzukiReductionData`slpCoercion);
    slps := [G`SuzukiReductionData`slpCoercion(gens) : gens in 
		G`SuzukiReductionData`stdCopy`SuzukiMaximals`slps];
...
intrinsic SuzukiStandardMaximalSubgroups(G :: GrpMat) -> [], []
    ...
    AssertAttribute(pointStab, "Order", q^2 * (q - 1));
    
    torusNormaliser := sub<Generic(G) | gens[1], gens[2]>;
    AssertAttribute(torusNormaliser, "Order", 2 * (q - 1));
	...
	AssertAttribute(sz, "Order", getSuzukiOrder(p^deg));
	...
	Append(~slps, [Evaluate(slp, UserGenerators(W)) where _, slp is
	    SuzukiNaturalConstructiveMembership(G, g : CheckInput := false,
...
intrinsic SuzukiMaximalSubgroupsConjugacy(G :: GrpMat, R :: GrpMat,
    ...
    slp := G`SuzukiReductionData`slpCoercion(slp);
...
function smallerSzConjugation(H, MR, MS)
    ...
    flag, slp := SuzukiNaturalConstructiveMembership(H, conjugator :
