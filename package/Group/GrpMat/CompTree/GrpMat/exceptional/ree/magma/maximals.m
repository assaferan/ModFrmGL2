 ...
 *    Date      : $Date:: 2017-08-27 10:06:22 +1000 (Sun, 27 Aug 2017)       $:
 *    Last edit : $Author:: jbaa004                                          $:
 *
 *    $Id:: maximals.m 3268 2017-08-27 00:06:22Z jbaa004                   $:
 *
 *****************************************************************************/
...
intrinsic ReeStandardMaximalSubgroups(G :: GrpMat :
    ...
    AssertAttribute(pointStab, "Order", q^3 * (q - 1));

    involCentraliser := sub<Generic(G) | gens[1], gens[2],
	getReeInfSylowGenMatrix(F, 0, 1, 0)>;
    AssertAttribute(involCentraliser, "Order", q * (q^2 - 1));
	...
	AssertAttribute(ree, "Order", getReeOrder(p^deg));
	...
	repeat
		...
		for sol in sols do
			...
			if o eq (q + 1) div 2 then
			    AssertAttribute(maximal, "Order", 12 * o);
			else
			    AssertAttribute(maximal, "Order", 6 * o);
			end if;
