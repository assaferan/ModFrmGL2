...
import "Code/Maximal-Subgroups/O8p2.m":DataO8p2;
...
import "Code/Maximal-Subgroups/U62.m":DataU62;
...
import "Code/Chains/chains.m":SubgroupChainsO8p2;
...
import "Code/Chains/chains.m":SubgroupChainsU62;
...
//      when "M": L := SubgroupChainsM ();
      ...
      when "O8p2": L := SubgroupChainsO8p2 ();
...
{if standard generators supplied or found for sporadic group G having 
 ...
 possibly modulo centre of G, and base points are correspondingly adjusted}
   ...
   if str eq "U62" then
      B := GoodBasePoints (G);
      H := G; H`Base := B;
      f := RandomSchreierBounded (H, 2^24);
      if f then delete H; return true, B; end if;
   end if;
