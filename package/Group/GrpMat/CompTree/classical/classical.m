 ...
 *    Date      : $Date:: 2016-11-10 06:51:43 +1100 (Thu, 10 Nov 2016)       $:
 *    Last edit : $Author:: eobr007                                          $:
 *
 *    $Id:: classical.m 3246 2016-11-09 19:51:43Z eobr007                  $:
 *
 *****************************************************************************/

import "recognition/black/odd/split.m": ConvertType;
import "omega3.m": RecogniseOmega3vsSL2 ;
...
function LargeToSmall(G, E, X, type, d, F, g, CB)
    ...
    if not f then return false; else return Evaluate(w, X)^CB; end if;
end function;

function SmallToLarge(S, E, X, type, d, F, g, CB)
    f, w := ClassicalRewrite(S, X, type, d, #F, g^(CB^-1));
    if f then return Evaluate(w, E); else return false; end if;
end function;

function LargeToWordGroup(G, E, W, type, d, F, g)
    f, w := ClassicalRewrite (G, E, type, d, #F, g);
    if f then return Evaluate(w, W); else return false; end if;
end function;

// we used CompTree to recognise G 
// must use CompTree structure to avoid black rewriting 

CompTreeMaps := function (G, S, d, F, E, W, K, stdGens, type, CB)
   ...
   phi := toFactors[#toFactors];
   tau := fromFactors[#toFactors];
   ...
   if type in {"Omega+", "Omega-", "Omega"} then K := Generic (K); end if;    
   phi := hom<Generic (G) -> K | x :-> LargeToSmall(G, x)^CB>;
   // phi := hom<Generic (G) -> K | x :-> LargeToSmall(G, x)>;
   tau := hom<K -> Generic (G) | x :-> SmallToLarge(S, x^(CB^-1))>;
...
SetupMaps := function (G, type, d, F, E, W)
   ...
   // Gold copy of SL2 found via CompTree is Omega3 
   if <type, d> eq <"SL", 2> and assigned G`O3Data then
      data := G`O3Data;
      delete G`O3Data;
...
// "PARENT of tr is ", Parent (tr);
// "PARENT of tr2 is ", Parent (tr2);

   // conjugate standard generators into domain D of goldToW[m]
   CB := Identity(S);
...
//      CB := tr * tr2^-1;
   ...
   if type eq "SU" and d eq 3 and #F eq 2 then 
       ...
       gamma := rec`g2slp;
       delta := rec`slp2g; 
   elif (Type (G) eq GrpMat and HasCompositionTree (G)) then
       // Characteristic (BaseRing (G)) eq Characteristic (F) and 
       // type eq "SL" and d eq 2 and HasCompositionTree (G)) then
       vprint ClassicalRecognition, 1: "Use Composition tree to set up maps";
        // CB here should be identity ...? 
       phi, tau, gamma, delta := CompTreeMaps (G, S, d, F, E, W, K, stdGens, 
					       type, CB);
   else 
...
ReduceToSmallDegree := function (H, type, d, q: Limit := 5)
   ...
   flag, K := RecogniseSmallDegree (H, type, d, q: Hard := false, Verify := false);
   if not flag then 
...
ConstructNatural := function (H, type, d, q: Limit := 2)
   ...
   if not natural and not ext then 
...
CTClassicalSolve := function (G, type, d, q: UseCT := false)
   ...
   // EOB October 2016 use CompTree more widely to reduce cost of maps 
   // particularly in dealing with small-rank classical groups constructed 
   // as subgroups of exceptional groups 
   if UseCT eq false then 
      // UseCT := (Type (G) eq GrpMat and (IsAbsolutelyIrreducible (G) eq false and 
        ...
        //       (d in {2} and q gt 9 and type in {"SL"}))) 
      UseCT := (Type (G) eq GrpMat and (IsAbsolutelyIrreducible (G) eq false 
           and <d, type> ne <2, "SU"> and <d, type> ne <3, "SL"> and q ge 16))
               or (MyIsBaseCase (type, d, q) eq false and q eq 2);
   end if;
   ...
   // We may recognise SL2 as Omega3 -- it occurs often 
   if <type, d> eq <"SL", 2> and Degree (D) eq 3 then
      "Special code to handle Omega (3, q)";
      flag, alpha, beta, gamma, delta, E,  W := RecogniseOmega3vsSL2 (G, q);
      G`O3Data := <alpha, beta, gamma, delta>;
      return true, E, W;
   end if;

   if q gt 2 and (Degree (D) ne d) then
      CleanCompositionTree (G);
      vprint ClassicalRecognition, 1: "*** AAA Bad gold copy";
      flag, E, W := Internal_ClassicalSolve (G, type, d, q);
      if flag then return true, E, W; else return false, _, _; end if;
   end if;
...
// "PARENT of tr2 is ", Parent (tr2);
   ...
   if #Set (Evaluate (R, E)) ne 1 then 
      CleanCompositionTree (G);
      vprint ClassicalRecognition, 1: "*** BBB Bad gold copy";
      flag, E, W := Internal_ClassicalSolve (G, type, d, q);
      if flag then return true, E, W; else return false, _, _; end if;
   end if;
...
function ClassicalElementToWordNatural(G, g)
    ...
    vprint ClassicalRecognition, 8 : "AAA Obtain SLP of element";
...
function ClassicalConstructiveMembershipEngine(G, g, type,
   ...
   if natural then 
    ...
    if natural then
       ...
       // EOB call function with G, not typeStr, for efficiency reasons  
       // flag, slp := ClassicalRewriteNatural (typeStr, cbm^-1, g);
       flag, slp := ClassicalRewriteNatural (G, cbm^-1, g);
    else 
