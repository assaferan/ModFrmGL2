...
import "basics.m": getExtensionDegree, ChooseSingerElt, fixSigns, 
ScaleMatrix2, FindScalarMultipleOverSubfield, EmbedInCorrectField, 
...
FindEigenspaces := function (s, ev, ones, G, TypeString)
   ...
   // indices of the orbits of length d (candidates for the orbit containing ell_11)
...
FindEigenspacesSOPlus := function (s, ev, ones, G, TypeString)
   ...
   for i in [1..e-1] do
      for j in [i+1..e] do
         ...
         if (val eq 1) then 
            onescount +:= 1;
            // `leave out' 1-dimensional eigenspace since L(d-1)(d) is coming up
