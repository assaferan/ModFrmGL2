...
import "basics.m": getExtensionDegree, ChooseSingerElt, ScaleMatrix2, 
fixSigns, FindScalarMultipleOverSubfield, EmbedInCorrectField, 
...
Formula_viii := function (T, i, j, k, defs)
...
FindEigenspacesSOOdd := function (s, ev, ones, G, TypeString)
   ...
   e := getExtensionDegree (d, TypeString);
   E := GF(q^e);
   // the orbits of length d (candidates for the orbit containing ell_11)
...
FindEigenspacesSOPlus := function (s, ev, ones, G, TypeString)
   ...
   eigs := &join(ev);
   index := [i: i in [1..#ev] | #ev[i] eq e]; 
   // the indices of the orbits of length d 
...
DetermineConstantsSL := function (G, CB, CBinv, defs, TypeString, NumberRandom)
   ...
   for count in [1..NumberRandom] do
      ...
      for j in [2..((e+1) div 2)] do
         ...
         jj := IndexPosition (defs,j,j);
         ell := 2*j-1;
         int := Formula_viii (T, i, j, ell, defs);
...
// Run the Reconstruct function: if it fails, try again with a random element 
...
RecogniseSymmetricSquare := function (GG, TypeString, q: NumberRandom := 100)
   ...
   for count in [1..NumberRandom] do
      ...
      if found eq false then
         vprint SmallDegree: "Could not find a special element: Representation is probably not symmetric square of" , TypeString;
         return false, _;
      else
