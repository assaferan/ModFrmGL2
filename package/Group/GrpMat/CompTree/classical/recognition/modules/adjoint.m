...
EmbedInCorrectField, FindScalarMultipleOverSubfield, fixSigns, 
...
FindEigenspacesSL := function (s, ev, ones, G, TypeString) 
   ...
   for o in [1..#ev] do 
      ...
      // construct the change of basis matrix CB and its inverse.
      fine := true;
      try
         MA := MatrixAlgebra (E, n); 
         l := Eltseq (L);
         ...
         ES := [Eigenspace (s, e).1: e in l] cat [OneEigenspace.i : i in [1..ones]];
...
FindEigenspacesSUEven := function (s, ev, ones, G, TypeString) 
   ...
   vprint SmallDegree: "FindEigenspacesSUEven failed";
   return false, _,_,_;
end function;

/* construct the matrix of constants C */
DetermineConstants := function (G, CB, CBinv, TypeString, s) 
   flag, d, def := DetermineDegree (Degree (G));
   ...
   M := MatrixAlgebra (E, n);
   repeat
       vprint SmallDegree: "Choose random element and write in new basis...";
       h := Random (G);
       K := CB * M!h * CBinv;
   ...
   for kkk in [0..MaxNumber] do
      ...
      for j in [4..dprime] do 
         C[1][j] := (C[1][j-1])^(q+1)* A11A22 * K[FindIndex (2,j-1,d)][FindIndex (2,j,d)]/
...
/* Run Reconstruct: if division by zero is attempted then work around with a random element */
...
RecogniseAdjoint := function (GG, TypeString, q: NumberRandom := 100) 
   ...
   for count in [1..NumberRandom] do
      ...
      found, C := DetermineConstants (G, CB, CBinv, TypeString, s);
      if found eq false then continue count; end if;
   ...
   if not assigned A then return false, _; end if;
...
/* find the preimage of g */
AdjointPreimage := function (G, g, limit: NumberRandom := 100)
   R := G`SmallDegree; 
   CB := R[2]; CBinv := R[3]; C := R[4]; 
   SCB := R[5]; SCBinv := R[6]; TypeString := R[7]; A := R[10]; form := R[8];
   q := Isqrt (R[9]); d := Degree (A);
   F := BaseRing (A); deg := Degree (F) div 2;

   for count in [1..limit] do
      flag, im := TryReconstruct (G, g, C, CB, CBinv, TypeString, NumberRandom, count eq 1);
      if not flag then return false, _; end if;
      if Determinant (im) eq 0 then continue count; end if;
      ...
      im := Generic (A)!im;
      flag, im := ElementFixesForm (A, F, TypeString, form, im);
      if flag then return true, im; end if;
   end for;
