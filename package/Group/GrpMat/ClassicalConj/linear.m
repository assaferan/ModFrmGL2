...
/*
  $Id: linear.m 56619 2018-03-15 05:53:52Z don $
...
{Internal function: The conjugacy classes of the extension of SL}
  if not (ISA(Type(BaseRing(G)),FldFin) and IsLinearGroup(G)) then
    return false,_,_;
  end if;

  m := LCM( [ Integers() | Order(Determinant(g)) : g in Generators(G) ] );
