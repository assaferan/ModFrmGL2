...
/*
  The form should be symmetric, g is an element of the orthogonal group 
  preserving the form.  
  
  Even q uses the paper by Dye "A geometric characterisation of the special
  orthogonal groups and the Dickson invariant".  
  
  Odd q uses the fact that the spinor norm is the determinant of the
  Wall form, modulo squares.  Similar code of Mark Stather from 2005
  (see ClassicalSylow.m) follows Zassenhaus "On the spinor norm".

  The intrinsic returns 0 if g is in Omega, 1 if g is in SO \ Omega.
*/
...
{The spinor norm of the element in the special orthogonal group fixing 
 the specified symmetric bilinear form}
  ...
  newmat := m1*form*Transpose(A)*Transpose(m1); // The Wall form

  // The following line ensures that the spinor norm of the reflection
  // in the hyperplane orthogonal to u is u*form*Transpose(u), in
  // accordance with Zassenhaus.
