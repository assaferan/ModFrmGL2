...
/* incorrect list for C_4 case for odd characteristic */

Original_PrimitiveListI := function (q) 
...
/* revised list for C_4 case for odd characteristic */
...
PrimitiveListI := function (q) 
   ...
   for x in Z do 
      ...
      if q mod 8 eq 1 then 
         ...
         if #K eq 0 then 
            E := ext<F | 4>;
            K := AllRoots (E!x[1][1], 4);
         end if;
         if exists(kappa){k : k in K | k in F} then 
            L cat:= [sub <gl | kappa * v, c, u, x>];
         end if;
         if exists(kappa){k : k in K | k^2 in F} then 
            L cat:= [sub <gl | kappa^2 * v, c, u, x>];
         end if;
      end if;
   end for;
