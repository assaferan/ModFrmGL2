...
// make into SL
  ...
  _,z1 := NormEquation(GF(q),GF(p)!(z^-1));
  g1 := GL(2,q)!ScalarMatrix(2,z1)*g1;
  //Derek Holt 10/1/17, make determinant of g1 1.
  _,z := IsSquare(Determinant(g1));
  assert z^(p+1) eq GF(q)!1;
  g1 := GL(2,q)!ScalarMatrix(2,z^-1)*g1;
  assert Determinant(g1) eq GF(q)!1;
