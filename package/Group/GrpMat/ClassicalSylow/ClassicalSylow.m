...
Eg.
  ...
  Derek Holt corrected some bugs in ClassicalSylow January 2017.
  ClassicalSylowConjugation still has some problems, particularly with
  orthogonal groups in even characteristic.
*/
 ...
 */
  ...
  if not RecognizeClassical(G:NumberOfElements:=500) then
  ...
  if Dimension(G) eq 2 and type eq "symplectic" then //don't want symplectic
     //DFH 10/1/17 edited next line to deal correctly with SU(2,q)
      type := UnitaryForm(G) select "unitary" else "linear";
  end if;
  q := #BaseRing(G);
  gens := Generators(G);
  if type eq "linear" then
    ...
    //Derek Holt 10/1/17 what about GO?
    if GF(q)!(-1) in {Determinant(g) : g in gens} then
       return "GO";
    elif forall{g : g in gens | SpinorNorm(g, cfs`bilinearForm) eq 0 } then
       return "Omega";
    else return "SO";
    end if;
...
// Uses Lemma 3.1 of M.I.M. AlAli 
  if type cmpeq "linear" then
    ...
    g:=EmbedMat(x,2,q,m);
    o:=pPart(q^m+1,r);
    //h:=g^IntegerRing()!((q^m+1)/r^o); Derek Holt 9/1/17
    h:=g^((q^m+1) div (r^o));
    ...
    //x:=X.2; Derek Holt 9/1/17 We want an element of order q^m+1
    RP:=RandomProcess(X); repeat x:=Random(RP); until Order(x) eq q^m+1;
    L:=GF(q^m); K:=GF(q);
    V:=VectorSpace(L,2); W:=VectorSpace(K,n);
    B:=Basis(L,K);
    phi:=function(w)
...
// Am using Kantor - Sylow's thm in poly time Prop 15.2
    x:=GL(1,q^n)![PrimitiveElement(GF(q^n))];
    x:=EmbedMat(x,1,q,n);
    //p:=IntegerRing()!(q^(n/2)); Derek Holt 9/1/17
    _,rq:=IsSquare(q);
    p:=rq^n;
    x:=x^(p-1);
    ...
    o:=pPart(p+1,r);
    //g:=x^IntegerRing()!((p+1)/r^o); Derek Holt 9/1/17
    g:=x^((p+1) div (r^o));
...
// Applies the field auto to the matrix x or module x or vector x
  ...
  if Type(x) eq ModGrpElt or Type(x) eq ModTupFldElt then
    ...
    P:=Parent(x);
    v:=Eltseq(x);
    //p:=Truncate(SquareRoot(q)); Derek Holt 10/1/17
    _,p := IsSquare(q);
...
// Returns a sylow p subgroup of GO-(2*n,2^l) 
    ...
    //for i in [1..Truncate(n/2)] do Derek Holt 10/1/17
...
// Check orders
  ...
  case type2:
    ...
    when "orth-":
      ...
      if IsEven(q) then Psi:=BilinearToQuadratic(G,Psi); end if;
    when "unitary":
      //r:=Truncate(SquareRoot(q)); Derek Holt 10/1/17
       _,r := IsSquare(q);
