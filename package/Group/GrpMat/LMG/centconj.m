...
ORIG_PCOSG := function(G, f, pt)
/* Polycyclic orbit-stabiliser calculation.
...
PCOSG := function(G, fg, fv, pt, ofact)
/* Polycyclic orbit-stabiliser calculation.
 * f is a group-action function S x G -> S, with G of type GrpPC, and pt in S.
 * Return orbit of pt, stabiliser of pt as subgroup of G, and
 * generator numbers in G that were used to build up orbit.
...
/*
orb,st,on := ORIG_PCOSG(G, ofact, pt);
"ORIG orb:", orb;
"ORIG st:", st;
"ORIG orbnos:", on;
*/
...
//f := ofact;
//printf "PCOSG: #PC = %o\n", #G;
//"pt:", pt; "Par pt:", Parent(pt);
  ng := NPCgens(G);
  ords := PCPrimes(G);
  Gim := [fg(G.i): i in [1 .. ng]];
  orb := {@ pt @};
  stabgens := [G|];
  orbnos := [];
//time
  for i in [ng .. 1 by -1] do
    ...
    Gi := Gim[i];
    im := fv(pt, Gi);
//"NEW G.i:", G.i; "NEW fg(G.i):", fg(G.i);
    //im := fv(pt, fg(G.i));
//assert im eq f(<pt , G.i>);
    pos := Position(orb,im);
    if pos eq 0 then
	//orb join:= {@ fv(o, fg(G.i^k)) : o in orb, k in [1..ords[i]-1] @};
//oorb := orb;
	s := #orb;
	offset := 0;
	for k := 1 to ords[i] - 1 do
	    for j in [1 .. s] do
		Include(~orb, fv(orb[offset + j], Gi));
	    end for;
	    offset +:= s;
	    assert #orb eq (1 + k)*s;
	end for;
//assert orb eq oorb join {@ fv(o, fg(G.i^k)) : o in oorb, k in [1..ords[i]-1] @};
        Append(~orbnos,i);
    else
      g := G.i;
      lo := #orb;
...
PCOSG_matact := function(G, fg, pt)
/* Polycyclic orbit-stabiliser calculation.
 * Version of PCOSG where the action is action of a matrix,
 * fg:G -> GL, fv is replaced by pt^mat, and pt is a space.
 * Return orbit of pt, stabiliser of pt as subgroup of G, and
 * generator numbers in G that were used to build up orbit.
 */
  local ng,ords,orb,stabgens,orbnos,im,posmg,lo,pow,g,h;
...
PCOSG_vecmat := function(G, fg, pt)
/* Polycyclic orbit-stabiliser calculation.
 * Version of PCOSG where the action is vector * matrix,
 * fg:G -> GL, fv is replaced by vec*mat mult, and pt is a vector.
 * Return orbit of pt, stabiliser of pt as subgroup of G, and
 * generator numbers in G that were used to build up orbit.
 */
  local ng,ords,orb,stabgens,orbnos,im,posmg,lo,pow,g,h;

  if Type(pt) eq ModTupFld then
    return PCOSG_matact(G, fg, pt);
  end if;

  ng := NPCgens(G);
  ords := PCPrimes(G);
  Gim := [Matrix(fg(G.i)): i in [1 .. ng]];
  orb := {@ Vector(Matrix(pt)) @};
  stabgens := [G|];
  orbnos := [];
  for i in [ng .. 1 by -1] do
    Gi := Gim[i];
	...
	s := #orb;
	offset := 0;
//printf "i: %o, orb s: %o, ords[i]: %o\n", i, s, ords[i]; time
	for k := 1 to ords[i] - 1 do
	    if s gt 10 then
		X := Matrix([orb[offset + j]: j in [1 .. s]]);
		X := X*Gi;
		for j in [1 .. s] do
		    Include(~orb, X[j]);
		end for;
...
//it for all BSGS calculations.
      ...
      factg := cqpcact;
      factv := func<p, g | p * g>;
      // rado,radst,radonos := PCOSG(cqpc,factg,factv,V.(d+1), fact);
      rado,radst,radonos := PCOSG_vecmat(cqpc, factg, V.(d+1));
        ...
        while pt le deg do
          ...
          for i in [1..#igenmats] do
            ...
            if pos eq 0 then //new orbit
              // fullo join:=  PCOSG(cqpc,factg,factv,imvec, fact);
              fullo join:=  PCOSG_vecmat(cqpc, factg, imvec);
              deg +:= 1;
              perms[i][pt] := deg;
...
//it for all BSGS calculations.
	  ...
	  factg := cqpcact;
	  factv := func<p, g | p ^ g>;
          // rado, radst, radonos := PCOSG(cqpc, factg, factv, orbrep, fact);
          rado, radst, radonos := PCOSG_vecmat(cqpc, factg, orbrep);
