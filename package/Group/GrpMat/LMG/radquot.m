...
GetRadquot := procedure(G : warning:=false )
  local r, factortype, socperm, Gtosocperm, sfclass, fromfacs, tofacs, ims,
   ...
   d, q, fac, p, e, imgp, injb, injt, m, im, wg, iwm, factoword, aggens, graph,
   P, em, oa2, oi2, imm, fromfac, tofac, factowd, Y, totdeg, o8pgraph, o8plus,
   ngensG, ngims;
  if warning then
    vprint LMG, 1:
 "Warning: we will need to find a permutation reptn of the radical quotient!";
  ...
  for o in O do
    ...
    if sfno eq cslen and Type(sf) eq GrpMat and
                                       RecogniseClassical(sf) cmpeq true then
       //in this case, things are a lot simpler, we can just use tofacs[sfno]
       vprint LMG,1 : "Using easier homomorphism method for top factor";
       toperm, imgp := OrbitAction(sf, sub<V|V.1>) where V:=VectorSpace(sf);
       Append( ~homs, map< Generic(G)->imgp |
                       g :-> g @ Function(tofacs[sfno]) @ toperm > );
      continue o;
    end if;
    ...
    if not assigned sf`AutInfo then
      ...
      else InitialiseAutInfoG(sf : trivial := sfno eq cslen );
      end if;
    end if;
    ai := sf`AutInfo;
    sfc := sfclass[ o[1] ];
    if sfc then //classical type
      ...
      if q^d gt 10^9 then
        error
          "Degree of permutation representation of radical quotient too big";
      end if;
      fac := Factorisation(q);
      p := fac[1][1]; e := fac[1][2];
    ...
    ImElt := function(g) //image of an element g in G under map to imgp
      local im, el, atup, genims, gaut, gsfi, genims1, genims2,
            SS, rq, A, em, P, Strip;
      Strip := function(x,g)
  ...
  //having defined the map, we now can evaluate it using rewriting
  ngensG := [NG.i : i in [1..Ngens(NG)]] where NG:=CompositionTreeNiceGroup(G);
  ngims := [m(g) : g in ngensG];
  //also need inverse map
  wg := WordGroup(im); iwm := InverseWordMap(im);
  G`LMGNode`Gtoradquot := map< Generic(G)->im |
                     g :-> Evaluate( w, ngims) where
                            _, w := CompositionTreeElementToWord(G,g),
                    gi :-> Evaluate( iwm(gi), [G.i : i in [1..Ngens(G)]] ) >;
  G`LMGNode`radquot := im;
  G`LMGNode`iwmrq := iwm;
  /* won't always do commands below at this stage, but do it for now */
  _:=LMGChiefSeries(G);
...
//perm rep of G/NRad(G) - not used at present
