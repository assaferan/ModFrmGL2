...
intrinsic LMGInitialise(G :: GrpMat[FldFin] : Al := "", Verify:= false,
             RandomSchreierBound := ZZZ`LMGSchreierBound, CTLimit := 3 )
{Initialize G for LMG computations, Return true if using BGSG}
   ...
   Initialize(G : Al := Al, VerifyOrder := Verify, CTLimit := CTLimit,
...
{Is H a subgroup of large matrix group G?}
  if IsKnownSubgroup(G, H) then return true; end if;
  if not assigned G`LMGNode then
    Initialize(G);
  end if;
...
{Index of subgroup H of large matrix group G}
  ...
  if not assigned H`LMGNode then
    Initialize(H : VerifyOrder := G`LMGNode`verified );
  end if;
  return #G div #H;
end intrinsic;
...
intrinsic LMGRadicalQuotient(G :: GrpMat[FldFin] : useRS := true) -> GrpPerm, Map, GrpMat
...
kernel}
  ...
  if useRS and G`LMGNode`RS then
...
{Sylow p-subgroup of large matrix group G}
  ...
  if assigned G`LMGNode`israd and G`LMGNode`israd then
    GG := G`LMGNode`radgp;
    S, P, m1 := LMGSolvableRadical(GG);
    S :=  sub< Generic(G) | {x @@ m1 : x in Generators( Sylow(P, p) )} >;
    fl := exists(t){t : t in LMGFactoredOrder(G) | t[1] eq p};
    S`Order := fl select t[1]^t[2] else 1;
    return S;
  end if;
  if not assigned G`LMGNode`series then
    GetCompSer(G);
...
{Test subgroups H, K of large matrix group G for conjugacy}
  ...
  if not assigned K`LMGNode then
     Initialize(K : VerifyOrder := G`LMGNode`verified );
  end if;
...
intrinsic LMGHomomorphism(G :: GrpMat[FldFin], ims :: SeqEnum) -> Map
{Homomorphism from G where ims are images of generators}
  if not assigned G`LMGNode then
    Initialize(G);
  end if;
  if G`LMGNode`RS then
    return hom< G -> Generic(Universe(ims)) | ims >;
  end if;

  _, nice_words := CompositionTreeNiceToUser(G);
  nice_ims := Evaluate(nice_words, ims);
  assert Type(nice_ims) eq SeqEnum;
  return map< Generic(G) -> Generic(Universe(ims)) |
       g :-> Evaluate(w, nice_ims)
