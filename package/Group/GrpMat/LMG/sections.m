 ...
 */
  ...
  vprint LMG,1: "Computing soluble radical";
  r := G`LMGNode;
  //first try BSGS on rad
  rad := r`rad;
  Initialize(rad : VerifyOrder := G`LMGNode`verified );
  if rad`LMGNode`RS then
    ...
    vprint LMG,1: "Computed soluble radical";
    return;
  end if;
  radfac := Reverse([ i : i in [1..#r`factortype] | r`factortype[i] eq 1 ]);
   //reversed because PC generators are numbered from top down
  ...
  rels := [];
  //calculate pcrels
  vprint LMG,2: "Computing PC-relations of solvable radical";
  for i in [1..pclen] do
    pos := radfac[i];
    ord := #r`ims[pos];
...
*/
  vprint LMG,2: "Defined PCGroup of solvable radical";
  vprint LMG,1: "Computed soluble radical";
 ...
 */
  ...
  vprint LMG,2 : "Computed PCGroup of Rad/UniRad";
end procedure;
 ...
 */
  ...
  rels := [];
  //calculate pcrels
  vprint LMG,2: "Computing PCGroup of SocleKernel/SocleStar";
  for i in [1..pclen] do
    pos := pkerfac[i];
    ord := #r`ims[pos];
    Append(~rels, F.i^ord = NF( r`Y[pos][1]^ord ) );
  ...
  vprint LMG,2: "Computed PCGroup of SocleKernel/SocleStar";
end procedure;
...
GetChiefSer := procedure(G)
  ...
  if not assigned G`LMGNode`pkerPC then
    vprint LMG,1: "Computing PKernel";
    GetPKer(G);
    vprint LMG,1: "Computed PKernel";
  end if;
  if not assigned G`LMGNode`Gtosocperm then
    vprint LMG,1: "Computing socle action";
    GetSocleAct(G);
    vprint LMG,1: "Computed socle action";
  end if;
  r := G`LMGNode;
  cseries := [ sub<G|> ];
  cfacname := [];
  ...
  if #[t : t in [1..#r`factortype] | r`factortype[t] eq 1] ne 0 then
    vprint LMG,1: "Computing chief factors of soluble radical";
    P := r`radPC;
    m1 := r`radtoPC;
    ...
    if LMGIsSoluble(G) then
      ...
      for l in [#E-1 .. 1 by -1] do
        ...
        Append(~radmodactions,
                       map< Generic(G)->GL(d,p) | g :-> GL(d,p)!(g@m1@r) > );
      ...
      vprint LMG,1: "Computed chief factors of soluble radical";
      return;
    end if;
   ...
   (L!ChangeUniverse( Eltseq(m.j @@ phi @ Morphism(CM[i],M)), Integers() ))
        ...
        Append( ~radmodactions, map< Generic(G) -> GL(d,p) |  g :->
  ...
  if #socfacs gt 0 then
    vprint LMG,1: "Computing chief factors of SocleStar";
    ...
    vprint LMG,1: "Computed chief factors of SocleStar";
  end if;

  //layer 3
  if #[t : t in [1..#r`factortype] | r`factortype[t] eq 3] ne 0 then
    vprint LMG, 1: "Computing chief factors of SocleActionKernel/SocleStar";
    P := r`pkerPC;
    m1 := r`pkertoPC;
    E := ElementaryAbelianSeriesCanonical(P);
    for l in [#E-1 .. 1 by -1] do
   ...
   (L!ChangeUniverse( Eltseq(m.j @@ phi @ Morphism(CM[i],M)), Integers() ))
    ...
    vprint LMG, 1: "Computed chief factors of SocleActionKernel/SocleStar";
  end if;

  //layer 4
  m1 := r`Gtosocperm;
  if #[t : t in [1..#r`factortype] | r`factortype[t] eq 4] ne 0 then
    vprint LMG, 1: "Computing chief factors of G/SocleActionKernel";
    P := r`socperm;
    E := ChiefSeries(P);
    cf := ChiefFactors(P);
    for l in [#E-1 .. 1 by -1] do
    ...
    vprint LMG, 1: "Computed chief factors of G/SocleActionKernel";
  end if; 
