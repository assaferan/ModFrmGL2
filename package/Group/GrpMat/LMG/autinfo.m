 ...
 */
   ...
   * 4 = field
   * 5 = graph (type "L", Sp(4,even) and "O8+" only)
   ...
   * Note that, in the nonlinear cases, all calculations are carried out not
 ...
 */
  ...
  grapho := type eq "O+" and d eq 8 and genims1 ne [];
...
InitialiseAutInfoG := procedure(S : trivial:=false )
  ...
  if trivial then
    vprint LMG, 3: "no outer automorphisms of group of degree", Degree(SS);
    A := SS;
    ...
    S`AutInfo :=  rec< AutInfoRFG | autgp:=A, outautgp:=OA, atopgp:=rep,
            ...
            autos:=[], gen:=[], slp:=[] >;
  elif IsAlternating(SS) and Degree(SS) ne 6 then
