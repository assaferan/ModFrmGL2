 ...
 */
 local r,rpc,rad,rtopc,rq,Gtorq,rm,m,ptom,gmacts,mats,cs,mgensG,src,
       ...
       orb, prep, grep, orbreps, intmorph, normsubS, sns, normsubSonSL,
       subdim, quodim, SM, subSM, quoSM, projSM, SpresM, subSpresM, repquoSM,
 ...
 for rep in orbreps do
   ...
   if quodim ne 0 then
     ...
     agens := [GL(Dimension(quoSM),p)!a: a in ActionGenerators(quoSM)];
     repquoSM := LMGHomomorphism(normsubS,agens);
   ...
   if not split then
     ...
     if  nc gt 1 and normsubS ne SA and subrec`order lt quolimit then
        ...
        * is very large, then we use a more direct method, with LMGIsConjugate,
        * which we do later.
...
/* Unfortunately need to use quotient here */
       ...
       for i in [1..#normgens] do
         ...
         for c in [1..nc] do
           ...
           for j in [1..ngs] do
             ...
             vec := Vector(tail[i][j]) +
                             Vector(TailVector(SpresM,comp,word[i][j])) * mat;
       ...
       if (calcpres) then
         if subdim ne 0 then
           if SA eq A then
             ...
             agens := [GL(Dimension(subSM),p)!a: a in ActionGenerators(subSM)];
             repsubSM := LMGHomomorphism(normsubS,agens);
     ...
     if  nc gt 1 and normsubS ne SA and subrec`order ge quolimit then
       ...
       smallind := LMGIndex(SA,sublist[1]) le translimit;
       if smallind then
"here";
         trans := [ LMGRightTransversal(SA, LMGNormaliser(SA,s) ) :
                                                          s in sublist ];
       end if;
       for i in [1..#normgens] do
         ngp[i] := [];
         ...
         for c in [1..nc] do
           if smallind then
             ...
             for d in [1..nc] do
               if LMGIsConjugate(SA,conjsub,sublist[d]) then
                 ngp[i][c] := d;
                 break;
               end if;
             end for;
...
/* use standard LowIndexSubgroups on solvable radical and then lift through
...
*/
  ...
  for i in [#rm .. 1 by -1] do
    ...
    lisubsl := [];
    if Print ge 1 then
      print "Extending through layer",i,"of size ",#rm[i][1];
