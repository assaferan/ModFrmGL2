...
Rearrange := procedure(G)
  ...
  ninsol, nsocfacs, socfac, sfclass, o8pgraph, socperm, socpermtriv, inpker,
  perm, x, conx, imnode, invim, socpermw, socpermg, Fg, Ftosp, Ftospw, Ftospg,
  S, node, genims, inner, ag, aw, mat, insr, deg, p, m, l;
  ...
  o8pgraph := []; //o8pgraph[i] is true iff socfac[i] is C8-group
       //of type O8+ and a graph isomorphism is induced on it within G
...
//Must be careful because gaut acts on transformed group.
  ...
  //if o8pgraph[i] is true then it must be true for all j in orbit of i.
  if deg ne 0 then
    for orb in Orbits(socperm) do
     for pt in orb do
      if o8pgraph[pt] then
        for pt2 in orb do o8pgraph[pt2] := true; end for;
        break;
      end if;
     end for;
    end for;
