...
sporadic_outer := [ 1, 2, 2, 1, 1, 1, 2, 2, 2, 2, 2, 1, 1, 1, 2, 2, 1, 2,
1, 1, 2, 1, 2, 1, 1, 1];
...
simple_short_name := function(tup)
    ...
    case tup[1]:
	when 19: name := "Z_" cat IntegerToString(tup[3]); // cyclic
...
intrinsic SimpleGroupOuterOrder(T::Tup) -> RngIntElt
{The order of the outer automorphism group of the simple group
denoted by the tuple T}
    return SimpleGroupOuterOrder(T[1], T[2], T[3]);
end intrinsic;

intrinsic ChiefFactorsToStringDescending(Q::SeqEnum[Tup]) -> MonStgElt
{A string representing the chief factors sequence Q}
    str := "";
    for i := 1 to #Q do
        t := Q[i];
