...
function ReadAtlasPermutation(filename, s, e, d)
    file := Sprintf("%o/%o.dat", libroot(), filename);
    F := LibDataOpenRead(file);
    Seek(F, s, 0);
    b := 0;
    x := d;
    while x gt 0 do
...
intrinsic PermutationGroup(N::MonStgElt) -> GrpPerm
{The permutation group corresponding to the 1st available permutation
representation for the ATLAS group named N}
    return PermutationGroup(ATLASGroup(N));
end intrinsic;
...
intrinsic MatrixGroup(N::MonStgElt) -> GrpMat
{The matrix group corresponding to the 1st available matrix
representation for the ATLAS group named N}
    return MatrixGroup(ATLASGroup(N));
end intrinsic;
