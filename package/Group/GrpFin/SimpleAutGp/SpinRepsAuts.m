...
// largeff used for constructing twisted representations, where we need to make FF larger
	...
	for i in [2..n-2] do;
		a:=beta[i-1];
		alpha[i] := function(t); return DirectSum(a(t),(a)(t)); end function;
		alpham[i] := function(t); return Transpose(alpha[i](t)); end function;	
	end for;

	alpha[n-1] := function(t); return DirectSum(beta[n-2](t),beta[n-1](-t)); end function;
	alpham[n-1] := function(t); return Transpose(alpha[n-1](t)); end function;
	alpha[n] := function(t); return DirectSum(beta[n-1](t),beta[n-2](-t)); end function;
...
// If it has length 1, the entry generates the full automorphism group, and lies in the classical group if this is possible.
	...
	if q ne 0 then;
		for i in [1..n-3] do;
			...
			Ym[n-1+i] := function(t); return Transpose(Y[n-1+i](t^q)); end function;
		end for;
		Y[2*n-2] := function(t); return Ym[n-1](1) * Y[n-1](-1) * Y[n-2] (-t) * Y[n-1](1) * Ym[n-1](-1); end function;
		Ym[2*n-2] := function(t); return Transpose(Y[2*n-2](t^q)); end function;
		Y[2*n-3] := function(t); return Y[2*n-2](t^(q+1)) * Y[n-2](1)^-1 * Y[n-1](t)^-1 * Y[n-2](1) * Y[n-1](t); end function;
		Ym[2*n-3] := function(t); return Transpose(Y[2*n-3](t^q)); end function;
...
FullSpinMinus := function(n,q : aut:=false)
  local p, e, nh, m1, m2, m3, m3g, mm1, mm2, k, gens, set, grp;
  assert IsEven(n);
  nh:= n div 2;
  p := Factorisation(q)[1][1];
  e := Factorisation(q)[1][2];
  nh:= n div 2;
  m1, m2, m3 := spininductminus(nh,q : aut:=aut);
  k := NumberOfRows(m1[1]); assert k eq 2^(nh-1);
  mm1 := m1; mm2 := m2;
  ...
  gens := [DiagonalJoin(m1[i],mm1[i]) : i in [1..nh*e]] cat
          [DiagonalJoin(m2[i],mm2[i]) : i in [1..nh*e]];
/*
  nm := #m1; assert nm eq #m2;
  gens := [ DiagonalJoin(m1[i],m1[i]) : i in [1..nm-1]] cat
          [ DiagonalJoin(m1[nm], MatToQ(m1[nm],q)) ] cat 
          [ DiagonalJoin(m2[i],m2[i]) : i in [1..nm-1]] cat
          [ DiagonalJoin(m2[nm], MatToQ(m2[nm],q)) ];
*/
  if aut then
     if IsOdd(q) then
...
SpinCircle := function(n,q : aut := false)
  local nh, m1, m2, m3, k, gens, grp;
  assert IsOdd(n);
  nh:= n div 2;
  m1, m2, m3 := spininductcirc(nh,q : aut:=aut);
  k := NumberOfRows(m1[1]); assert k eq 2^nh;
  gens := aut select m1 cat m2 cat m3 else m1 cat m2;
  grp := sub< GL(k,q) | gens >;
  return grp;
end function;
...
// Fi and Fim are the four families of generators from spininductplus
  ...
  // Permute these families by the triality automorphism of the Dynkin diagram, induced by (1,3,4)
