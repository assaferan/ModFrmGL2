...
LowIndexSubgroups(G::GrpPerm, maxind:: RngIntElt :
                  Presentation:=false, Print:=0, Algorithm := "",
		  Limit := Infinity ) -> SeqEnum
{Subgroups of index up to maxind in permutation group G}

  if Algorithm eq "" then
    //decide default algorithm using layer sizes
    L:=ElementaryAbelianSeries(G);
    mL := #L lt 2 select 0 else Max([Index(L[i],L[i+1]): i in [1..#L-1]]);
    Algorithm := mL le 1000 select "Subgroups" else "Recursive";
  end if;
  vprint Subgroups, 1 : "Using algorithm", Algorithm;
...
{Subgroups of index between T[1] and T[2] in permutation group G}
  ...
  if Algorithm eq "Recursive" then
	...
	s := Subgroups(G : IndexLimit := b, Presentation:=Presentation);
    else 
...
intrinsic
LowIndexSubgroups(G::GrpPerm, T:: SeqEnum[RngIntElt] :
                  Presentation:=false, Print:=0, Algorithm := "",
		  Limit := Infinity ) -> SeqEnum
{Subgroups of index between T[1] and T[2] in permutation group G}
...
intrinsic
LowIndexSubgroups(G::GrpPerm, N::GrpPerm, T:: SeqEnum[RngIntElt] : Print:=0) -> SeqEnum
{Subgroups of index between T[1] and T[2] in permutation group G that contain
 the normal subgroup N of G}
  local subs;
  require IsNormal(G,N) :
...
intrinsic
LowIndexSubgroups(G::GrpMat, T:: SeqEnum[RngIntElt] :
                  Presentation:=false, Print:=0, Algorithm := "",
		  Limit := Infinity ) -> SeqEnum
{Subgroups of index between T[1] and T[2] in matrix group G}
...
intrinsic
LowIndexSubgroups(G::GrpMat, N::GrpMat, T:: SeqEnum[RngIntElt] : Print:=0) -> SeqEnum
{Subgroups of index between T[1] and T[2] in matrix group G that contain the
 normal subgroup N of G}
  local subs;
  require IsNormal(G,N) :
