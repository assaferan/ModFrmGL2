...
//K normal in FP-group G, p prime. Alternative version of GModule.
  ...
  // nkgens := NumberOfGenerators(K);
  // printf "\nNumber of generators for K in GModuleH is %o.\n", nkgens;
  // if nkgens gt 100 then
  //     K := Rewrite(G, K: Simplify := false);
  //     K<[x]> := SimplifyLength(K);
  // end if;     

  P, phi := pQuotient(K, p, 1);
  // P, phi := ElementaryAbelianQuotient(K, p); P;
