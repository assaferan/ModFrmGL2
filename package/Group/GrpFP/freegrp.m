...
/* Return true/false if H is subgroup of free group F and, if so, a list
  ...
  FS`FSInfo := rec< Format(F`FSInfo) | issupergroup := true, supergroup := FS,
                                                        supergroupind:=0 >;
  //we put supergroupind = 0 because we won't be keeping this group
 ...
 * free generators set up as a supergroup.
...
local gens, fg, dfa, sg, s, cos, y;
  ...
  assert [ FS | FS.i : i in [1..Ngens(FS)]] eq F`FSInfo`freegens;
