...
/*****************************************************
  ...
  $Id: invar_forms.m 57092 2018-06-12 03:06:25Z don $
...
{ The semilinear dual of the G-module M with respect to the 
  ...
  G := Group(M);
  f := GModuleAction(M);
  return GModule(G,[ConjugateTranspose(f(G.i)^-1,mu) : i in [1..Ngens(G)]]);
end intrinsic;


// In the following two intrinsics lambda is a homomorphism from the action
// group of the module to the field.
...
{ The twisted dual of the G-module M with respect to the linear character lambda }
  G := Group(M);
  f := GModuleAction(M);
  return GModule(G,[ScalarMatrix(Dimension(M),lambda(G.i))*Transpose(f(G.i)^-1)
    : i in [1..Ngens(G)]]);
end intrinsic;
...
{ The twisted semilinear dual of the G-module M with respect to the 
  ...
  G := Group(M);
  f := GModuleAction(M);
  return GModule(G,[ScalarMatrix(Dimension(M),lambda(G.i))*ConjugateTranspose(f(G.i)^-1,mu)
    : i in [1..Ngens(G)]]);
end intrinsic;
