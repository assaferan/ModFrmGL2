...
function is_coercible_tenspecelt(T,t)
  ...
  return false,"Incompatible.";
end function; //end intrinsic;

function is_coercible_seqenum(T,S) // MW: not sure what happens if S is empty?
  D := [ Dimension(X) : X in Frame(T) ];
  R := T`Ring;

  if &*D ne #S then // check frames
    return false,"Incompatible frames.";
  end if; // MW: invalid when D is empty?
...
intrinsic IsCoercible( T::TenSpc, x::Any ) -> BoolElt
{Determines if x is coercible into T.}
 if ISA(Type(x),SeqEnum) then
  if #x ne 0 and not ISA(Type(x[1]),RngElt)
   then return false,"Bad type in coercion"; end if;
  return is_coercible_seqenum(T,x); end if;
 if ISA(Type(x),TenSpcElt) then return is_coercible_tenspecelt(T,x); end if;
