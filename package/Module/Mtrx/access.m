...
{Given a dxd matrix and a d-dimensional algebra, return whether it represents
 ...
 R:=RepresentationMatrix(e); if R eq M then return true,e; end if; z:=0;
 for i,j in [1..d] do if R[i,j] eq 0 and M[i,j] eq 0 then continue; end if;
  if R[i,j] eq 0 or M[i,j] eq 0 then return false,_; end if;
  if z ne 0 and z ne M[i,j]/R[i,j] then return false,_; end if;
  z:=M[i,j]/R[i,j]; end for; return true,e*z; end intrinsic;
...
intrinsic
 RepresentationMatrixOfMatrix(M::Mtrx[AlgAssV] : Side:="Right") -> Mtrx
