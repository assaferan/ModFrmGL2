...
intrinsic SparseMonomialMatrix(g::GrpPermElt) -> MtrxSprs
{The d by d +-1 sparse monomial matrix corresponding to the degree (2*d)
permutation g}

//"g:", g; Parent(g);

    d := Degree(Parent(g));
    require IsEven(d): "Permutation degree must be even";
    d div:= 2;

//"d:", d;
    X := SparseMatrix(IntegerRing(), d, d);
    for i := 1 to d do
	j := i^g;
	jj := (d + i)^g;
//printf "i: %o, j: %o, jj: %o (im of %o)\n", i, j, jj,d+i;
	if j gt d then
	    X[i, j - d] := -1;
	    assert jj eq j - d;
	else
