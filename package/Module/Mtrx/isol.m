...
other information (set E to elementary divisors of X if known)}

    require BaseRing(X) cmpeq Z: "Matrix must be integral";

//"LatticeBasisMatrix:"; X: Magma;
...
DELAY := false;
        if DELAY then
            ...
            vtime Isolution: EDY := ElementaryDivisors(Y);
vprint Isolution: "EDY", {* x: x in EDY *};
            det := &*EDY;
...
intrinsic LatticeBasisMatrixSimple(X::Mtrx) ->
    Mtrx, Mtrx, Mtrx, [], Mtrx, []
{Simple version, acting on full matrix in one go; return sequence L
of rows to keep, and transformation matrix T}

    vprintf Isolution: "LatticeBasisMatrixSimple: %o by %o\n",
	Nrows(X), Ncols(X);
    ...
    vprint Isolution: "Get kernel";
    vtime Isolution: N := KernelMatrix(ReverseRows(X));
    Nnr := Nrows(N);
    if Nnr eq 0 then
	return [1 .. Xnr], 1;
    end if;
    ...
    rank := Xnr - Nnr;
    N := EchelonForm(N);
    piv := [Xnr + 1 - Min(Support(N[i])): i in [1 .. Nnr]];
    N := ReverseColumns(N);
    ind := [i: i in [1 .. Nnr] | N[i, piv[i]] ne 1];
    indc := Set(ind);
    delc := {piv[i]: i in [1 .. Nnr] | i notin indc};
    vprint Isolution: "ind:", ind;
    vprint Isolution: "delc:", delc;
    keep := [i: i in [1 .. Xnr] | i notin delc];

    N := ColumnSubmatrix(N, keep);
    N := RowSubmatrix(N, ind);
    vprintf Isolution: "Rank: %o, use %o row(s) of %o\n", rank, #keep, Xnr;

    IndentPush();
    if 1 eq 0 then
	U := ReverseColumns(UnimodularExtension(ReverseColumns(N)));
    else
	U := UnimodularExtension(N);
    end if;
    IndentPop();

    vprint Isolution: "Total LatticeBasisMatrixSimple time:", Cputime(zeit);

    return keep, U;

end intrinsic;

intrinsic ReduceGramMatrix(F::Mtrx) -> Mtrx, [], Mtrx
{Return (r by r) reduced Gram matrix F2, indices
sequence L and transformation matrix T so that F2 equals
T*ColumnSubmatrix(RowSubmatrix(F, L), L)*Transpose(T) and r is the rank
of F}

    /*
    B,T,_,L := LatticeBasisMatrix(F);

    F2 := ColumnSubmatrix(B, L)*Transpose(T);
    return F2, L, T;
    */

    L,T := LatticeBasisMatrixSimple(F);

    if T cmpeq 1 then
	T := MatrixRing(Z, #L)!1;
    end if;

    F2 := T*RowSubmatrix(ColumnSubmatrix(F, L), L)*Transpose(T);
    return F2, L, T;
