...
intrinsic MultiplyRow(~X::MtrxSprs, c::RngInt, i::RngIntElt)
...
intrinsic MultiplyRow(X::MtrxSprs, c::RngInt, i::RngIntElt) -> MtrxSprs
...
intrinsic MultiplyColumn(~X::MtrxSprs, c::RngElt, j::RngIntElt)
{Multiply column j of X by the scalar c in place}

    requirerange j, 1, Ncols(X);
    l, c := IsCoercible(BaseRing(X), c);
    require l: "Argument 2 not coercible into ring of matrix";
    if IsOne(c) then
	return;
...
intrinsic MultiplyColumn(X::MtrxSprs, c::RngElt, j::RngIntElt) -> MtrxSprs
{Return the matrix obtained from X by multiplying column j by the scalar c};

    MultiplyColumn(~X, c, j);
    return X;
end intrinsic;
