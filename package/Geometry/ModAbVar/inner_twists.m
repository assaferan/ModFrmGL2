...
// MW Dec 2018, this was utterly stupid and using absolute precision before??
   assert #v eq #w;
   for i in [1..#v] do
     if Abs(v[i]) lt prec and Abs(w[i]) lt prec then continue; end if;
     if Abs(v[i]) lt prec or Abs(w[i]) lt prec then return false; end if;
     if (Abs(v[i] - w[i]))/Max(Abs(v[i]),Abs(w[i])) gt prec then
         return false;
      end if;
   end for;
   return true;
...
function IsInnerTwist(chi, aplists, P)
   ...
   for j in [1..#aplists] do
       aplist := aplists[j];
       if CC_Sequence_AlmostEqual(twist, aplist, 10^(-Precision(CC)/2)) then
          return true, j eq 1; // use precision relative to CC
       end if;
   end for;
   return false, false;
end function;
