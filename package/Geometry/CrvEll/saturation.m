...
OUTLINE AND REMARKS
   ...
   subgp G < E(K), we hope to efficiently identify the "p-divisible part of G",
   namely { P in G : p*R = P for a R in E(K) }, by reducing at enough primes q.
   ...
   ANSWERS:
	  ...
	  L/K_1 is Galois, and it can happen evry cyclic subgroup of Gal(L/K_1)
	  either fixes all the p-torsion, or fixes one of the R's. Ergo, for
	  every q such that E(Fq) has one p-torsion point, E(Fq) must either
          contain all the p-torsion, or contain one of the R's. In particular,
          for every prime allowed in Step 1, P will be in p*E(Fq). (This is why
          we try Step 4 if we're not done after Step 1.)

   NOTES
    + Optimizations are based on assumption that the input is nearly saturated.
...
function findq (E, p, bad, pqDB, i, q0, fldrat)
     ...
     // In view of Sato-Tate, use only the middle part of each Hasse interval
     ST := 0.7;
     A := Ceiling(q0/p);
...
function findqSpecialCase (E, p, bad, pqDB, i, q0, fldrat)
    ...
    vprintf ECSaturation, 2: "Searching for q (not precomputed, spec case): ";
    timeq := Cputime();
...
intrinsic Saturation(points::SeqEnum[PtEll], N::RngIntElt :
                     OmitPrimes := [], TorsionFree := false, Check := true ) 
       -> SeqEnum[PtEll]
...
//       "Not implemented for relative extensions";
   ...
   if Check then points := IndependentGenerators(points);  end if;

// MW (Apr 2018), hacks to ensure integrality and absoluteness
   if not IsIntegralModel(E) then
    I,i,v:=IntegralModel(E); pts:=[i(pt) : pt in points];
    pts:=Saturation(pts,N : TorsionFree:=TorsionFree,
		    Check:=Check, OmitPrimes:=OmitPrimes);
    return [v(pt) : pt in pts]; end if;
   if not fldrat and not IsAbsoluteField(K) then
    KK:=AbsoluteField(K); EK:=EllipticCurve([KK!a: a in aInvariants(E)]);
    pts:=[EK![KK!pt[1],KK!pt[2],KK!pt[3]] : pt in points];
    pts:=Saturation(pts,N : TorsionFree:=TorsionFree,
		    Check:=Check, OmitPrimes:=OmitPrimes);
    return [E![K!pt[1],K!pt[2],K!pt[3]] : pt in pts]; end if;
// also appearing in MordellWeilGroup, MordellWeilShaInformation, and below
// don't think need FldCyc here ... ?
   ...
   // When E has CM defined /K, p is special when p is nonsplit in the CM field
   // TO DO:
   // larger database, because for nonsplit p,
   // Prob(p|#Eq given q = 1 mod p) still 1/p asymptotically BUT need q > p^2
   ...
   // need to include tors when checking p-divisibility (at least when p | #T)
   ...
   vprintf ECSaturation: "Precomputing auxiliary primes q up to %o: ", B;
   vtime ECSaturation:
   if fldrat then 
   ...
   while true do
	...
	if (p le 163 or fldnum) and not ker subset tors then
	    vprintf ECSaturation, 2:
	     "Special case: try primes q such that #E(Fq)[%o] = %o^2\n", p, p;
		...
		if ker subset tors then 
                ...
                q, Eq, res, ii, q0 :=
		  findqSpecialCase(E, p, bad, pqDB, ii, q0, fldrat);
		vprintf ECSaturation, 3: "q = %o\n",q;
		...
		//   E(Q)/p*E(Q) -> E(F_q)/p*E(F_q) -> E(F_q)[p] -> mu_p -> Z/p
		//
		//           P  :->   . . . .      :->        Tate_p(T, P)
                //
                if fldrat then
                ...
                L := Matrix(2, [Fp | Log(TatePairing(T, P, p)) :
				     T in [T1,T2], P in rpoints]);
		ker meet:= Kernel(L);
		vprintf ECSaturation, 3 : "current kernel is %o\n", ker;
	    end for;
	end if;
	...
	if not ker subset tors then
            ...
            for v in basis do
		     ...
		     "False positive in saturation at %o : %o\n", p, v;
                end if;
            end for;
...
group generated by the given points.}
   ...
   // if IsEmpty(points) then return points, true; end if;
   // MW: Apr 2018, check this below: also, needs torsion!
   ...
   // require fldrat or IsAbsoluteField(K): // MW Apr 2018, OK now
   //    "Not implemented for relative extensions";
...
// MW (Apr 2018), hacks to ensure integrality and absoluteness
   if not IsIntegralModel(E) then
    I,i,v:=IntegralModel(E); pts:=[i(pt) : pt in points];
    pts,b:=Saturation(pts : TorsionFree:=TorsionFree,
		      Check:=Check, HeightBound:=HeightBound);
    return [v(pt) : pt in pts],b; end if;
   if not fldrat and not IsAbsoluteField(K) then
    KK:=AbsoluteField(K); EK:=EllipticCurve([KK!a: a in aInvariants(E)]);
    pts:=[EK![KK!pt[1],KK!pt[2],KK!pt[3]] : pt in points];
    pts,b:=Saturation(pts : TorsionFree:=TorsionFree,
		      Check:=Check, HeightBound:=HeightBound);
    return [E![K!pt[1],K!pt[2],K!pt[3]] : pt in pts],b; end if;
// also appearing in MordellWeilGroup, MordellWeilShaInformation, and above
// don't think need FldCyc here ... ?

   n := #points;
   if n eq 0 then if TorsionFree then return points, true; end if;
      T, mT := TorsionSubgroup(E); tors := [T.i @ mT : i in [1..Ngens(T)]];
      return tors cat points, true; end if; // MW, Apr 2018, handle n eq 0
   R := Regulator(points);
   ...
   vprintf ECSaturation : "Saturation bound = %o\n", N;
   
   if N gt 1 then
      points0 := points;
      vprintf ECSaturation : "Saturation : ";
      vtime ECSaturation :
   ...
   vprintf ECSaturation : "Search bound = %o\n", X;
   
error if X ge 2^30, "Search bound is too large";

   vprintf ECSaturation : "Search : ";
   vtime ECSaturation :
   small := Points(E : Bound := X);
   small := {pt : pt in small | not IsId(pt)};
