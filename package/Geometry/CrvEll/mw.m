...
// MW (Apr 2018), hacks to ensure integrality and absoluteness
    if not IsIntegralModel(E) then I,i,v:=IntegralModel(E);
     G,m,b1,b2:=MordellWeilGroup(I); mp:=m*v; return G,mp,b1,b2; end if;
    if Type(F) ne FldRat and not IsAbsoluteField(F) then
     K:=AbsoluteField(F); EK:=EllipticCurve([K!a: a in aInvariants(E)]);
     G,m,b1,b2:=MordellWeilGroup(EK);
     mp:=map<G->E | x:->[F!y[1],F!y[2],F!y[3]] where y:=m(x)>;
     return G,mp,b1,b2; end if;
    if Type(F) eq FldCyc then K:=NumberField(DefiningPolynomial(F));
     ...
     mp:=map<G->E | x:->[F!y[1],F!y[2],F!y[3]] where y:=m(x)>;
     return G,mp,b1,b2; end if;
// also appearing in MordellWeilShaInformation and twice for Saturation funcs
