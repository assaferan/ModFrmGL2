...
a sequence of generators for the known subgroup of the Mordell-Weil group
 modulo torsion,
and a sequence specifying all known lower and upper bounds on Sha[n]
 for integers n.}

  require t eq FldRat or ISA(t, FldAlg) where t is Type(BaseRing(E)) :
         "The curve must be over the rationals or a number field";

  require Type(RankOnly) eq BoolElt :
          "Optional parameter 'RankOnly' should be a boolean";
  require Type(ShaInfo) eq BoolElt :
          "Optional parameter 'ShaInfo' should be a boolean";
  require Type(Sha) eq RngIntElt :
          "Optional parameter 'Sha' should be an integer";

  return MordellWeilShaInformation(E :
                                   RankOnly:=RankOnly, ShaInfo:=ShaInfo,
				   Sha:=Sha, Effort:=Effort,
				   HeightBound:=HeightBound, Silent:=Silent);
...
{"} // "
  ...
  require Effort ge 1 :
	 "Optional paramter 'Effort' should be a positive integer";

  // already done?
  if assigned E`MWProof and E`MWProof and not ShaInfo and Sha eq 1 then
    sha1:=[<n,sha[n]> : n in Sort(Setseq(Keys(sha)))] where sha is E`ShaBounds;
    return E`MWRankBounds, E`MWGens, sha1;
  end if;

  // default behaviour is to print info (the original purpose of the function)
...
// made messy by caching behavior
  if not IsIntegralModel(E) then I,i,v:=IntegralModel(E);
   A,B,C:=MordellWeilShaInformation(I : RankOnly:=RankOnly, ShaInfo:=ShaInfo,
				    Sha:=Sha, Effort:=Effort,
				    HeightBound:=HeightBound, Silent:=Silent);
   B:=[v(pt) : pt in B]; E`MWGens:=B; E`MWRankBounds:=A; E`MWProof:=I`MWProof;
   if assigned I`ShaBounds then E`ShaBounds:=I`ShaBounds; end if;
   return A,B,C; end if;
  F:=BaseField(E);
  if Type(F) ne FldRat and not IsAbsoluteField(F) then
   K:=AbsoluteField(F); EK:=EllipticCurve([K!a: a in aInvariants(E)]);
   A,B,C:=MordellWeilShaInformation(EK : RankOnly:=RankOnly, ShaInfo:=ShaInfo,
				    Sha:=Sha, Effort:=Effort,
				    HeightBound:=HeightBound, Silent:=Silent);
   B:=[EK![K|pt[1],pt[2],pt[3]] : pt in B]; E`MWGens:=B;
   E`MWRankBounds:=A; E`MWProof:=EK`MWProof;
   if assigned EK`ShaBounds then E`ShaBounds:=EK`ShaBounds; end if;
   return A,B,C; end if;
  if Type(F) eq FldCyc then K:=NumberField(DefiningPolynomial(F));
   assert IsIsomorphic(F,K); EK:=EllipticCurve([K!a : a in aInvariants(E)]);
   A,B,C:=MordellWeilShaInformation(EK : RankOnly:=RankOnly, ShaInfo:=ShaInfo,
				    Sha:=Sha, Effort:=Effort,
				    HeightBound:=HeightBound, Silent:=Silent);
   B:=[EK![K|pt[1],pt[2],pt[3]] : pt in B]; E`MWGens:=B;
   ...
   if assigned EK`ShaBounds then E`ShaBounds:=EK`ShaBounds; end if;
   return A,B,C; end if;
// also appearing in MordellWeilShaGroup and twice for Saturation funcs
  ...
  if type eq FldRat and Conductor(E) le 10^10 then
    ...
    Done(~done,rl,ru,ranal,~MWgens,sha,~sha1,
	 RankOnly,Sha,verbosity,HeightBound);
    if done then return [rl,ru], MWgens, sha1; end if; 
  end if;
