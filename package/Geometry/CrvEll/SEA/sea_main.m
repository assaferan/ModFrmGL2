...
intrinsic SEA(E::CrvEll :
	...
	*/
    ...
    // MW: Put curve over absolute extension, so Trace/Norm work as expected
    if Ilog(p,q) ne Degree(DefiningPolynomial(FF)) then
     K:=GF(q); Embed(FF,K); // no IsIsomorphic here, and want fixed GF(q)
     return SEA(EllipticCurve([K!a :  a in aInvariants(E)])); end if;
