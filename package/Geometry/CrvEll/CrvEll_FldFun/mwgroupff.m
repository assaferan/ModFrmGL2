 ...
 ************
  CHANGE LOG:
              ...
              **** PLEASE RECORD ALL CHANGES HERE ****
                                     ...
                                     -- Steve
  Oct 2018, Mike:
      + Fixed the p-part of TorsionBound in char p. Needed to add checks
        for p-torsion in the kernel of fibre specialisation: Can happen
        when the fibre is supersingular (and the curve over the function field
        has a non-const j-invariant, so in particular is ordinary).
  Nov 2010, Steve:
...
function my_saturate_atp(PTS,p,T) U:=IdentityMatrix(Rationals(),#PTS);
 for v in VectorSpace(GF(p),#PTS) do if v eq 0 then continue; end if;
  w:=ChangeRing(v,Integers()); pt:=&+[w[c]*PTS[c] : c in [1..#PTS]];
  b,dpt:=IsDivisibleBy(pt,p); if not b then continue; end if;
  assert exists(c){c: c in [1..#PTS] | w[c] ne 0};
  U[c]:=ChangeRing(w,Rationals())/p; PTS[c]:=dpt;
  return my_saturate_atp(PTS,p,U*T); end for; return PTS,T; end function;

function my_saturation(PTS,P) T:=IdentityMatrix(Rationals(),#PTS);
 for p in P do PTS,T:=my_saturate_atp(PTS,p,T); end for;
 return PTS,T; end function; // this is hacky, but it should work

function check_overK(E,f,Lgeom,basinv,r)
 if #basinv eq 0 then return basinv; end if;
 K:=CoefficientRing(BaseRing(E)); L:=CoefficientRing(BaseRing(Codomain(f)));
 d:=Degree(L,K); if d eq 1 then return basinv; end if;
 if &and[IsCoercible(E,&+[f(Lgeom.i*b[i]) : i in [1..r]]) : b in basinv]
  then return basinv; end if; S:=[];
 for b in [1..#basinv] do m:=0;
  while true do m:=m+1; pt:=&+[f(Lgeom.i*m*basinv[b][i]) : i in [1..r]];
   if IsCoercible(E,pt) then break; end if; end while;
  basinv[b]:=basinv[b]*m; S:=S cat [E!pt]; end for;
 _,T:=my_saturation(S,PrimeDivisors(d));
 A:=Rows(T*ChangeRing(Matrix(basinv),Rationals())); return A; end function;

// utility code to get around torsion vs degree problem
...
{The height pairing lattice of the free quotient of the Mordell-Weil
 group of E, together with a map to the elliptic curve.}
  ...
  if assigned E`MordellWeilLattice then
    MWL:=E`MordellWeilLattice; return MWL[1],MWL[2];  end if;
  ...
  Frob:=ChangeRing
    (FrobeniusActionOnPoints(geombasis,q:gram:=GramMatrix(Lgeom)),Integers());
  inv:=Kernel(Frob-IdentityMatrix(Integers(),r));
  basinv:=Basis(inv);
  basinv:=check_overK(E,f,Lgeom,basinv,r); // MW Oct 2018
  ...
  MWLmap:=hom<MWL -> E |
		v :-> &+[E| (Integers()!v[i])*basis[i] : i in [1..#basinv] ] >;
  E`MordellWeilLattice:=<MWL,MWLmap>;
  return MWL,MWLmap;
end intrinsic;

intrinsic MordellWeilGroup(E::CrvEll[FldFunRat] : Al:="Geometric")
 -> GrpAb, Map
{The Mordell-Weil group of an elliptic curve defined
...
intrinsic GeometricMordellWeilGroup(E::CrvEll[FldFunG]) -> GrpAb, Map
{Given an elliptic curve over a function field, compute the geometric
 Mordell-Weil group, with torsion group as over the finite extension
 that captures the free part.} // MW Oct 2018
 L,f:=GeometricMordellWeilLattice(E); basis:=[f(x) : x in Basis(L)];
 F:=BaseRing(Codomain(f)); EF:=ChangeRing(E,F);
 T,m:=TorsionSubgroup(EF); t:=#Invariants(T);
 inv:=Invariants(T) cat [0: i in [1..Rank(L)]]; G:=AbelianGroup(inv);
 mp:=map<G->EF|a:-> &+[EF| Eltseq(a)[i]*m(T.i): i in [1..t]] + 
                    &+[EF| Eltseq(a)[i+t]*basis[i]: i in [1 .. #basis]]>;
 return G,mp; end intrinsic;

intrinsic GeometricMordellWeilLattice(E::CrvEll[FldFunG]) -> Lat, Map
{The height pairing lattice of the free quotient of the Mordell-Weil group
of E (over an extension field), together with a map to the elliptic curve.}
...
intrinsic TorsionBound(E::CrvEll[FldFunRat],n::RngIntElt,B::RngIntElt) 
  ...
  if p eq 0 then
    ...
    if B eq 0 then
      ...
      Bp := p^Valuation(B,p);
      if Bp gt 1 then B := B div Bp; end if;
    end if;
    // Is E twisted constant? If not, it is ordinary. Otherwise, 
    // p^e torsion reduces injectively into good fibres.
    // Also, if p > 11 then there can be no p-torsion.
    j := jInvariant(E);
    bEp :=  (p le 11) and (not(j in F));
    if bEp then
      ss_pol := SupersingularPolynomial(p);
    end if; 
  ...
  repeat
	...
	t := #TorsionSubgroup(Ered);
	if p gt 0 then
	  vp := Valuation(t,p);
	  if vp gt 0 then // Ered must be ordinary!
	    B := GCD(B,t div (p^vp)); Bp := GCD(Bp,p^vp);
	  else
	    B := GCD(B,t);
	    if bEp then
	      // check whether Ered is ordinary
	      if not IsZero(Evaluate(ss_pol,jInvariant(Ered))) then
		Bp := 1;
	      end if;
	    else
	      Bp := 1;
      ...
      if ((tries ge n) and (Bp ne 0)) or ((B*Bp) eq 1) then 
        return B*Bp; 
      end if;
    end for;
    e+:=1;
...
intrinsic TorsionBound(E::CrvEll[FldFun],n::RngIntElt,B) -> RngIntElt
  ...
  if p eq 0 then
    ...
    if B eq 0 then
     ...
     // Is E twisted constant? If not, it is ordinary. Otherwise, 
     // p^e torsion reduces injectively into good fibres.
     // Also, if p > 11, if gK=genus(K)=0, or (at a rough estimate)
     // p > Max(29,Sqrt(96gK)), if gK>0, then there can be no p-torsion.
     j := jInvariant(E);
     if DimensionOfExactConstantField(K) eq 1 then
	bEp := not(j in F);
     else
	kc,kcmap := ExactConstantField(K);
	try // success <=> j in kc
	  j := Inverse(kcmap)(j);
	  bEp := false;
	catch e
     ...
     gK := Genus(K);
     bEp :=  bEp and ((gK eq 0) select (p le 11) else
		(p le Max(29,Floor(Sqrt(96*gK)))));
     if bEp then
      ss_pol := SupersingularPolynomial(p);
     end if; 
  else error "Unsupported base ring";
