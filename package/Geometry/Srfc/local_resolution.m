...
// correspond to the same irreducible component of the singular locus
    ...
    // for dimension 0 components, just take the first of equivalent sets
    ss0 := [* ss[1] : ss in sing_sets01[1] *];
    //  and get a list of the other singularities to ignore
    sslst := &cat[[s[i] : i in [2..#s]] : s in sing_sets01[1] | #s ge 2];
    if #sslst gt 0 then
      js := Sort(Setseq(Seqset([s[1] : s in sslst])));
      ss_ign := [*<j,Reverse(Sort([s[2] : s in sslst|s[1] eq j]))> : j in js *];
    else
      ss_ign := [**];
    end if;
...
// in dat.
    ...
    if #js eq 1 then
      if bbad[js[1]] then //remove intersection part over singular locus of patch
	S := ints[js[1]] meet SingularSubscheme(dat[cis[js[1]]][2]);
	if not IsEmpty(S) then
	  return Degree(Complement(ints[js[1]],S));
	end if;
      end if;
      return degs[js[1]];
    end if;
...
//printf "new blow_up_cmps: %o\n%o\n",idx,Sp1;
	...
	if #new_divs gt nnd then
      ...
      sings0,sings1,sings_ign := new_sings(tree,leaves);
//printf "sings0: %o\nsings1: %o\nsings_ign: %o\n",sings0,sings1,sings_ign;
	...
	sings cat:= [* <s[1],((tree[s[1]])`sing_cmpts)[s[2]]> : s in ss *];
      ...
      // and remove the singularities to be ignored so they don't reoccur in later rounds
      for si in sings_ign do
	for i in si[2] do Remove(~((tree[si[1]])`sing_cmpts),i); end for;
      end for;
    end while;
...
{ Returns the intersection matrix of the irreducible exceptional divisors
  ...
  leaving the diagonal ones (self-intersections) as 0.}
    ...
    if assigned dsd`imat then
      ...
      vtime MultsAndInts: mat := int_mat_nd(dat,cls,ld);
    end if;
    if (mat[1,1] ne 0) or not get_self_ints then
      ...
      vtime MultsAndInts: sis := self_ints(B[i],mat,dat,cls,ld);
