...
intrinsic Blowup(X::Sch,Y::Sch : Ordinary := true, Xirred := true) -> Sch, MapSch
{ The blowup of scheme X along subscheme Y. X should be affine, ordinary projective or
  ...
  projective space. The saturation part of the computation is much easier if X
  is irreducible. For simplicity this is assumed : set parameter Xirred to false
  if not true. }
	...
	IX := Ideal(X);
	/* mch - 05/18 - since ReesIdeal doesn't currently correctly saturate
           the result, we must do it here to remove 'redundant components' */
	if Xirred then
	  b := R!1;
	  for f in Basis(IY) do 
	      if f notin IX then b := f; break; end if;
	  end for;
	  Ib,hm1 := ReesIdeal(R,IX,IY : a := b);
	else
         // R/IX not integral. Here, we need to find a set of elements
         // f1,..,fr in IY s.t. each fi is a non-zero divisor in R/IX
         // and IY = <IX,f1,..,fr> - do naive analysis that may fail!
	  _,ps := PrimaryDecomposition(IX);
	  B := [R | b : b in Basis(IY) | &and[b notin p : p in ps]];
	  error if not (IY eq ideal<R|Basis(IX) cat B>),
	   "Didn't find a relative basis for the ideal of Y over the ideal",
	   " of X consisting of non-zero divisors on X";
	  Ib,hm1 := ReesIdeal(R,IX,ideal<R|B>);
	  Ib := &meet[ColonIdeal(Ib,hm1(b)) : b in B];
        end if;
    end if;
    Rb := Generic(Ib);
    n := Rank(Rb)-Rank(R);
