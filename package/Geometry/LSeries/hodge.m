...
intrinsic HodgeStructure
(HS::HodgeStruc : PrintHodgeVector:=false,PHV:=false) -> HodgeStruc {}
 return HodgeStructure(HS`A : PrintHodgeVector:=PrintHodgeVector,PHV:=PHV);
 end intrinsic;
...
{Tensor product of Hodge structures}
 if IsEmpty(H1) or IsEmpty(H2) then
  return HodgeStructure([] : PHV:=H1`phv or H2`phv); end if;
...
intrinsic '*'(m::RngIntElt,H::HodgeStruc) -> HodgeStruc {}
 ...
 if m eq 0 or IsEmpty(H) then return HodgeStructure([] : PHV:=H`phv); end if;
...
{Iterated tensor product of a Hodge structure}
 ...
 if m lt 0 then return Dual(H^(-m)); end if;
 return &*[H : i in [1..m]]; end intrinsic;
...
{Given a Hodge structure, twist it by k (reduce weight by 2k)}
 if k eq 0 then return H; end if;
 if not assigned H`phv then H`phv:=false; end if;
 return HodgeStructure([<h[1]-k,h[2]-k,h[3]> : h in H`A] :
		       PHV:=H`phv); end intrinsic;

intrinsic
EffectiveHodgeStructure
(H::HodgeStruc : PHV:=assigned H`phv and H`phv,
                 PrintHodgeVector:=assigned H`phv and H`phv) ->
 HodgeStruc, RngIntElt
