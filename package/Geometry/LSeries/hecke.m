...
intrinsic HodgeStructure(psi::GrpHeckeElt :
...
{Given a Hecke character, return its Hodge structure (over Q)}
 ...
 g:=[i in ooC select +1 else 0 : i in [1..r]] cat &cat[[0,1] : i in [1..c]];
 return HodgeStructure(0,g : PrintHodgeVector:=PrintHodgeVector, PHV:=PHV);
 end intrinsic;
...
{Determines if the underlying data for two Grossencharacters is internally
 ...
 if NumberField(Order(Modulus(Parent(P1`hecke)))) ne
    NumberField(Order(Modulus(Parent(P2`hecke)))) then return false; end if;
 if P1`hecke ne P2`hecke then return false; end if;
 if P1`field_is_cm and not P2`field_is_cm then return false; end if;
 if P2`field_is_cm and not P1`field_is_cm then return false; end if;
 if P1`field_is_cm and P1`dirich ne P2`dirich then return false; end if;
 if Conductor(P1) ne Conductor(P2) then return false; end if;
 if P1`field_is_cm and P1`type ne P2`type then return false; end if;
...
intrinsic HodgeStructure(gr::GrossenChar :
                         PrintHodgeVector:=false,PHV:=false) -> HodgeStruc
{Given a Grossencharacter, return its Hodge structure (over Q)}
 if not gr`field_is_cm then
  return TateTwist
          (HodgeStructure(gr`hecke : PrintHodgeVector:=PrintHodgeVector,
			             PHV:=PHV),-gr`wt); end if;
 g:=[-Min(t[1],t[2]) : t in gr`type]; g cat:=[1+x : x in g];
 return HodgeStructure(gr`wt,g : PrintHodgeVector:=PrintHodgeVector, PHV:=PHV);
 end intrinsic;
