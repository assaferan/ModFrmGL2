...
// and put this first, then?
  ...
  function FIX(f) C:=Coefficients(f);
   ...
   if AbsolutePrecision(f) eq Infinity() then
    return Polynomial(Coefficients(f)); end if;
  ...
  function cf(p,d: Precision:=0)
   ...
   L1loc:=LocalFactorFun(L1,Precision);
   if d le Degree(L1) then l1:=L1loc(p,d);
   else l1:=Polynomial(Coefficients(L1loc(p,Degree(L1)))); end if;
   L2loc:=LocalFactorFun(L2,Precision);
   if d le Degree(L2) then l2:=L2loc(p,d);
   else l2:=Polynomial(Coefficients(L2loc(p,Degree(L2)))); end if;
