...
function make_complex(f,prec) // MW 9 Dec 2016, Dino bug
 if prec eq 0 then prec:=Precision(ComplexField()); end if;
 return Polynomial(ChangeUniverse(Coefficients(f),ComplexField(prec)));
end function;
...
function TwistedLSeries(C,A,Precision,LCseries,LCAseries)
  ...
  if IsZero(ch) then  return LSeries(1: Precision:=Precision);
    ...
    L:=TensorProduct(LC,LA,[<p,cond,make_complex(loc,Precision)>
