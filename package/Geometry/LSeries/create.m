...
(HS::HodgeStruc,N::RngIntElt,cffun::. :
 ...
 -> LSer {The LSeries of a Hodge structure, conductor, and coeff func}
...
intrinsic Sign(L:: LSer : Force:=false) -> .
{Sign of an L-series, 0 if not computed yet (use CheckFunctionalEquation then)}
 if Force eq true and L`sign eq 0 then _:=CFENew(L); end if;
 return L`sign; end intrinsic;

////////////////////////////////////////////////////////////////////////

intrinsic HodgeStructure(f::ModFrmElt :
...
{The HodgeStructure of a modular form}
 ...
 return HodgeStructure([<0,w-1>,<w-1,0>] :
		       PrintHodgeVector:=PrintHodgeVector, PHV:=PHV);
 end intrinsic;
...
intrinsic LSeries(f::ModFrmElt : Embedding:=0, Precision:=0, Bound:=-1) -> LSer
{L-series associated to a modular form f, assuming that it satisfies
 ...
 Bound determines how many coefficients to use to determine the BaseRing.}
             
  require IsZero(EisensteinProjection(f)): "Modular form must be cuspidal";
  require IsIntegral(Weight(f)) and Weight(f) ge 2: "Weight must be >=2 in Z";
  B:=BaseRing(f :  Bound:=Bound); basetype:=Type(B);
...
intrinsic HodgeStructure(chi::GrpDrchElt :
                         PrintHodgeVector:=false,PHV:=false) -> HodgeStruc
{HodgeStructure associated to a Dirichlet character Chi: (Z/mZ)^* to C^*}
 ...
 require Type(BaseRing(Parent(chi))) in
   [FldCyc,RngInt,FldRat,FldRe,FldCom]:
   "Character must take values in Z, a cyclotomic field, real or complexes";
 return HodgeStructure([<0,0,chi(-1) eq 1 select 0 else 1>] :
		       PrintHodgeVector:=PrintHodgeVector, PHV:=PHV);
 end intrinsic;
...
intrinsic TateTwist(L::LSer,k::RngIntElt) -> LSer
{Given an L-series, take its Tate twist by k (reduce weight by 2k)}
 if k eq 0 then return L; end if; return Translate(L,-k); end intrinsic;
