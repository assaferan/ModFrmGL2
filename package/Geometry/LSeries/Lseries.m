...
function GammaSeries(z0,terms)
  ...
  repeat
    ...
    COEFF:=Coefficients(Qvec[n+1]-Qvec[n]);
    err:=#COEFF eq 0 select 0 else VMax(COEFF); // MW fix
    n+:=1;
...
//  if IsDefined(L`aacoeff,n) then return L`aacoeff[n]; end if;
  ...
  if n eq 1 then return 1; end if; // need not be true in non-Euler case
...
the latter (true, default) is faster but as yet unproved to work in general.
  "CoefficientGrowth" is a name of a function C(x) or C(L,x) that bounds the Dirichlet
