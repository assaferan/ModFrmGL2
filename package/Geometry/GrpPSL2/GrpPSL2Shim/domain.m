...
InternalShimuraSequenceDomain := function(domain,D);
  ...
  if #Y eq 0 then
    ...
    if #Y gt 1 then
      ...
      Sort(~T);
   // Corner case: if an isometric circle meets the real axis at a right angle
   // it could be -pi/2 or pi/2; include both: fixes bug from Aurel Page
      if Abs(T[#T][1]-Pi(RealField(D`prec))/2) lt D`eps then
         T := [<-T[#T][1], T[#T][2]>] cat T;
      end if;
      t := 1;
      while T[t][1] lt 0 do
