...
intrinsic SubgroupFromGens(G::GrpPSL2, gens::SeqEnum, N::RngIntElt) -> GrpPSL2
...
intrinsic PSL2SubgroupFromGens(gens::SeqEnum, R::Rng, N::RngIntElt) -> GrpPSL2
     {returns the subgroup of PSL2(R) generated by gens}
     return SubgroupFromGens(PSL2(R), gens, N);
end intrinsic;
...
intrinsic SubgroupFromMod(G::GrpPSL2, N::RngIntElt, H0::GrpMat) -> GrpPSL2
     {returns the subgroup of G generated by gens.}
     H := New(GrpPSL2);
     H`MatrixGroup := G`MatrixGroup;
     H`BaseRing := G`BaseRing;
     ...
     cosets := Transversal(H`ModLevel, H`ImageInLevel);
     H`FS_cosets := [FindLiftToSL2(c) : c in cosets];
     return H;
end intrinsic;
...
intrinsic PSL2Subgroup(H::GrpMat) -> GrpPSL2
 {returns a subgroup of PSL2(Z) whose image is H (assumes -I in H)}
     ...
     return SubgroupFromMod(PSL2(Integers()), N, H);
end intrinsic;
/*
intrinsic PSL2Subgroup(H::GrpMat) -> GrpPSL2
 {returns a subgroup of PSL2(Z) whose image is H (assumes -I in H)}
     ...
     require Dimension(H) eq 2 : "Group must be of 2x2 matrices";
     require GL(2,BaseRing(H))![-1,0,0,-1] in H : "Currently assume -I in H";
     H0 := H meet SL(2, BaseRing(H));
     new_gens := [FindLiftToSL2(g) : g in Generators(H0)];
     N := Modulus(BaseRing(H));
     cong_gens := [SL(2,Integers())!ElementToSequence(g) : g in Generators(CongruenceSubgroup(N))];
     gens := cong_gens cat new_gens;
     return SubgroupFromGens(PSL2(Integers()), gens, N);
end intrinsic;
*/
