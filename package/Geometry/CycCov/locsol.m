...
TestRealSolvability := function(f);
  if Type(BaseRing(Parent(f))) cmpeq FldRat then  
	...
	K := BaseRing(Parent(f));
	assert Type(K) cmpeq FldNum;
	N := #RealEmbeddings(K.1);
	for i in [1..N] do
		rts := [ r[1] : r in Roots(Polynomial([ RealEmbeddings(c)[i] : c in Coefficients(f)] ))];
		if #rts eq 0 and Evaluate(f,0) lt 0 then return false; end if;
	end for;
	return true;
  end if;
end function;
