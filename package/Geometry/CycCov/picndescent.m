...
procedure qthPowerClasses(inf,f,x0,e,G0,c0,mu,~W,~im1,~Foundim1,~SpaceGenerated,~k_v,~pi);
  ...
  if e ge 2 then
    assert RelativePrecision(x0) + Valuation(x0) ge e;
  end if;
  ...
  if (Ngens(SpaceGenerated) lt dimim) or Safety ge 1 or (not Foundim1 and d mod q ne 0) then
    ...
    for r in RepsForF_v do
      ...
      while (RelativePrecision(fx1) lt (extraval +1)) do
        ...
        ct +:= 1;
        if ct ge 10 and Valuation(ktok_v(Evaluate(f_global,k_vtok(x1)))) eq k_v`DefaultPrecision then
          vprintf ConstantNeighborhoods, 1: " Relative precision of f(x1) too small. x1 appears to be a root of f(x).\n";
		//if this is incorrect (i.e., x1 is not a root) the computation is still correct, but might never terminate.
          break;
        end if;
      end while;
      //Precision is sufficient now
      v1 := Valuation(fx1); 
        ...
        if #G1 eq 0 or (#G1 eq 1 and #{ g : g in G1 | Degree(g) eq 1} eq 1) then
          ...
          if c1 le 0 then
            if #G1 eq 0 then
              ...
              //Check that this neighborhood has a point!
              s := Min({E1 - v1,extraval+1});
	      if s eq  extraval +1 then 
	      //This Neighborhoods has a point.
	      impt := mu(x1);  
          ...
          else // s eq extraval+1
            vprintf ConstantNeighborhoods, 2: "        higher precision needed to ensure neighborhood has a point.\n";
