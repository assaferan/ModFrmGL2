...
//end if;
 ...
 fm := MonicModel(f,q);
 if fm ne f then vprintf Selmer, 1:
     "Changing to the monic model y^q = %o for the curve.\n", fm;
 f := fm; end if; // BC Jan 2019
...
if #SfakePic1 eq 0 then
  ...
  if isSha then 
    ...
    error if q eq 2 and genus mod 2 eq 1,
      "Upper bound is not obtainable.\n"*
      "Upper bound for the rank of Pic^0(X)/2*Pic^0(X) is "*Sprint(r_upper)*".\n";
