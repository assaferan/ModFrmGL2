...
function hgm_trace(X,q,t) b,p,f:=IsPrimePower(q); Z:=Integers(); assert b;
 if q gt 2^25 then error "p^f is too large"; end if; // practical limit
 if q gt 2^28 then error "p^f is too large"; end if; // limit on C code
 pr:=Ceiling(Log(1+2*Floor(Degree(X)*q^(X`weight/2)+1))/Log(p));
 K:=pAdicField(p,pr); KK:=pAdicQuotientRing(p,pr);
 if p eq 2 then T:=1; else T:=teich(t*X`Mvalue*K.1); end if; T:=KK!T;
 if Degree(X) ne 0 then
  if q ne 2 then
   s:=HypgeomMotHelper(X`fps,X`Garray,(Z!T) mod (p^pr),p^f,pr,X`dvalue);
  else s:=0; end if; // sum is empty when q is 2;
  return Z!((q^X`dvalue+s)*KK!(1/(1-q))); end if; // Helper lacks r=0 term
 s:=&+[KK|T^r : r in [0..q-2]]; return Z!(s*KK!(1/(1-q))); end function;

intrinsic
 HypergeometricTrace(H::HypGeomData,t::RngQZElt,q::RngIntElt) -> RngIntElt
{Given a HGM, t in Q, and prime power q with v_p(Mt)=0, return the qth trace}
 require q ge 2: "q must be at least 2";
 b,p,f:=IsPrimePower(q); require b: "q must be a prime power";
 require Valuation(MValue(H)*t,p) eq 0: "Must have v_p(Mt)=0";
 return hgm_trace(H,q,t); end intrinsic;
...
function ef_mul(X,t,p,Precision,Check : t1:=false) deg:=Degree(X);
 ...
 if IsOdd(deg) then // necessarily even weight
  ...
  if p^PREC gt 2^25 then error "p^f is too large"; end if; // practical limit
  if p^PREC gt 2^28 then error "p^f is too large"; end if; // limit on C code
 ...
 if IsEven(Weight(X)) and IsFinite(v) and IsEven(v) then
  m1:=Multiplicity(X`cycB,1); T:=Parent(g).1;
  K:=(-1)^((m1-1) div 2)*2*&*[Abs(x) : x in GammaList(X)];
...
function ef_good(X,t,p,Precision,Check) deg:=Degree(X);
 if Precision eq 0 then prec:=(deg+1) div 2; else prec:=Precision; end if;
 if Precision eq 0 and #X`alpha ne #X`beta then prec:=deg; end if;
 d:=Evaluate(X`char,t); // 2 can be "good" when alpha!=beta ?
...
function ef_fake(X,t,p,Precision) deg:=Degree(X); // utility, don't use func eq
 ...
 b,w:=IsCoercible(PolynomialRing(Z),Polynomial(Coefficients(f)));
 if not b then return Polynomial([1]); else return w; end if; end function;

forward wild_euler;
AbsPrec:=Precision;
intrinsic EulerFactor
(X::HypGeomData,t::RngQZElt,p::RngIntElt :
 Degree:=0,Precision:=0,Check:=false,Fake:=false,WILD:=false) -> .
{The pth Euler factor of the hypergeometic motive specialised at t}
 ...
 if Valuation(X`bad,p) ne 0 and WILD then return F(wild_euler(X,t,p)); end if;
 ...
 if #X`alpha ne #X`beta then return F(ef_good(X,t,p,Degree,true)); end if; // ?
 // probably should check good vs mult in that case...
...
function new_wild_factor(H,t,p) vp:=Valuation(MValue(H),p);
 ...
 dH:=Degree(H)-(t ne 1 select 0 else (IsOdd(Weight(H)) select 2 else 1));
 if Valuation(t*p^vp-1,p) gt 0 then ef:=ef_mul(pH,t*p^vp,p,dH,true);
 else ef:=EulerFactor(pH,t*p^vp,p : Check,Degree:=dH); end if;
...
function wild_euler(X,t,p)
 if t eq 1 then ef:=get_euler_t1(X,p);
  if not Type(ef) eq BoolElt then return PolynomialRing(Z)!ef; end if;
  return PolynomialRing(Z)!1; end if;
 f:=new_wild_factor(X,t,p);
 if f ne 1 and IsCoercible(PolynomialRing(Z),f) then
  return PolynomialRing(Z)!f; end if;
 spin:=func<N|&*[d^(d*MoebiusMu(N div d)) : d in Divisors(N)]>;
 Mn:=&*[spin(x) : x in X`cycA]; Md:=&*[spin(y) : y in X`cycB];
...
function wild_cond(X,t,p)
 ...
 if vv gt 0 then
  ...
  return frv_guess(X,vt,p)-e-ew; end if;
 // above cases handle everything, below can be ignored
...
function hodge_struc_HGt1(X,PHV)
 w:=X`weight; f:=Numerator(X`hodge); d:=#X`alpha;
 if w eq 0 then HS:=HodgeStructure([] : PHV:=PHV);
 else HS:=&+[(Z!Coefficient(f,i))*
              HodgeStructure([<i,w-i,0>,<w-i,i,0>] : PHV:=PHV) :
 	     i in [0..(w-1) div 2]]; end if;
 if IsEven(w) then w2:=w div 2; c:=Z!Coefficient(f,w2);
  HS:=HS+(c div 2)*HodgeStructure([<w2,w2,0>,<w2,w2,1>]); // hack
 ...
 if IsEven(w) and IsEven(d) then
  HS:=HS-HodgeStructure([<w2,w2,0>,<w2,w2,1>]);
...
function hodge_struc_HG(X,t,PHV)
 if t eq 1 then return hodge_struc_HGt1(X,PHV); end if;
 ...
 return HodgeStructure(X`weight,&cat GAMMA : PHV:=PHV); end function;

intrinsic HodgeStructure(H::HypGeomData,t::RngQZElt :
...
{The Hodge structure of hypergeometric data at t}
 ...
 return hodge_struc_HG(H,t,PrintHodgeVector or PHV); end intrinsic;

intrinsic HodgeStructure(H::HypGeomData :
                         PrintHodgeVector:=false,PHV:=false) -> HodgeStruc
{The Hodge structure of odd weight hypergeometric data}
 require #H`alpha eq #H`beta: "Hypergeometric data must be balanced";
 require IsOdd(Weight(H)): "Weight must be odd, else specify t";
 return hodge_struc_HG(H,2,PrintHodgeVector or PHV); end intrinsic;
...
function lseries_t1(X,BAD,GAMMA,PR,ZETA,SE)
 ...
 else HS:=hodge_struc_HGt1(X,false); end if;
...
function unknown_lseries(X,t,BadPrimes,GAMMA,PR,ZETA,SE)
 ...
 cf:=function(p,d) if IsDefined(BAD,p) then return BAD[p][2]; end if;
                   return EulerFactor(X,t,p : Precision:=d,WILD); end function;
 WILD_PRIMES:=Set(PrimeFactors(X`bad));
 ...
 if #GAMMA eq 0 then GAMMA:=hodge_struc_HG(X,t,false);
...
function get_lseries0(X,t,BadPrimes,GAMMA,P,Identify,ZETA,SE)
 ...
 if Identify then
  ...
  if X`weight eq 1 and t ne 1 then T:=IDENTIFY(X,t);
   if Type(T) eq CrvEll then return LSeries(T : Precision:=P); end if;
   if Type(T) eq CrvHyp then
    return LSeries(T : Precision:=P,LocalData:="Ogg"); end if;
...
function Tfr(A,B,p,f,r) q:=p^f;
 return &+[Q|FractionalPart(p^i*(a+r/(q-1))) : a in A,i in [0..f-1]]
       -&+[Q|FractionalPart(p^i*(b+r/(q-1))) : b in B,i in [0..f-1]];
end function;

function XandTqr(A,B,p,f,r,Qp) q:=p^f;
 Av:=&*[Qp|Gamma(Qp!FractionalPart(p^i*(a+r/(q-1)))) : i in [0..f-1], a in A];
 Bv:=&*[Qp|Gamma(Qp!FractionalPart(p^i*(b+r/(q-1)))) : i in [0..f-1], b in B];
 Xqr:=Av/Bv; mr:=Lr(B,p^f,r); return (Qp!Xqr)/q^mr,Tfr(A,B,p,f,r);
 end function;

function get_traceK(A,B,d,p,f,t,Qp)
 q:=p^f; C0,v:=XandTqr(A,B,p,f,0,Qp); u:=p^(f*d); T:=teich(t*Qp.1);
 for r in [1..q-2] do
  Cr,w:=XandTqr(A,B,p,f,r,Qp); u:=u+q^d*T^r*(Cr/C0)*(Qp!-p)^(Z!(w-v)); end for;
 return u/(1-q); end function;
...
{Given arrays of rationals for alphas and betas and a prime power q
 ...
 require #A eq #B: "Number of alpha's must equal number of beta's";
 require q ge 2: "q must be at least 2";
 b,p,f:=IsPrimePower(q); require b: "q must be a prime power";
 require Type(Precision) eq RngIntElt and Precision ge 1: "Bad precision";
 if p eq 2 then Precision:=Precision+1; end if; Qp:=pAdicField(p,Precision);
 require Valuation(t,p) eq 0: "t must have zero valuation at p";
 require Valuation(LCM([Z|Denominator(x) : x in A cat B]),p) eq 0:
         "p cannot be wild for alpha's or beta's";
 ...
 if #A eq 0 then d:=0; else d:=Degree(Denominator(F)); end if;
 return get_traceK(A,B,d,p,f,t,Qp); end intrinsic;

/*
PHD:=PossibleHypergeometricData(4); t:=3; p:=13;
for h in PHD do H:=HypergeometricData(h);
 HypergeometricTrace(H,t,p) eq Z!HypergeometricTraceK(H`alpha,H`beta,t,p);
 end for;
*/

// alpha=beta=[] case, think is -1 when t=1 mod p, else 0

intrinsic HypergeometricTraceK
(A::SeqEnum[RngQZElt],B::SeqEnum[RngQZElt],t::FldPadElt,q::RngIntElt
 : Precision:=5) -> FldPadElt
{Given arrays of rationals for alphas and betas and a prime power q
 and a compatible p-adic specialization t, return the HypergeometricTrace}
 require #A eq #B: "Number of alpha's must equal number of beta's";
 require q ge 2: "q must be at least 2";
 b,p,f:=IsPrimePower(q); require b: "q must be a prime power";
 require Type(Precision) eq RngIntElt and Precision ge 1: "Bad precision";
 ...
 require Valuation(LCM([Z|Denominator(x) : x in A cat B]),p) eq 0:
         "p cannot be wild for alpha's or beta's";
 Kv:=Parent(t); require p eq Prime(Kv): "Non-compatible p-adic fields";
 /* g:=AbsoluteMinimalPolynomial(t); Kv:=LocalField(Qp,g);
    R:=RamifiedRepresentation(Kv); t:=Roots(g,R)[1][1]; */
 require Valuation(t) eq 0: "t must have zero valuation at p";
 A:=Sort([FractionalPart(a) : a in A]); B:=Sort([FractionalPart(b) : b in B]);
 s:=0; T:=FunctionField(Rationals()).1; F:=Parent(T)!0;
 S:=[<x,0> : x in A] cat [<1-x,1> : x in B]; Sort(~S);
 for i in [1..2*#A] do
