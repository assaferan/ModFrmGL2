...
function jacket_mot_ef(J,p : deg:=J`deg) assert J`m ne 1; // triv case
 ...
 if #S gt 16 then prec:=2*prec+10; end if; // HACK, Jan 2019
 ...
 if #ROO gt 16 then // random limit, Jan 2019
  ROO:=[-r : r in ROO]; z:=PolynomialRing(Universe(ROO)).1;
  F:=Polynomial(Reverse([Z!c : c in Coefficients(&*[z-r : r in ROO])]));
 else // use symmetric functions rather than rote multiplication
  W:=[&+[&*[Kp|ROO[u] : u in U] : U in Subsets({1..#ROO},k)] : k in [0..#ROO]];
  F:=Polynomial([p^z*(Z!(a/p^z)) where z:=Valuation(a): a in W]); // Magma bug
  end if; return Evaluate(F,Parent(F).1^v),Sort(RES); end function;
...
{Given a Jacket motive and an ideal of prime norm in the field of definition,
 which induces to a prime norm in the cyclotomic field too,
...
// require J`t eq 1: "Must be Jacobi motive for now (t=1)";
 ...
 require IsPrime(Norm(P)): "Ideal does not have prime norm in cyclo field";
...
intrinsic EffectiveHodgeStructure
(J::JacketMot : PrintHodgeVector:=false,PHV:=false) -> HodgeStruc
{The effective Hodge structure of a Jacket motive}
 return EffectiveHodgeStructure
         (J`HS : PrintHodgeVector:=PrintHodgeVector, PHV:=PHV); end intrinsic;
...
function init_jacket_mot(POS,NEG,t,rho,tate) rho:=FRAC(rho);
 ...
 for k in EulerPhiInverse(Degree(K)) do C:=CyclotomicField(k);
  if IsIsomorphic(K,C) then K:=C; break; end if; end for;
...
// require J`t eq 1: "Must be a Jacobi motive (t=1) for now";
 ...
 // if J`m eq 4 and J`t eq 1 then I:=4*Integers(J`K); end if;
 // another spec case: hmm, FAILED for JacobiMotive([1/4,3/4]) ?
 // guess the point is that J`K is reduced to Q in that case
 // in reality, could special-case all small J`m, if really wanted to
 ...
 DATA:=[* *]; q:=2; u:=S.0; U:=S; pi:=Pi(RealField(prec)); bU:=0; miss:=0;
 while #S ne 1 do // HACK, skip if S.0 is only element
  ...
  for P in Ps do ev:='@'(P,psi : Precision:=prec); Q:=[];
   ...
   if #Set(Q) gt 1 then // maybe check a full orbit of conjugates exists?
    ...
    try r:=1/ComplexEvaluation(J,P : Precision:=prec); z:=Log(ev*r)/2/pi;
     appr:=BestApproximation(Imaginary(z),1000*o^2); // hopefully this works...
     assert o mod Denominator(appr) eq 0; Q:=[FractionalPart(appr)];
    catch e; continue; end try; end if; // P needs to be OK in cyclo field
   DATA cat:=[* <P,-Integers(Denominator(Q[1]))!Numerator(Q[1])> *]; end for;
  u,U:=HeckeCharacter(S,DATA : RequireGenerators:=false);
  if #U eq 1 then break; end if;
  if #U eq bU then miss:=miss+1; else miss:=0; bU:=#U; end if;
  if miss eq 10 then
   printf "Didn't get unique kernel (size %o)\n",#U; break; end if; end while;
