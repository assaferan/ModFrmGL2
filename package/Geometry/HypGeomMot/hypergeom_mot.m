...
{Return the degree of given hypergeometric data}
 return Max(#X`alpha,#X`beta)+#X`common_ab; end intrinsic;

intrinsic DefiningPolynomials(X::HypGeomData) -> RngUPolElt, RngUPolElt
{The polynomials (products of cyclotomics) that define X}
 C:=X`common; R:=Parent(X`alpha_poly);
 CP:=&*[R|CyclotomicPolynomial(c) : c in C];
 return CP*X`alpha_poly,CP*X`beta_poly; end intrinsic;

intrinsic CyclotomicData(X::HypGeomData) -> SeqEnum, SeqEnum
{The cyclotomic data for X}
 return Sort(X`cycA cat X`common),Sort(X`cycB cat X`common); end intrinsic;

intrinsic AlphaBetaData(X::HypGeomData) -> SeqEnum, SeqEnum
{The alpha-beta data for X}
 return Sort(X`alpha cat X`common_ab,X`beta cat X`common_ab); end intrinsic;
...
{Whether two instances of hypergeometric data are equal}
 return X`alpha eq Y`alpha and X`beta eq Y`beta and X`common eq Y`common;
 end intrinsic;
...
intrinsic MonodromyGroup(H::HypGeomData) -> GrpMat
{The monodromy group associated to H (finite when weight 0 and no common data)}
 require #H`alpha eq #H`beta: "H must have same number of alpha and beta";
 f,g:=DefiningPolynomials(H); A:=CompanionMatrix(f); B:=CompanionMatrix(g);
 G:=sub<GL(Degree(H),Integers())|A,B>; return G; end intrinsic;
...
function hgm_init(ALPHA,BETA,print_type) ALPHA:=Sort(ALPHA); BETA:=Sort(BETA);
 ...
 if #ALPHA ne #BETA then // unbalanced case  // Apr 18
  wt:=Max(#ALPHA,#BETA)-1; if IsOdd(wt+1+&+X`Garray) then wt:=wt-1; end if;
  X`hodge:=0; X`weight:=wt;
  X`dvalue:=(wt+1-2*Multiplicity(BETA,0)-&+X`Garray) div 2; end if;
...
intrinsic HypergeometricData // everything goes through this, I think
...
{The hypergeometric data given by alphas and betas (rationals)}
 ...
 H:=hgm_init(ALPHA,BETA,Print); // unique call
...
{Given a hypergeometric datum, swap the alpha and beta}
 return HypergeometricData
 (H`beta cat H`common_ab,H`alpha cat H`common_ab :
  Print:=H`print_type,SameSize:=#H`alpha eq #H`beta); end intrinsic;
