...
intrinsic Degree(phi::MapSch)->RngIntElt
  {Degree of a non-constant morphism between curves}
  if assigned(phi`degree) then return phi`degree; end if;
  ...
  boo,Dom := IsCurve(Domain(phi));
  if boo then 
    boo,Cod := IsCurve(Codomain(phi));
  end if;
  require boo: "Only implemented for maps between curves";
  if not (IsIdentical(Dom,Domain(phi)) and IsIdentical(Cod,Codomain(phi))) then
    phi`degree := Degree(map<Dom -> Cod | DefiningPolynomials(phi)>);
  end if;
  if not(assigned(phi`degree)) then
    C:=Codomain(phi);
