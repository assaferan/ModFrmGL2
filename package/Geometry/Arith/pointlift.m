...
function Hensel(F,dim,P,tprc:rand:=false,Strict:=true)
  ...
  *****/
  ...
  repeat
    ...
    flag:=false;
    for i in [1..#F] do v:=MinValuation(F[i]);
       if v gt 0 then
          for u in [v..1 by -1] do // MW, May 2017
              TEMP:=ShiftVal(F[i],-u);
              if Evaluate(TEMP,[Rtok(p) : p in P]) eq 0 then
                 vprint LocSol,2:
                 "Extra content",u,"removed from",i,"th polynomial";
                 F[i]:=TEMP; flag:=true; break u; end if;
              if u eq 1 then
                 vprint LocSol,2: "Non-removable content in",i,"th polynomial";
              end if; end for;
       end if;
    end for;
  until not(flag); 
