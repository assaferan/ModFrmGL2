...
intrinsic WittInvariants(M::AlgMatElt : Minimize:=false,AA:=false) -> SeqEnum
{Returns the Witt (Hasse-Minkowski) invariants at all bad primes of
 ...
 if Minimize then E:=[e : e in E | e[1] le 2 or e[2] eq -1 or
                                   IsOdd(Valuation(D,e[1]))]; end if;
 if AA then U:=AssociativeArray(); for x in E do U[x[1]]:=x[2]; end for;
  return U; end if; return E; end intrinsic;

intrinsic WittInvariants(L::Lat : Minimize:=false,AA:=false) -> SeqEnum
{Returns the Witt (Hasse-Minkowski) invariants
 at all bad primes of the quadratic form given by the lattice L}
 return WittInvariants(GramMatrix(L) : Minimize:=Minimize,AA:=AA);
 end intrinsic;

intrinsic WittInvariants(f::RngMPolElt : Minimize:=false,AA:=false) -> SeqEnum
{Computes the Witt (or Hasse-Minkowski) invariants
 at all bad primes of the quadratic form given by f}
 return WittInvariants(SymmetricMatrix(f) : Minimize:=Minimize,AA:=AA);
 end intrinsic;
...
intrinsic HasseMinkowskiInvariants
(M::AlgMatElt : Minimize:=false,AA:=false) -> SeqEnum
{Returns the Witt (Hasse-Minkowski) invariants at all bad primes of
 the quadratic form given by the symmetric matrix M}
 return WittInvariants(M : Minimize:=Minimize,AA:=AA); end intrinsic;

intrinsic HasseMinkowskiInvariants
(L::Lat : Minimize:=false, AA:=false) -> SeqEnum
{Returns the Witt (Hasse-Minkowski) invariants
 at all bad primes of the quadratic form given by the lattice L}
 return WittInvariants(L : Minimize:=Minimize,AA:=AA); end intrinsic;

intrinsic HasseMinkowskiInvariants
(f::RngMPolElt : Minimize:=false, AA:=false) -> SeqEnum
{Computes the Witt (or Hasse-Minkowski) invariants
 at all bad primes of the quadratic form given by f}
 return WittInvariants(f : Minimize:=Minimize, AA:=AA); end intrinsic;
