...
*/
  val := Domain(find_coset)!Eltseq(x);
  // This could be left out if we use a different function for Hecke
  if not IsCoercible(ModLevel(G), val) then return 0,0; end if;
  index_g := find_coset(val);
  g := Domain(find_coset)!Eltseq(index_g[2]);
  s := val*g^(-1);
  return index_g[1], s;
...
// This function returns a map from elements in M2Z with
// determinant which is invertible modulo the level
// to a corresponding coset of G

// At the moment, we just build a map by going
// over all points in the reduction mod N

// TODO : be able to use any codomain (not necessarily CosetRepresentatives(G)
// TODO : Add TMTO (using Hellman)

function get_find_coset(G)
  ...
  find_coset := map<M2Z -> codom |
                    x :-> find_coset_N(ModLevel(G)!x)>;
  return find_coset;
end function;
