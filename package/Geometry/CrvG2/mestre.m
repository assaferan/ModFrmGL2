...
intrinsic HyperellipticCurveFromIgusaClebsch(
    ...
    involutions) defined over the field in which the invariants lie. 
    The base field must be the rationals or a number field or a finite field of
    characteristic bigger than 5. In case such a curve does not exist a curve
    defined over some quadratic extension is returned.}
    ...
    require Type(F) in {FldFin,RngInt,FldRat} or ISA(Type(F),FldNum) :
...
function FindPointOnConic(L) // ::RngMPolElt) 
	...
	until t;
	return x1,x2;
    elif Type(K) eq FldRat or ISA(Type(K),FldNum) then
	P<x,y,z> := ProjectiveSpace(K, 2);
	C := Conic(P, L);
	bool, S := HasRationalPoint(C);
	if not bool then
	  error if ISA(Type(K),FldNum),
	    "Not implemented for curves over number fields "*
            "not defined over their fields of definition."; // whatever
