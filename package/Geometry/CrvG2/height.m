...
{Computes the canonical height of P.}
  // Some checks
  require Type(Precision) eq RngIntElt: "Precision must be an integer";
  require Precision ge 0: "Precision cannot be nonnegative";
  ...
  RF:=Precision eq 0 select RealField() else RealField(Precision); // MW
  K := Parent(P);
  if P eq K!0 then
    vprintf JacHypHeight:" P = 0, so height is zero.\n";
    return TR eq FldRat select RF!0 else 0;
  end if;
  D := K`Delta;
  if TR eq FldRat then // this is rather unreadable ... break into functions?!

    J:=Jacobian(K); pt:=Points(J,P)[1]; o:=Order(pt); // MW
    if o ne 0 then
      vprintf JacHypHeight:" %o*P = 0, so height is zero.\n", o;
      return RF!0; end if;

    eqn := DefiningPolynomial(K);
    Pol := PolynomialRing(RF); // MW: was RealField
    x := Pol.1;
    ...
    for n := 1 to bound do
      ...
      if Degree(eqnIns) eq -1
        or Degree(eqnIns) eq 0 and Abs(ConstantCoefficient(eqnIns)) lt 10^-prec
      // MW: this is *sooooo* dodgy: should use exact test at top?
      // Currently problems with [1,-4,3,5,-4,-3] and pt order 2
      then
        vprintf JacHypHeight, 2:
      ...
      // Patch by Steve:
      eqnIns /:= LeadingCoefficient(eqnIns); // Roots likes monic (05-09,Steve)
      r := [ r[1] : r in Roots(eqnIns) ];
      if #r eq 0 then   
    ...
    return height;
  elif TR eq FldFunRat then  // still rather unreadable ...
