...
function irreducible_norm_decompositions(f)
  ...
  if ISA(Type(k),FldFin) then
    ...
    if #Fg eq 3 and d eq 3 then
      //this is the case of S3 acting transitively on 6 roots.
      //there are 3 different norm expressions in that case.
      //for each cubic factor of Fg, we are finding all three 
      fct:=Factorisation(Polynomial(ext<k|Fg[1]>,f));
      cnd:=[c[1]: c in fct | Degree(c[1]) eq 2];
      assert #cnd eq 3;
      return [* c : c in cnd *];
    end if;
      
    //the only transitive subgroup of S4 that admits multiple norm expressions
    //is V4. We need each factor of Fg individually.
    //For any of the other transitive subgroups of S6 and S4, there is only
    //one norm expression, so there is only one factor of Fg, and there is only
    //one expression to select. In those cases, the code below also does
