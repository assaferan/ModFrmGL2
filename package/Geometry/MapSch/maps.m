...
{Function for internal use}
    ...
    E1 := [[incl_hom(e) : e in f]: f in E];
    I_gr := ideal<PG|[incl_hom(b) : b in Basis(I)] cat
	&cat[[PG.(n+i)*fs[j]-PG.(n+j)*fs[i] : j in [i+1..m], i in [1..m]]: fs in E1]>;

    vprint SchImage : "Saturating..";
    if irred then
      // only need to saturate by one non-zero elt of one sets of eqns in E
      i := 1;
      while i le m*(#E) do
	if not IsInRadical((&cat(E))[i],I) then break; end if;
	i +:= 1;
      end while;
      vtime SchImage : I_gr_sat := ColonIdeal(I_gr,(&cat(E1))[i]);
    else
      vtime SchImage : I_gr_sat := &meet[ColonIdeal(I_gr,e) : e in &cat(E1)];
    end if;
