...
function LPolynomialOld(C) // CrvHyp[FldFin])
   R := CoefficientRing(C);
   error if not HasFunctionField(C), "Function field of curve must be computable";
   if Genus(C) eq 0 then //easy case!
       ...
       if assigned J`EulerFactor then
           ...
           usezeta,meth,C1,tw := UseZetaMethod(C : Zeta);
           ext_deg := Degree(R) div Degree(BaseRing(C1));
           if usezeta then
               charpol := ApplyZetaMethod(meth,C1,tw,ext_deg);
...
function UsePAdicMethods(C : Zeta:=false)

  p := Characteristic(BaseRing(C));
  if not Zeta and p gt 1000 then 
     return 0,0,0;    // characteristic too large
     // TO DO criterion makes no sense, but something is needed here
  elif p gt 2 then
...
/* SRD:
   'Zeta:=true' means we have to return the zeta function, 
   i.e. group methods are not an option (in general)
*/
function UseZetaMethod(C : Zeta:=false)
    ...
    twist,C2,methno := UsePAdicMethods(C1 : Zeta:=Zeta);
    if twist eq 0 then 
        return false,0,C1,1;
    else
  ...
  case method_number:
    when 1: //use naive count
     ...
     when 2: // C is a CrvEll
      ...
      charpol := PolynomialRing(IntegerRing())![1,(#C-q-1),q] where
                         q is #BaseRing(C);
    when 3: // use Kedlaya
