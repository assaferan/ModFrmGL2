...
(of some hyperelliptic curve over Q).}
 ...
 // end if;
  ...
  error if not BoundIsProven,
    "Upper bound is not obtainable.\n"*
    "Upper bound for the rank of Pic^0(X)/2*Pic^0(X) is "*Sprint(R)*".\n";
...
(of some hyperelliptic curve over a number field).}
  ...
  if BaseField(J) cmpeq Rationals() then
    ...
    //end if;
      Ind1Locally := HasIndexOneEverywhereLocally(C);
      BoundIsProven := Genus(C) eq 2 or Ind1Locally;
    error if not BoundIsProven,
      "Upper bound is not obtainable.\n"*
      "Upper bound for the rank of Pic^0(X)/2*Pic^0(X) is "*Sprint(r)*".\n";
    if Degree(C) mod 2 eq 0 then
      ...
      if #SelPic eq 0 and BoundIsProven then
        //PIC1 may not be divisible by 2 in Sha
        if IsOdd(J) then
		r -:= 1;
...
{The 2-Selmer group of the Jacobian J (of some hyperelliptic curve over a number field).}
  ...
  if Al eq creutz then
     A := Domain(StoA); assert ISA(Type(A), Rng);
     // and Codomain(StoA) is a group that contains S
     StoA := map< S -> A | s :-> s@@StoA, a :-> S!(a@StoA) >;
  end if;
