...
intrinsic Label(G::GrpPSL2) -> MonStgElt
{}
  if not assigned G`Label then
    ...
    /*
     if assigned G`FS_cosets then
	   ...
	   G`Label := G`Label cat Sprintf("%o\n,", G`FS_cosets[coset_idx]);
        end for;
        if #G`FS_cosets gt 5 then
	   G`Label := G`Label cat
	        Sprintf("... (there are %o cosets)\n", #G`FS_cosets);
        end if;
     // end if;
     elif assigned G`Generators then
        num_printed_gens := Minimum(5, #G`Generators);
        G`Label := "Arithmetic subgroup of PSL2 generated by\n";
        for gen_idx in [1..num_printed_gens] do
	    G`Label := G`Label cat Sprintf("%o,\n", G`Generators[gen_idx]);
        end for;
        if #G`Generators gt 5 then
	     G`Label := G`Label cat
