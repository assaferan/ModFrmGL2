...
function cc(a)
    ...
    V, embinv := VectorSpace(K, BaseRing(K));
    emb := embinv^(-1);
    return &+[cc_Q21(Eltseq(a)[i])*emb(Basis(V)[i]) : i in [1..Dimension(V)]];
end function;
...
function GaussSum(eps)
    if Order(eps) eq 1 then
	return Q21!1;
    end if;
    return &+[Q21!zeta7^i*Q21!eps(i) : i in [0..K-1]];
end function;
...
function pr35(flist, check)
    ...
    embs := [e : e in embs | &and[e(Coefficient(fold, i)) eq flist[i]
				  : i in [1..check]]];
    assert #embs eq 1;
    return [0] cat [embs[1](Coefficient(fold, i)) : i in [1..prec-1]];
end function;

function chartwist(flist, i)
    return [chi(n)^i*flist[n] : n in [1..#flist]];
end function;

function make_real_twist_orbit(alist, Kf_to_KK,Tpluslist, prec)
    Kf := Domain(Kf_to_KK);
    fpos :=[g : g in NN |
    ...
    assert #embs eq 1;
    emb := embs[1];
    flist := [0] cat [cc(Kf_to_KK(emb(Coefficient(f,i)))) : i in [1..prec-1]];
    if f in N49 then
	...
	T := Tpluslist[i];
	intsn := intsn meet Kernel(ChangeRing(T, Kf) - Kf!alist[i+2]);
    end for;
    Hb := [ChangeRing(x*BasisMatrix(Cplus),Kf) : x in Basis(intsn)];
	...
	ftwists cat:= [[Q21!gauss_sum(chi^(-2*i)))*a : a in ftw];
	mutw := Rop(chi^(-2*i))*mu;
	mutwists cat:= [mutw];
	if mutw in ChangeRing(oldspace35, Kf) then
	    mutwists cat:= [ChangeRing(Tr357_mat, Kf)*pi35(mutw)
			    / #CosetReps357];
	    ftwpr35 := pr35(ftw, #alist);
	    ftwpr35B7 := &+[Q21!gauss_sum(chi^(-2*i))*ftwpr35[j+1]*qKK^(7*j)
			    : j in [0..prec-1]];
	    ftwists cat:= [Eltseq(ftwpr35B7) cat
			   [0 : i in [1..prec-#Eltseq(ftwpr35B7)]]];
	    Append(~powerlist, i);
	elif mutw in ChangeRing(oldspace49, Kf) then
	    mutwists cat:= [ChangeRing(Tr495_mat, Kf)*ChangeRing(B491, Kf)
			    * mutw / #CosetReps495];
	    ftwB7 := &+[Q21!gauss_sum(chi^(-2*i))*ftw[j+1]*qKK^(5*j)
			    : j in [0..prec-1]];
	    ftwists cat:= [Eltseq(ftwprB7) cat
			   [0 : i in [1..prec-#Eltseq(ftwB7)]]];
	    Append(~powerlist, i);
	end if;
    end for;
    return sub<Universe(mutwists) | mutwists>, ftwists, powerlist;
end function;
...
5],[1, 2,0, 5],[ 50,-11,2205, -485],[71,-32,2940,-1325]]];
    ...
    Q21rt2<rt2> := NumberField(x21^2-2);
    Q21rt17<rt17> := NumberField(x21^2-17);
    Q7<zeta7> := sub<Q21 | zeta21^(3*4)>;
    Q7plus<z7plus>, Q7plus_to_Q7 := sub<Q7 | zeta7 + zeta7^(-1)>;
    Q3<zeta3> := sub<Q21 | zeta21^(-7)>;
    ...
    a2s := a2s cat [* a2stw[i] : i in [1..5] *];
    a3s := a3s cat [* a3stw[i] : i in [1..5] *];
    ...
    assert Dimension(oldspace35) eq Dimension(oldspace351) + Dimension(oldspace357);
    assert Dimension(oldspace49) eq Dimension(oldspace491) + Dimension(oldspace495);
